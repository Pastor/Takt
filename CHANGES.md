...
- Зарегистрирована и проработана до стадии «Анализ» (без разработки) фича **0021**
  «Смена операторов: `<=` присваивание, `=` сравнение»: созданы карточка, ADR
  (`docs/adr/0021-*`, с EBNF-схемами «как есть»/«целевое», правило 18) и анализ
  (`docs/analyze/0021-*`) с разбором влияния на понятность, читаемость,
  многословность и совместимость. Ключевой вывод: прямой запрос (`<=` присваивание,
  `=` сравнение) отвергнут — коллизия с реляционным `<=` и расхождение с C-целью
  компиляции; рекомендован эквивалентный по цели **Option B** — `:=` присваивание
  (стиль ST/IEC 61131-3), `=` сравнение, с сохранением реляционного `<=`. Любой
  вариант — слом совместимости (мажорная версия языка + мигратор), поэтому решение
  о внедрении оставлено за заказчиком; в разработку фича не берётся. Статус:
  `СОЗДАНА → АНАЛИЗ`.
