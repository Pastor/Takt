...
export const TICK_HZ_MAX = 1_000_000_000;

/**
 * Частота модельных часов прогона из поля ввода, Гц - то же, что `clock N Hz` в модели.
 *
 * Целая: объявление `clock` дробной частоты не знает. Пусто, негодное и не больше
 * нуля - ноль, частота не задана: такт берётся из `clock` модели, иначе 1 мс.
 * Больше предела - предел. С задержкой не связана: задержка - пауза реального
 * времени между тактами на экране, а частота - модельное время одного такта.
 *
 * @param {string|number} value
 * @returns {number}
 */
export function tickHz(value) {
  const hz = Math.floor(Number(value));
  if (!Number.isFinite(hz) || hz <= 0) return 0;
  return Math.min(hz, TICK_HZ_MAX);
}
