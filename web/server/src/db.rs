...
const MIGRATIONS: &[(i64, &str)] = &[
    ...
    // 9 -> 10: частота модельных часов прогона по сценариям.
    (
        9,
        "ALTER TABLE projects ADD COLUMN IF NOT EXISTS run_frequencies TEXT NOT NULL DEFAULT '{}';",
    ),
    // 10 -> 11: наблюдаемые выходы прогона по моделям.
    (
        10,
        "ALTER TABLE projects ADD COLUMN IF NOT EXISTS run_watch TEXT NOT NULL DEFAULT '{}';",
    ),
];
...
CREATE TABLE projects (
    ...
    -- Частота модельных часов прогона по сценариям, Гц: объект JSON "имя
    -- сценария - целое". Форма и ключ те же, что у задержки, и правит их тот же
    -- перенос при переименовании и удалении сценария.
    run_frequencies  TEXT NOT NULL DEFAULT '{}',
    -- Наблюдаемые выходы прогона по моделям: объект JSON "имя файла модели -
    -- список имён портов". Правило ключа то же, что у задержки.
    run_watch        TEXT NOT NULL DEFAULT '{}',
