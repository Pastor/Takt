...
/**
 * Фабрика языкового сервера Lam для LSP4IJ (фича 0038, задача 0038-01).
 *
 * Регистрируется точкой расширения `com.redhat.devtools.lsp4ij.server`
 * (`lam-lsp4ij.xml`) и сопоставляется языку Lam через `languageMapping`. Поднимает
 * готовый сервер `lam-lsp` (фича 0011) по stdio — семантическую подсветку,
 ...
 */
class LamLspServerFactory : LanguageServerFactory {

    override fun createConnectionProvider(project: Project): StreamConnectionProvider =
        LamLspConnectionProvider()
}

/**
 * Транспорт запуска `lam-lsp` (stdio) поверх [OSProcessStreamConnectionProvider]
 * (рекомендован LSP4IJ — использует `OSProcessHandler` для отслеживания процесса).
 *
 * Тихая деградация (R3): если бинарник не разрешён ([LamLspBinary.resolve] →
 ...
 */
class LamLspConnectionProvider : OSProcessStreamConnectionProvider() {
    init {
        val binary = LamLspBinary.resolve(LamLspSettings.getInstance().serverPath)
        if (binary != null) {
