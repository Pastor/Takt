...
pub(crate) enum EvalError {
    ...
    /// Номер бита вне разрядов значения (фича 0307).
    ///
    /// `width` — сколько разрядов у значения **на самом деле**: 64 у целого
    /// (ширина носителя), `1` у логического, `слов × 64` у бит-вектора
    /// `[bit;N > 64]` (фича 0078). Прежде текст всегда называл `0..64`, и на
    /// 96-битном векторе это было **ложью**: разряд 70 тот же эталон исполняет.
    ...
    pub(crate) fn message(&self) -> String {
        match self {
            ...
            EvalError::BitIndexOutOfRange { bit, width } => {
                // ⚠️ Граница печатается ВКЛЮЧИТЕЛЬНО: «0..128» на 128-разрядном
                // значении читается как «128 допустим», а он — первый
                // недопустимый. У логического разряд один, и множественное
                // число там звучало бы ошибкой.
                if *width == 1 {
