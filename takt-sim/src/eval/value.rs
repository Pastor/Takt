...
pub enum Value {
    /// Целое значение.
    ///
    /// Носитель — `i128` (фича 0157): `u64` — тип языка, и его верхняя половина
    /// в знаковый 64-битный не укладывается. Ширина **типа** значения этим не
    /// задаётся: усечение и обёртку по объявленному типу делает
    /// [`crate::eval::coerce_to_type`] (ADR 0127), а `i128` лишь даёт запас,
    /// чтобы промежуточный результат не переполнялся там, где итог законен.
    Number(i128),
    Real(f64),
    Boolean(bool),
    Array(Vec<Value>),
    ...
    Fixed {
        ...
        m: u8,
        n: u8,
        /// Насыщение вместо переноса (фича 0170, `q(m, n) sat`).
        ///
        /// ⚠️ Признак путешествует **со значением** по той же причине, что `m`
        /// и `n`: арифметика самоописательна, и без него операция не знала бы,
        /// прижимать результат или переносить. Зеркально полю `sat` в
        /// `TypeNode::Fixed` компилятора.
        sat: bool,
    },
    ...
    /// Длительность (фича 0134) в **наносекундах** — каноническое представление
    /// языка. Отдельный вариант, а не `Number`: иначе `t + 1` прошло бы молча, и
    /// запрет смешения (`SE-065`) обходился бы через симулятор.
    ///
    /// ⚠️ Эталон меряет время наносекундами, а не единицами профиля: профиль —
    /// свойство **генерации** (целям нужен целый счётчик), а не модели.
    Duration(i64),
}
