...
impl Unit {
    ...
    pub(super) fn mark_state_entry(&mut self) {
        ...
        } = &mut self.0
        {
            ...
            *ticks_in_state = 0;
            // Периодические блоки `every` (0134-09) отсчитываются заново от входа:
            // очистка — сигнал `execute_every` переинициализировать аккумулятор
            // под число блоков нового состояния.
