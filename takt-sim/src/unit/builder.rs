...
pub(crate) fn default_field(ty: &TypeNode, model: &ModelNode) -> Value {
    match ty {
        ...
        // Длительность несёт СВОЙ вид значения (фича 0354): целочисленный ноль
        // делал `d := d + 1s;` ошибкой `SIM-005` «операция '+' не определена
        // для операндов целое и длительность» — при том, что все восемь целей
        // тот же вход переводят. Единица носителя — наносекунды (ADR 0134).
...
fn build_node(
    ...
    // Стартовое состояние — в общий реестр прогона (фича 0245): условие
    // `S(Модель) = Состояние` спрашивает о модели, которая может не сделать ни
    // одного перехода, и без этой записи её состояние было бы неизвестно до
    // первого перехода — то есть проверка «модель ещё в старте» отвечала бы
    // отказом «модель не запущена».
