...
pub(crate) enum UnitKind {
    ...
    Node {
        ...
        /// Исполнен ли `exit` при ЗАВЕРШЕНИИ автомата (фича 0430).
        ///
        /// Уход в терминал — тоже выход из состояния: цели `c`, `rust`, `st` и
        /// `sv` исполняют там `exit`, а эталон не исполнял его вовсе — один
        /// вход давал `hits = 1` против `11` у всех четырёх. Флаг держит «ровно
        /// один раз»: терминальный узел тикается и дальше, а выходят из
        /// состояния однажды.
...
impl Unit {
    ...
    pub fn execution(&mut self, name: &str) -> Result<(), Diagnostic> {
        ...
        //
        // ⚠️ ПОСЛЕ выхода из состояния они не исполняются (фича 0430): узел
        // завершён, и его блоки молчат — ровно как ветвь `END` в порождённом C,
        // где `switch` уже ничего не выбирает. Прежде эталон продолжал крутить
        // `always` состояния после завершения: замер дал `14, 15, 16` против
        // `14, 14, 14` у прошивки. Блоки уровня МОДЕЛИ (шаг 1) при этом
        // исполняются по-прежнему — они и в C печатаются до `switch` (0083).
