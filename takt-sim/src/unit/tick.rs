...
impl Unit {
    ...
    fn tick_node(&mut self, soft: bool) -> TickResult {
        ...
        // ⚠️ Состояние С ТЕЛОМ переходов не требует и автомат не завершает
        // (фича 0534): `always` без рёбер работает вечно. Прежде уход в
        // терминал случался по одному лишь отсутствию рёбер, и тело
        // исполнялось РОВНО ОДИН раз — автор писал «всегда», а получал
        // «однажды».
