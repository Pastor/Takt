    ...
    let (params, ret, body, loc, name) = {
        ...
        match &*borrowed {
            ...
            FunctionDefinitionNode::Builtin(name, _, _) => {
                ...
                // ⚠️ Формулировка «пока не поддерживается» здесь ОБЯЗАТЕЛЬНА:
                // по ней заглушку видит гейт 0217, и запись реестра
                // `scripts/stub-branches.txt` называет держателя — фичу 0247.
                // Убрав маркер, мы спрятали бы заглушку от машины.
                return Err(Diagnostic::error(
                    Location::Builtin,
                    format!(
                        "встроенная функция '{name}' в позиции выражения пока не \
                         поддерживается симулятором: 'S(Модель)' вычислима только \
                         в условии 'S(Модель) = Состояние'"
                    ),
                )
                .with_code("SIM-020"));
            }
