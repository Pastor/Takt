...
///
/// ⚠️ **Под `Ok(None)` остаются ровно две законные формы** (замер фичи 0249):
/// запись одного бита (`b.2 := 1`) и запись среза (`arr[0:2] := 1`). Всё
/// прочее — не место записи вовсе — отвергает семантика (`SE-111`, `SE-112`)
/// ещё до прогона.
fn resolve_place<'a>(
    ...
    match expr {
        ExpressionNode::Assign(lhs, rhs) => {
            ...
            let Some(var_rc) = resolve_place(lhs, &mut path, ctx)? else {
                // ⚠️ Прежний текст перечислял три законных места («не в
                // переменную, поле или элемент массива») и умалчивал о бите,
                // срезе, порте и ячейке — то есть называл автору не ту причину.
                // После фичи 0249 форма левой части судится семантикой, и за
                // этим отказом остаются ровно две записи; текст называет обе.
                return Err(Diagnostic::error(
                    loc_of_assign(lhs),
                    "симулятор пока не поддерживает эту запись: он исполняет \
                     присваивание переменной, полю структуры, элементу массива, порту \
                     и ячейке '#АДРЕС', но не запись отдельного бита ('x.N := …') и не \
                     запись среза ('x[a:b] := …')"
                        .to_string(),
                )
                .with_code("SIM-017"));
                ...
                return Err(Diagnostic::error(
                    ...
                    format!(
                        "встроенная функция '{name}' значения в этой позиции не \
                         имеет: 'S(Модель)' вычислима только в условии \
                         'S(Модель) = Состояние'"
                    ),
                )
                .with_code("SIM-020"));
