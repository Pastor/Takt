...
pub struct SimStep {
    ...
    /// Стенд внешних функций на этот такт (фича 0209).
    ///
    ...
    ///
    /// ⚠️ Поле необязательно, и умолчания у него нет: не задали — вызов
    /// `extern fn` со значением отвечает прежним `SIM-019`. Ноль по умолчанию
    /// сделал бы прогон зелёным там, где эталон и прошивка расходятся.
    #[serde(rename = "extern")]
    pub extern_stubs: Option<BTreeMap<String, ExternValue>>,
    pub guard: Option<Guard>,
}

/// Чем сценарий подменяет одну внешнюю функцию (фича 0209).
#[derive(Deserialize, Clone)]
#[serde(untagged)]
pub enum ExternValue {
    /// Таблица «первый аргумент → значение». Ключ — строка: в JSON ключом
    /// объекта иначе не бывает.
