//! Текст `SIM-017` называет то, за что он остался отвечать (фичи 0249, 0250).
//!
//! ## Зачем сторож
//!
//! Исходный текст — «присваивание не в переменную, поле или элемент массива
...
//!
//! Дальше объём отказа сужался дважды: фича 0249 отдала форму левой части
//! семантике (`SE-111`, `SE-112`), фича 0250 научила эталон записи разряда. За
//! `SIM-017` осталась **одна** запись — срез, и текст обязан называть её и
//! способ обойти.
//!
//! ⚠️ Проверять надо именно **текст**: любая неисполнимая запись останавливает
//! прогон одинаково, поэтому «отказ случился» ничего не говорит о том, верную
//! ли причину прочёл автор.
//!
//! ⚠️ Слов «пока не поддерживается» в тексте быть не должно. Они означают
//! обещание будущей работы (и по ним же гейт 0217 ищет заглушки), а обещать
//! нечего: срез не переводит **ни один** потребитель, и эталон отказывает
//! заодно с целями, а не вместо них.
...
/// **Запись разряда отказом БОЛЬШЕ НЕ ЯВЛЯЕТСЯ** (фича 0250).
///
/// Контрпример к предыдущему тесту: пока текст перечислял две формы, «починка»
/// могла оставить разряд за отказом и остаться незамеченной.
#[test]
...
///
/// Исходная формулировка перечисляла три места и тем утверждала, что порт и
/// ячейка `#АДРЕС` не поддержаны. Сторож ловит возврат к такому обещанию:
/// прогон, где записаны переменная, поле, элемент, разряд и порт, обязан
/// пройти.
#[test]
