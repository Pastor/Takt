...
//! [0030-02](../../../../../docs/features/0030-comprehensive-example-fix.md#разработка), образец —
...
const CONTRACTS: &[Contract] = &[
    ...
    // воздействию пересчитывается измерение, — сходится сам и завершается:
    ...
    // инстанцировании. Цепочка проверяет обе — «Heating» встречается дважды,
    // между ними полный цикл первой ступени, а завершает прогон состояние
    // `Finished` после последовательной композиции.
    //
    // ⚠️ Регулятор перестал быть моделью (фикс 0182-01): состояний `Control` и
    // `Settled` в цепочке больше нет — контур стал ТИПОМ ДАННЫХ, и его
    // состояние живёт в полях переменной, а не в вершинах автомата.
    Contract {
