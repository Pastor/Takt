//! Пять форм `match` — потактовая сверка значений (фича 0478).
//!
//! # Что доказывает набор
//!
//! Перебор целей (0450) отвечает на вопрос «принял ли вывод инструмент». Для
//! `match` этого мало: выбранная не та ветвь, потерянный образец или
//! пропущенный `default` дают **валидный** код с другим поведением — и у
//! каждой цели своя идиома (`switch` у `c`, цепочка сравнений у `rust`, `case`
//! у `sv`, цепочка `IF` у `st`), так что сравнивать тексты нечем.
//!
//! ⚠️ Приращения форм — СТЕПЕНИ ДВОЙКИ (1, 2, 4, 8, 16): сумма 31 достижима
//! единственным набором выборов, и подмена любой ветви сдвигает её однозначно.
//! При одинаковых приращениях потеря ветви сошлась бы в сумме.
...
static void wr_num(MatchForms_Out_NumericPort port, int64_t v, void *ud) {{
    (void)port;
    (void)ud;
    last_beat = (long)v;
}}
