...
//!
//! ⚠️ **Момент выхода из состояния-цепочки** — главное, что здесь ловится: обе
//! формы обязаны уводить состояние в том же скане, в котором завершился
//! последний шаг (фича 0443 — до неё цель тратила лишний скан), и обе обязаны
//! совпасть с эталоном. `iec2c` принимал и сдвинутый вывод: он проверяет
...
fn st_table_form_matches_case_form_on_chain() {
    ...
    // ⚠️ И с ЭТАЛОНОМ тоже: до фичи 0443 цель тратила лишний скан на выходе из
    // цепочки (машина шагов уходила наружу из отдельной ветви `CASE`, то есть
    // скан спустя), и трасса ST отличалась сдвигом. Теперь последний шаг уводит
    // состояние в том же скане — как эталон и цель `c`.
    assert_eq!(case, expected, "форма CASE разошлась с эталоном");
