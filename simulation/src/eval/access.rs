...
mod tests {
    ...
    fn field_read_by_name() {
        assert_eq!(
            read_member(
                &point(),
                &Member::Identifier(grammar::parser::ast::Identifier::new("y"))
            ),
            Ok(Value::Number(2))
        );
    }
    ...
    fn unknown_field_is_error() {
        let err = read_member(
            &point(),
            &Member::Identifier(grammar::parser::ast::Identifier::new("z")),
        )
        .unwrap_err();
    ...
    fn field_on_non_struct_is_error() {
        let err = read_member(
            &Value::Number(5),
            &Member::Identifier(grammar::parser::ast::Identifier::new("x")),
        )
        .unwrap_err();
