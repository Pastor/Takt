...
//! `.lam` через `build_unit`, то есть тем же путём, что и в CLI, и сверяются
...
use simulation::state_io::{load_from_file, restore, save_to_file, snapshot};
use simulation::{TickResult, Unit, Value, build_unit};

fn unit_from_src(src: &str) -> Unit {
    let (ast, _) = grammar::parse(src, 0).unwrap_or_else(|e| panic!("разбор: {e:?}"));
