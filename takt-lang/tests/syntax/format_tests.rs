...
//! переименованных в `takt-lang/` и `takt-sim/` ещё фичей 0100, — а помощник
...
fn r2_blank_line_between_comment_groups_survives() {
    ...
    // Перечисление печатается по варианту на строке (фикс 0197-01) — здесь
    // проверяется пустая строка МЕЖДУ группами комментариев, а не его раскладка.
