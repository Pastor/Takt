...
//!
//! ⚠️ `SE-093` — **временная** диагностика: значение выставляют пока не все
//! цели (`c`/`c-hal`/`rust` умеют с задачи 0187-03, `sv`/`sv-mmio`/`st`/`st-at`
//! — нет; снимется задачей 0187-04). Молчать здесь нельзя — программа
//! собралась бы, а значение исчезло.
...
fn output_initial_value_is_reported_as_not_yet_emitted() {
    // Временная диагностика: значение выставляют не все цели. Снимется
    // задачей 0187-04.
