...
//!
//! ⚠️ `SE-093` — **временная** диагностика: значение хранится, но цели пока не
//! выставляют его до первого такта (задачи 0187-03…05). Молчать здесь нельзя —
//! программа собралась бы, а значение исчезло.
...
#[test]
fn output_initial_value_is_reported_as_not_yet_emitted() {
    // Временная диагностика: значение сохранено, но цели его пока не
    // выставляют. Снимется задачами 0187-03…05.
    ...
    assert!(
        ...
        "молчаливая потеря значения недопустима: {codes:?}"
    );
}
