...
//! # Что было
//!
//! Вычислитель констант уже умел называть, почему тело функции исполнить
//! нельзя («чтение переменной», «порт», «неподдержанный оператор»,
//! «исчерпанный бюджет»), но свёртка инициализаторов выбрасывала его
//! диагностику вместе с ошибкой — и один вход давал **четыре** ответа:
//!
//! | Потребитель | Ответ |
//! |---|---|
...
//! | `rust` / `sv`, `sv-mmio` | `RS-011` / `SV-002`; `plantuml` принимал |
//!
//! # Границы правила
//!
//! Отказ **узкий**: спрашивается наличие вызова. Вычислимый вызов
//! сворачивается в литерал и остаётся законным, прочие невычислимые формы не
//! затронуты.
...
///
/// ⚠️ Проверяется именно причина, а не факт отказа: до фичи слова вычислителя
...
///
/// ⚠️ Проверка порядка: `SE-114` (фича 0300) судит невычислимую **арифметику**,
/// а голый вызов арифметикой не является — разойдясь, две проверки дали бы на
/// один вход два разных объяснения.
#[test]
...
///
/// Без него «невычислимый отвергается» означало бы «отвергается любой вызов»:
/// замер показал, что `var x := seed();` согласован у всех девяти потребителей
/// (значение `3`), и ломать его нельзя.
#[test]
...
///
/// ⚠️ Пока проверка стояла и в свёртке, и в обходе `validate`, у одного кода
...
/// **мертва**, и это находка фичи 0306, а не догадка.
#[test]
fn initializer_refusal_lives_in_a_single_place() {
    ...
    // Ищется ЭМИССИЯ, а не упоминание: в шапке модуля код назван словами —
    // там сказано, где правило живёт, и запрещать это значит запрещать
    // объяснение.
    assert!(
