...
/// **T6: специализация импортированной модели с аргументами — `SE-120`.**
///
/// Копия снимается с уже разрешённых тел, и `Rc` переменных разделился бы с
/// исходной моделью. Отказ громкий; контроль — тот же вход **в одном файле**,
/// где специализация обязана работать.
#[test]
fn specialize_with_arguments_across_import_is_se120() {
    let dir = temp_dir("spec-refuse");
    write(&dir, "tuner_lib.takt", TUNER);
    let root = write(
        &dir,
        "app.takt",
    ...
    let codes = codes_of_c(&dir, &root, &options);
    assert!(
        codes.contains(&"SE-120".to_string()),
        "специализация модели из другого файла обязана отвергаться с причиной, получено {codes:?}"
    );

    // Контроль: без границы импорта тот же вход специализируется.
    let single = write(
        &dir,
        "single.takt",
        &format!("{TUNER}\nstart Main = Tuner(limit := 30) | Tuner(limit := 60);\n"),
