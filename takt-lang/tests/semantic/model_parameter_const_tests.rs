...
/// Обычная константа подчиняется **тому же** правилу, что и константа-параметр:
/// имя квалифицировано владельцем.
///
/// ⚠️ Прежняя редакция теста утверждала обратное («имени НЕ меняет») — это было
/// требование R7 фичи 0185 («вывод корпуса байт-в-байт»), из-за которого
/// квалификацию тогда получили **только** константы-параметры. Исключение снято
/// фичей [0193] по решению заказчика: два правила на один вид объявления и были
/// дефектом — две модели с одноимённой константой давали одно объявление, и
/// вторая молча считала чужим значением. Предмет теста прежний, ожидание
/// обновлено.
///
