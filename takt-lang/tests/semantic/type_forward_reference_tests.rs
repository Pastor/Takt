//! Имя типа видно во всём файле; цикл структур — `SE-124` (фича 0352).
//!
//! # Что было сломано
//!
//! Объявления типов строились одним проходом **в порядке текста**, поэтому имя,
//! объявленное ниже, не разрешалось. Замер 2026-08-21 (`scripts/probe.sh`):
//! `struct Aa { b: Bb }` выше `struct Bb` давало у эталона поле-структуру,
//! ставшую **числом** (`v=Aa{b=0,n=0}` — ошибку глотал
//! `.unwrap_or(TypeNode::Unsupported)`), а у семи целей — отказ (`CC-015`,
//! `ST-002`, `RS-014`, `SV-002`). Контрольный вход — тот же файл с обратным
//! порядком — принимали все девять потребителей, то есть ломал **порядок**.
//!
//! # Почему цикл диагностируется здесь же
//!
//! Пока порядок был значим, цикл `A → B → A` был **недостижим**: одна из
//! структур всегда ссылалась вперёд и получала неподдержанный тип. Сняв
//! зависимость от порядка, фича сделала цикл разрешимым — и без `SE-124`
//! породила бы вывод, отвергаемый инструментами целевых языков при нулевом
//! коде возврата `taktc`.
...
///
/// Отдельный тест: разрешение спускается в элемент массива, и без спуска
/// форма просочилась бы мимо правки.
#[test]
...
/// проверка сторожит разрешение **до неподвижной точки**.
#[test]
...
///
/// ⚠️ Тест на **две** ссылки вперёд подряд, и он не декоративен: мутация
/// «один виток вместо неподвижной точки» на паре `A → B` **не ловится** —
/// финальная попытка разрешить остаток служит вторым проходом. Цепочка из трёх
/// требует настоящего повторения.
#[test]
...
///
/// Без этой проверки «починка», регистрирующая любое имя, прошла бы тесты
/// выше и молча приняла бы опечатку.
#[test]
...
///
/// Занятие имени переехало в предпроход, и второй вызов `claim_type_name` в
/// основном цикле дал бы **ложную** `SE-108` на каждой структуре; этот тест
/// вместе с положительными выше отделяет верное поведение от такой поломки.
#[test]
...
///
/// Без него `SE-124` могла бы срабатывать на любой вложенности — и правка
/// читалась бы как «вложенные структуры запрещены».
#[test]
...
///
/// Эта проверка стоит **раньше** и гарантирует завершение разрешения
/// псевдонимов до неподвижной точки; подмена кода означала бы, что порядок
/// проверок нарушен.
#[test]
