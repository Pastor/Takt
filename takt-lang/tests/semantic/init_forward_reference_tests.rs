...
//! соседних: ссылка назад и ссылка вперёд на константу — законны, а чтение
//! порта отвергается **чужим** правилом (`SE-113`, фича 0266), и граница эта
...
/// инициализаторе **этой** фичей не задето.
///
/// ⚠️ Форма перестала быть законной позже — её отвергает `SE-113` (фича 0266,
/// молчаливое расхождение эталона с целями). Граница от этого не исчезла, а
/// стала проверяемой точнее: 0246 отвечает за ссылку вперёд и обязана молчать о
/// порте, чем бы тот ни судился. Мутация «считать порт ссылкой вперёд» валит
/// именно этот тест.
#[test]
