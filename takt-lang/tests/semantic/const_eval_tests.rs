...
/// **Точная** арифметика над дробными вычисляется (фича 0300): сложение,
/// вычитание и умножение десятичных литералов округления не требуют.
///
/// ⚠️ Прежде отвергалась **любая** дробная арифметика, и довод был верен лишь
/// наполовину: «представление выбирают флаги, округление q задано эталоном» —
/// это про округление. Цена умолчания измерена (ADR 0300): один вход давал
/// ноль у эталона и настоящее число у цели, молча.
#[test]
...
/// А **деление** отвергается с причиной: в десятичной записи оно не
/// представимо, а выбор округления — та самая часть, что задана эталоном.
#[test]
