...
/// **A6.** Вторая ветвь `FormatError` печатается тем же форматом.
///
/// ⚠️ Ожидание **изменено фичей 0229**: прежде `Unsupported` печаталась своей
/// формой («Ошибка форматирования 'файл': печать узла 'Formula' …») и **без
/// позиции**. Теперь это полноценная диагностика `FM-001` — с путём, строкой,
/// колонкой и кодом, как у ветви `Parse`. Текст называет вид узла строчными
/// (`formula`), потому что берётся у самого узла, а не собирается `Debug`-дампом.
#[test]
fn unsupported_node_is_a_diagnostic_with_position() {
    ...
    assert!(
        fmt.contains(
            "probe.takt:2:5: Ошибка компиляции [FM-001]: печать узла 'formula' \
             пока не поддерживается форматтером"
        ),
        "отказ печати узла обязан идти общим форматом с позицией: {fmt:?}"
    );
    assert_eq!(code, 1);
}
