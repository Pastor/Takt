...
fn different_values_produce_different_models() {
    ...
    // Значение места инстанцирования — В ИНИЦИАЛИЗАТОРЕ копии, присваиваний
    // после _init нет: в этом режиме модель и есть настройка.
    assert!(
        body.contains("model->gain = 2;") && body.contains("model->gain = 3;"),
        "значения обязаны быть инициализаторами копий:\n{body}"
    );
...
fn sv_accepts_different_sets_via_specialization() {
    ...
    assert!(
        code.contains("tuner_p1_gain <= 2;") && code.contains("tuner_p2_gain <= 3;"),
        "каждая специализация обязана получить свой регистр со своим сбросом:\n{code}"
    );
}
