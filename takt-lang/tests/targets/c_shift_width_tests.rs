...
/// Величина сдвига **переменная** — печатается как прежде.
///
/// ⚠️ Названная граница (задача 0392-02): у цели `rust` она уже насыщается
/// (0334), у `c` — нет, и `cc -Werror` на неё молчит. Цена правки — ветвление
/// на каждый такой сдвиг, и мерить её надо на реальном выводе.
#[test]
fn variable_shift_amount_is_left_alone() {
    ...
    assert!(
        text.contains("model->a >> model->n"),
        "переменная величина остаётся прежней — граница названа:\n{text}"
    );
}
