...
/// **T9.** Разряд за объявленной шириной не печатается доступом за границу.
///
/// В C это UB, в Rust — паника. Обе цели отказывают с причиной; эталон на том
/// же входе отвечает `SIM-011` в такте.
#[test]
fn bit_beyond_vector_is_refused_by_both_targets() {
    let c = generate_c("beyond_c", BEYOND).expect_err("цель `c` обязана отказать");
    assert_eq!(c.code.as_deref(), Some("CC-022"), "код отказа цели `c`");
    assert!(
        c.message.contains("за пределом бит-вектора"),
        "отказ обязан называть причину:\n{}",
        c.message
    );
    let r = generate_rust("beyond_rs", BEYOND).expect_err("цель `rust` обязана отказать");
    assert_eq!(r.code.as_deref(), Some("RS-011"), "код отказа цели `rust`");
    assert!(
        r.message.contains("за пределом бит-вектора"),
        "отказ обязан называть причину:\n{}",
        r.message
    );
}
