...
fn c_hal_emits_address_table_and_hal() {
    ...
    // Фича 0176: `address LED = 0x00200004;` — однобитный порт с адресом без
    // позиции бита. Умолчание «бит 0» объявляется предупреждением `SE-090`, а не
    // принимается молча каждым потребителем по-своему. Внешней карты здесь нет,
    // поэтому других предупреждений быть не должно.
    assert_eq!(
        warnings
            .iter()
