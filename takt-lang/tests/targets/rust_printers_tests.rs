...
/// Поле структуры целью `rust` **транслируется** (фичи 0293, 0341).
///
/// ⚠️ Прежде этот тест утверждал обратное — «поля структур не транслируются,
/// отказ `RS-011`», — и **закреплял дефект**: структуры цель переводит с фичи
/// 0293, а отказ приходил от `expression_type`, считавшего **любой** доступ
/// через точку логическим (`x.7` и `s.field` неразличимо). Класс тот же, что у
/// фичи 0191: поведение зафиксировано тестом раньше, чем измерено.
#[test]
