    ...
    for result in results {
        ...
        match &result.verdict {
            ...
            Verdict::Unsupported(atoms) => {
                ...
                // ⚠️ Первая строка причину НЕ называет, и это честность, а не
                // лень: `Verdict::Unsupported` несёт только имена атомов, а
                // причин у отказа несколько (фича 0145 добавила к ним потолок
                // размера). Прежняя формулировка «атом — не имя состояния и не
                // отслеживаемый предикат» на входе за потолком была прямо
                // ложной: атомы там как раз отслеживаемые. Различать причины
                // вердиктом — кандидат в `FEATURES.md`.
                println!(
                    "  атом(ы) {} — проверка не выполнена по одной из причин ниже.",
                    atoms.join(", ")
                );
                println!(
                ...
                println!(
                    "  Вне охвата: `float`/`q` в предикате, арифметика/функции/порты в нём, \
                     состояния вложенных моделей."
                );
                // Фича 0145: потолок считается по РЁБРАМ (рёбра управляющего
                // графа × квадрат произведения доменов), а не по вершинам, —
                // иначе граница названа бы величиной, которая стоимость не
                // предсказывает, и совет «сузьте домен» звучал бы наугад.
                println!(
                    "  Также вне охвата — превышение потолка размера: рёбер получается \
