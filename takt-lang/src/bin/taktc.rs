...
/// здесь уже расходилась по целям (задача 0028-01).
fn print_compile_error(diag: &takt_lang::diagnostics::Diagnostic) {
    eprintln!("{}", takt_lang::diagnostics::format_compile_error(diag));
}
...
fn main() {
    ...
    let errors =
        takt_lang::collect_compile_diagnostics(&options.input_file, &source, &options.include_dirs);
    if !errors.is_empty() {
        for diag in &errors {
            print_compile_error(diag);
        }
