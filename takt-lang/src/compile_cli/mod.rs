...
///
/// Пересчёт зависит от профиля времени, а профиль сегодня потребляют только цели
/// на базе C (`c`/`c-hal`). Прочие цели время ещё не порождают (`RS-023`/`ST-015`/
/// `SV-015`), поэтому печатать «во что превратилась длительность» для них
/// преждевременно — их задачи-преемники добавят себя сюда вместе с реализацией.
