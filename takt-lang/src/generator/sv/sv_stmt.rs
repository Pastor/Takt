...
//!
//! Тело состояния Lam — **императивная последовательность**: `v := 1; w := v;`
...
pub(crate) fn print_statement(
    ...
    match stmt {
        ...
        // Пустой `return` возвращать нечего: у функции Lam всегда есть значение.
        ...
        // обязаны предшествовать операторам, а Lam разрешает объявить переменную
        // посреди блока (`stacker.lam:117-120`), поэтому порядок исходника здесь
        ...
        // синтеза. Условие Lam их не гарантирует, а неразворачиваемый цикл
        ...
        // игнорирует; здесь то же, и молчания в этом нет: `lamc verify`
...
/// операторам блока, а Lam разрешает объявить переменную посреди тела
/// (`stacker.lam:117-120`: `var ds := 0;` идёт после других операторов). Проба
/// 2026-07-16 подтвердила: `logic [7:0] dr;` после присваивания даёт
/// `%Error: syntax error, unexpected IDENTIFIER, expecting "'{"`.
///
/// Вынос смысла не меняет: объявление в Lam видимо до конца блока, а
...
mod tests {
    ...
    ///
    /// В синтезируемом RTL цикл обязан разворачиваться в схему; условие Lam
    ...
    ///
    /// Формула есть свойство для верификации (`lamc verify`, фича 0049), а не
