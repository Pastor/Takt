...
pub(in crate::generator::sv) fn reset_value(
    ...
    Ok(match expr {
        ...
        //
        // ⚠️ РАСПАКОВАННОМУ МАССИВУ `'0` НЕ ПРИСВАИВАЕТСЯ (фича 0365):
        // verilator отвечает ошибкой «CONST is not an unpacked array». Форма
        // сброса у него — агрегат нулей, тот же, каким печатается
        // инициализатор (0309); носитель один.
        ExpressionNode::None => {
            // ⚠️ ПЕРЕЧИСЛЕНИЕ сбрасывается своим значением, а не `'0` (фича
            // 0379): перечисления SV строго типизированы, и `m <= '0;` над
            // регистром типа `mode_e` verilator встречает **ошибкой**
            // `%Error-ENUMVALUE` («Implicit conversion to enum … from 'bit'»),
            // а гейт цели считает предупреждение ошибкой. Значение остаётся
