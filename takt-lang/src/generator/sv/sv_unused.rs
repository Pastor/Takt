...
//!
//! ⚠️ **Параметр бывает использован ЧАСТИЧНО** (фича 0466): индекс печатается
//! сужением по размеру массива (`a[2'(i)]`, фича 0365), и старшие разряды не
//! читает никто — verilator отвечает тем же `UNUSEDSIGNAL`, но уже про биты.
//! Тот случай судит признак ПО ДЕРЕВУ (`sv_stmt::index_only_variables`):
//! объявление поглотителя обязано встать до операторов, то есть до того, как
//! тело напечатано.
//!
