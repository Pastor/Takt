//! Отображение типов Lam в типы Rust (задача 0050-03).
//!
//! ## Зачем это ядро фичи
//!
...
//!
//! Проба 2026-07-16: `#[repr(u8)] enum A { Idle = 670 }` (`elevator.lam:121`)
...
pub(crate) fn reject_float_width(width: crate::generator::FloatWidth) -> Result<(), Diagnostic> {
    if width == crate::generator::FloatWidth::W32 {
        return Err(Diagnostic::error(
            Location::Codegen,
            "--float-width=32 несовместим с целью 'rust': вещественный тип Lam \
...
mod tests {
    ...
    ///
    /// Реальный случай корпуса — `elevator.lam:121`. Проба показала, что
