...
pub(crate) struct Scope<'a> {
    ...
    ///
    /// Формула-ОПЕРАТОР печатается тем же носителем, что формула-элемент, и
    /// флаг обязан дойти до печатника тел: прежде он спрашивался только у
    /// карты, а тела о нём не знали.
...
fn call(
    ...
    // `taktc`.
    //
    // ⚠️ ВНЕШНЯЯ функция сюда тоже входит (фича 0418). Прежде здесь стояло
    ...
    // `Local` (класс 0292). Цена умолчания: `sink(2 ** 3)` отвергалось
