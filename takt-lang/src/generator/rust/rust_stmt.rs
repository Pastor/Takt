//! Трансляция операторов Lam в Rust (задача 0050-07).
//!
//! ## Чем Rust проще ST
//!
...
//! `IF/ELSIF`, потому что образцы Lam — произвольные выражения. В Rust всё это
//! ложится один в один: `let` живёт в теле, вызов — законный оператор, `match`
//! есть в языке.
//!
//! Единственная тонкость — `match`: образцы Lam произвольны, а образцы Rust —
...
///
/// Нужно, чтобы решить, печатать ли `let` или `let mut`. В Lam изменяемость не
...
fn collect_assigned_expr(expr: &ExpressionNode, out: &mut BTreeSet<String>) {
    ...
    // Присваивание — оператор, а не подвыражение: в Lam его негде спрятать
...
/// `stacker.lam` оставался с отложенными объявлениями (`needless_late_init`).
///
...
pub(crate) fn print_block(
    ...
    // требует именно переезда: `travel_time` в `stacker.lam` объявляет
...
pub(crate) fn print_statement_ctx(
    ...
    match stmt {
        ...
        StatementNode::Variable(name, ty, init) => {
            let ident = rust_value_name(name, crate::diagnostics::Location::Codegen)?;
            let ty_name = rust_type(ty, &format!("переменная '{}'", name))?;
            // `mut` ставится по факту присваивания, а не по объявлению: в Lam
            ...
            // печати инициализатора: `var x := x;` в Lam читает ВНЕШНИЙ `x`.
            scope.locals.push(name.clone());
            Ok(0)
        }
        StatementNode::If { cond, then_, else_ } => {
        ...
        // `for` Lam — это C-подобный `for(init; cond; step)`, а не итератор.
        ...
        // Образцы Lam — произвольные выражения, а `match x { y => … }` в Rust
        ...
        // Формула в теле блока — предмет верификации (`lamc verify`), а не
            ...
            out.warnings.push(
                Diagnostic::warning(
                    ...
                    "LTL-формула в теле блока не транслируется в Rust: \
                     проверяйте её через 'lamc verify'. Порождённый код формулу \
                     не содержит"
                        .to_string(),
                )
                .with_code("RS-010"),
