...
//!
//! ⚠️ **Граница названа отказом `CC-025`**: состояние-композиция (`A | B`,
//! `A + B`) ведётся машиной шагов, которая меняет `model->state` **изнутри
//! тела**, — строкой «откуда → страж → куда» это не выражается. Реализация
//! одной моделью (`= M`, в том числе корневая обёртка `start Main = M;`)
//! таблицей выражается: её завершение есть предикат `M_is_done(&model->x)`.
    ...
    for state_name in states.iter() {
        ...
        let (done, next) = match &element {
            ...
            Element::StateExtend { extend, next, .. } => {
                let loc = raw_rc.borrow().loc();
                match done_predicate(extend, state_name, map, is_main, loc)? {
...
/// готовностей своих ветвей, и берётся она **у того же носителя**, что печатает
/// их тик (`c_compose::generate_parallel_items_tick`): своё знание о раскладке
/// полей разошлось бы с ним при первой же правке (класс 0084/0193/0195).
/// Цепочка `A + B` остаётся отказом `CC-025`: её завершение ведёт машина шагов,
/// которая меняет состояние **изнутри** тела такта.
fn done_predicate(
    extend: &StateExtend,
    state_name: &Name,
    map: &CMap,
    ...
    match extend {
        ...
        StateExtend::Concatenation(_) => Err(Diagnostic::error_with_note(
            loc,
            format!(
                "состояние-цепочка '{}' не переводится в табличную форму автомата (--fsm=table)",
                state_name.local()
            ),
            loc,
            "причина: последовательная композиция ведётся машиной шагов, меняющей состояние \
             изнутри тела такта, — строкой «откуда → страж → куда» это не выражается. \
             Скомпилируйте модель формой по умолчанию (--fsm=switch)"
                .to_string(),
        )
        .with_code("CC-025")),
    }
