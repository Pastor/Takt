...
///
/// Единственный класс операторов, который цель `c` вправе пропустить молча:
/// `debug(…)` и `S(…)` — средства отладки и запроса состояния, кода не
/// порождающие. Всё прочее, что печатник не умеет, обязано дойти до автора
/// диагностикой (иначе оператор исчезает при рапорте об успехе).
