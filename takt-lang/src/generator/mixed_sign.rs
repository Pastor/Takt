...
/// (фикс 0359-01).
///
/// Правило разное по операторам и по стороне знакового операнда, и держать его
/// копиями в четырёх целях значило бы разойтись — что и случилось: первая
/// редакция 0359 не покрыла `=`/`!=` вовсе, и `sv` считал `-1 == 255` истиной
/// (проверено прогоном verilator), `rust` и `st` не собирались.
///
