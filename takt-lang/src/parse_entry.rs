...
fn parser_error_to_diagnostic(
    ...
    match error {
        ...
        .with_code("SY-006"),
        // Адресный литерал вне позиции размещения — своя диагностика (фича 0189).
        //
        // Грамматика принимает `0xАДРЕС:бит` только там, где адрес осмыслен:
        // после `at` и в операторе `address`. В выражении тот же токен даёт
        // «нераспознанный токен» со списком из двух десятков ожидаемых — здесь
        // это сообщение заменяется правилом языка. Прежде литерал в выражении
        // принимался и давал **молчаливое расхождение**: симулятор считал
        // адрес числом, а цель `c` теряла оператор целиком.
        ParseError::UnrecognizedToken {
