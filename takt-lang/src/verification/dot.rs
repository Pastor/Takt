...
fn refusal_message(verdict: &Verdict) -> String {
    match verdict {
        ...
        // ⚠️ Причина берётся у вердикта (фича 0258), а не пересказывается здесь:
        // прежний текст утверждал «не имя состояния и не отслеживаемый
        // предикат» и на входе за потолком был прямо ложным — атомы там как раз
        // отслеживаемые.
