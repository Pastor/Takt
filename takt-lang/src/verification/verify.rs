...
pub enum Verdict {
    ...
    ///
    /// Атом-предикат над данными (`cond`/булев `var` над `bit`/`bool`/целым/
    /// `enum`) **отслеживается** (фича 0068). `Unsupported` остаётся, когда атом:
    /// опечатка; состояние вложенной модели; предикат вне подмножества
    /// (арифметика/функция/порт); `float`/`q`; превышение потолка перечисления.
    ///
    /// Не молчаливое «ложно»: делать вид, что проверено, нельзя.
    ///
    /// ⚠️ Причина названа **полем**, а не выбором читателя из списка (фича
    /// 0258): прежде вердикт нёс только имена атомов, и CLI печатал **все**
    /// возможные причины разом, предлагая пользователю угадать свою.
    Unsupported {
...
/// Причина, по которой свойство не проверено (фича 0258).
///
/// Прежде причин было пять, а сообщение — одно на все: пользователь выбирал
/// подходящую сам, и на входе за потолком первая строка **утверждала ложное**
/// («атом — не отслеживаемый предикат», хотя атомы там как раз отслеживаемые;
/// фича 0145 сняла ложь, заменив утверждение нейтральным, но причину не
/// назвала). Образец решения — `FM-001` фичи 0229: один вид, названный словом.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UnsupportedReason {
    /// Атом — не имя состояния текущей модели и не предикат над данными:
    /// опечатка в имени либо состояние **вложенной** модели.
    UnknownAtom,
    /// Предикат над данными вне поддержанного подмножества: арифметика, вызов
    /// функции, порт, доступ к биту (фича 0068, правило 3).
    PredicateOutsideSubset,
    /// Домен отслеживаемой переменной не перечислим: `float`/`q`, массив,
    /// структура (фича 0068, правило 5).
    ///
    /// ⚠️ **Сегодня недостижима, и это замер, а не догадка** (фича 0258,
    ...
    /// ⚠️ Недостижимость держится на том, что «отслеживаем» и «домен есть» —
    /// ОДИН вопрос с одним носителем (`verification::domain`, фича 0498).
    /// Пока ответчиков было два, они разъехались, и ветвь ожила с причиной
    /// **ложной**: домен перечислимого типа не находился (фича 0497).
    /// Проверено на входах `float`, `q(4, 4)` и структура. ⚠️ Четвёртый вход
    /// того замера — `[bit; 8]` — из списка **изъят фичей 0498**: упакованный
    /// бит-вектор есть скаляр (0078), и сегодня он отслеживается наравне с
    ...
    /// (фича 0145 — метрика считается по рёбрам, а не по вершинам).
    SizeOverLimit,
    /// Начальное значение отслеживаемой переменной неизвестно: инициализатор
    /// не сворачивается либо значение вне домена своего типа.
    ///
    /// ⚠️ **Сегодня ветвь ЗАЩИТНАЯ — как `DomainNotEnumerable`** (замер фичи
    ...
    /// не дают.
    ///
    /// Ветвь оставлена намеренно: расширится подмножество предикатов или
    /// ослабнет запрет — она окажется на переднем крае, а молчаливая подмена
    /// причины была бы ложью (то же решение, что у `DomainNotEnumerable`).
    InitialValueUnknown,
}
...
impl UnsupportedReason {
    ...
    ///
    /// ⚠️ Текст **называет причину и следствие**, а не вид варианта Rust
    /// (класс фичи 0231): читателю нужно понять, что делать.
...
/// абстракция по формуле 0068).
///
/// Атом формулы — **имя состояния** FSM (`S` истинен, когда автомат в состоянии
/// `S`) **или** предикат над данными (`cond`/булев `var`). Для предикатов вершина
/// становится парой `(состояние, оценка отслеживаемых переменных)`
/// ([`super::data_kripke`], консервативное ядро — данные полностью
/// недетерминированы, `Holds` надёжен). Если атомы — только имена состояний,
/// путь прежний (0049) байт-в-байт.
///
/// # Пример
///
/// ```
...
pub(crate) fn select_kripke(model: &ModelNode, phi: &Ltl) -> Result<Kripke, Verdict> {
    ...
    // состояния, в чистой абстракции управления (0049) ложен во всех вершинах.
