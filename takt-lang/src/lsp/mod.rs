...
"#;
    ...
    /// фикстура намеренно объявляет `type u8 = [bit;8]` — псевдоним, затеняющий
    /// встроенное имя (язык это позволяет ради обратной совместимости), — и
    /// потому законно получает предупреждение о каноне именования `CS-001`.
    /// Прежняя проверка «диагностик нет вовсе» приравнивала совет к отказу.
    #[test]
