...
"#;
    ...
    ///
    /// ⚠️ Проверяются именно ошибки, а не всякая диагностика (фича 0226):
    /// фикстура законно получает предупреждения (например, о каноне
    /// именования `CS-001`), и прежняя проверка «диагностик нет вовсе»
    /// приравнивала совет к отказу. ⚠️ Прежде фикстура объявляла
    /// `type u8 = [bit;8]` — затенение встроенного имени; с фичи 0243 это
    /// `SE-107`, и объявление из фикстуры убрано.
    #[test]
