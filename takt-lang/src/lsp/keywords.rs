...
pub(super) const BUT_KEYWORDS: &[(&str, &str)] = &[
    ...
    (
        "after",
        "выдержка перехода: прошло не меньше указанного (after 3s)",
    ),
    ("every", "периодическое действие через заданный интервал"),
    // Операторы LTL и типы формул (фича 0178: до неё в списке отсутствовали,
    // хотя ключевыми словами языка являются с самого начала верификации).
...
const COMPLETION_EXCLUDED: &[(&str, &str)] = &[
    // `_` — подстановочный образец ветки `match`. По роли это знак препинания,
    // а не имя: дополнять нечего, и в списке имён он был бы шумом.
    (
        "_",
...
mod tests {
    ...
    ///
    /// Иначе список исключений копит мусор: слово, выведенное из языка, осталось
    /// бы «исключением» навсегда и маскировало бы своё исчезновение.
    #[test]
