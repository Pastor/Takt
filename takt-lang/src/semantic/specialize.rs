...
fn specialize_one(
    ...
    // Модель, пришедшая из другого файла, к этому месту уже прошла **весь**
    // конвейер: подключаемый файл строится теми же стадиями (фича 0296), но
    // строится он раньше — внутри стадии 0 импортёра. Копировать её нельзя по
    // той же причине, по какой копия не снимается после разрешения тел вообще:
    // `clone` разделит `Rc` переменных, и тела копии будут писать в поля
    // исходной модели. Замер 2026-08-19 (ADR 0296): прежде такой вход давал
    // код возврата **ноль** и C, который `cc` отвергает
    // («no member named 'tuner' in 'struct …P1'»).
    if source.borrow().origin == crate::semantic::ModelOrigin::Imported {
        return Err(Diagnostic::error(
            call_loc,
            format!(
                "Специализация модели '{}', подключённой из другого файла, не \
                 поддерживается: её тела уже разрешены на собственные переменные, и \
                 копия разделила бы их с исходной моделью. Задайте значения \
                 параметров в файле, где модель объявлена, либо соберите в режиме \
                 --parameters=assign",
