...
fn specialize_one(
    ...
    *counter += 1;
    ...
    // Тела копии (фича 0395). У модели, объявленной в **этом** файле, тел ещё
    // нет — их разрешат стадии 2–6 уже на переменные копии. У модели,
    // пришедшей ИМПОРТОМ, они разрешены (весь конвейер прошёл внутри стадии 0
    // импортёра, 0296), и ячейки-снимки в них указывают на исходную модель:
    // без перепривязки цель `c` печатает доступ к чужому полю. Обход
...
fn set_initializer(
    ...
    match model.variables.get_mut(param) {
        // ⚠️ Формы объявления параметра ДВЕ, и вторая — у импортированной
        // модели (фича 0395): подключаемый файл проходит весь конвейер внутри
        // стадии 0 импортёра (0296), а он включает `constify_parameters`, —
        // значит к моменту специализации параметр уже `Const`. Приняв только
        // `Simple`, специализация отвечала бы `SE-086` о «ненайденном»
        // параметре, который объявлен рядом.
        Some(VariableNode::Simple { expr, .. } | VariableNode::Const { expr, .. }) => {
            *expr = value;
            Ok(())
        }
        // Проверка 0185-02 (`SE-077…079`) гарантирует, что имя — параметр;
        // иное здесь означает регресс конвейера.
