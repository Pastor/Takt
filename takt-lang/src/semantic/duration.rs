...
/// Множитель «миллисекунды → такты» для **вычисляемой** выдержки (фича 0183).
///
...
/// сравнение приходится вести в тактах: `dwell >= expr_ms * (hertz / 1000)`.
/// Множитель обязан быть **целым** — иначе любой ответ был бы округлением, то
/// есть выдержкой не той, что заявлена.
///
/// `None` — профиль «часы»: там выдержка меряется миллисекундами напрямую, и
/// множитель не нужен.
///
/// # Ошибки
///
