...
pub(super) fn is_boolean_ast_condition(
    ...
    match cond {
        ...
        // `&` и `|` над БУЛЕВЫМИ операндами — булевы (фича 0232). ⚠️ Условная
        // грамматика логических `&&`/`||` не принимает вовсе: `&`/`|` — та
        // самая и единственная форма конъюнкции и дизъюнкции условий, и
        // объявлять её «неявным приведением числа» значило бы предупреждать о
        // единственной доступной записи. Числовой операнд по-прежнему даёт
        // предупреждение — рекурсия проверяет обе стороны.
        AC::And(_, l, r) | AC::Or(_, l, r) => {
            is_boolean_ast_condition(l, model) && is_boolean_ast_condition(r, model)
        }
        // Индексация массива возвращает элемент числового типа
        AC::ArraySubscript(_, _, _) => false,
        // Доступ к ОДНОМУ биту (`x.3`) — булев сигнал (фича 0232): значение 0
        // или 1, и требовать от него `!= 0` значит спорить с идиомой языка.
        AC::BitAccess(_, _, _) => true,
