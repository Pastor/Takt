...
/// `validate`, реализация **плоская** — `A + B` остаётся
/// `Extend::Concatenation` со списком элементов, и каждый элемент этот обход
/// видит сам. Упаковка цепочки в синтетическую модель со ступенями отвергнута
/// решением ADR 0057 (её безусловный `next` между ступенями дал бы неверный
/// тайминг), а её реализация снята как мёртвая — ADR 0278.
///
/// ⚠️ Заведут такую упаковку заново — обход придётся дополнить спуском:
/// синтетической модели нет в `models` родителя, рекурсия `validate_model_all`
/// до её состояний не дойдёт, и пустая модель внутри `A + B` уедет в цели
/// молча.
