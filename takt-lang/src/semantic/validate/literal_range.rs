...
///
/// Этот же диапазон нужен свёртке инициализатора - она нормирует **вычисленное**
/// значение по типу объявления - и полю ввода порта на странице прогона. Вторая копия
/// границ разошлась бы с первой, и одна и та же запись получала бы разные вердикты.
pub fn type_range(ty: &TypeNode) -> Option<(i128, i128)> {
    match ty {
