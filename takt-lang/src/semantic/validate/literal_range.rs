...
//!
//! ⚠️ Проверяется **литерал автора**, а не всякое значение. Инициализатор
//! объявления к моменту проверки уже свёрнут (фича 0192), поэтому вычисленное
//! значение приходит сюда литералом — и было бы неотличимо от написанного
//! руками. Разводит их **свёртка**: вычисленное она нормирует по типу
//! объявления (правило 0127: беззнаковое — обёртка `mod 2ⁿ`), и до проверки
//! доходит уже значение в диапазоне; литерал автора она не трогает вовсе, и его
//! выход за границы остаётся ошибкой (фича 0207). В присваивании внутри тела
//! свёртки нет — там проверяется именно записанный литерал.
...
///
/// `pub(crate)`: этот же диапазон нужен свёртке инициализатора (фича 0207) —
/// она нормирует **вычисленное** значение по типу объявления. Вторая копия
/// границ разошлась бы с первой, и одна и та же запись получала бы у проверки и
/// у свёртки разные вердикты.
