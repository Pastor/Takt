...
/// (фича 0391, решение заказчика 2026-08-23).
///
/// # Почему не ноль
///
/// Правило 0353 («переменная без инициализатора получает ноль») для
/// перечисления даёт значение, которого в наборе может не быть: при
/// `enum Mode { Idle = 5, Work = 7 }` ноль — состояние, о котором не знает ни
/// один `match`. Замер 2026-08-22 показал и цену: восемь потребителей давали
/// `0`, а цель `rust` — `Idle` (5), **молча** (все инструменты вывод
/// принимают, коды возврата совпадают).
///
/// # Почему ПЕРВЫЙ ПО ТЕКСТУ, а не наименьший
///
/// Порядок объявления в языке значим (правило 0034: агрегат позиционный), и
/// при `enum E { B = 2, A = 1 }` «первый» и «наименьший» — разные ответы.
/// Автор видит текст, а не таблицу значений.
///
/// `None` — перечисление без вариантов; такой вход отвергает `SE-105` (0172),
/// то есть ветвь защитная.
