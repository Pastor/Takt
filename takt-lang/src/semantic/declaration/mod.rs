    ...
    for name in order {
        ...
        let literal = match const_eval::fold_to_literal_in(source, model, &known) {
            ...
            Err(cause) => {
                ...
                // (фича 0310): вычислитель отвечает двумя родами сообщений, и
                // ошибку самой записи проглатывать нельзя — иначе вход уедет к
                // потребителям, где они разойдутся молча. Так `300 as i8` в
                // объявлении давал `0` у эталона, `44` у `c`/`rust` и потерю
                // инициализатора у `st`.
