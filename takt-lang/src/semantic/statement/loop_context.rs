//! Признак «мы внутри цикла» на построении тел — `SE-132` (фича 0530).
//!
//! # Что и почему
//!
//! `break` и `continue` вне цикла компилятор принимал молча, и дальше
//! потребители расходились. Замер 2026-09-03 на записи
//!
//! ```takt
//! start Run {
//!     always { x := x + 1; break; }
...
//!
//! Код возврата `taktc` — нулевой, диагностики нет ни у компилятора, ни у
//! эталона: молчаливое расхождение эталона и цели, то есть класс, ради которого
//! в проекте заведены потактовые сверки.
//!
//! # Почему отказ, а не поведение
//!
//! Задать `break` вне цикла смысл можно («прервать тело блока»), но это была бы
//! новая конструкция управления, которую обязаны одинаково воспроизвести восемь
//! целей: в C для неё нужен не `break`, а флаг или `goto`. Язык такого не
//! обещает — наоборот, документ обещает **ошибку** (`SIM-024` у эталона внутри
//! функции). Поэтому запись отвергается там, где её видно раньше всех: на
//! построении тела.
//!
//! # Как устроено
//!
...
//! учитывается разрушением [`LoopGuard`], поэтому ранний возврат по `?` не
//! «залипает».
