...
pub struct FormulaSite {
    ...
    /// Позиция САМОЙ формулы; вместилище — запасной ход (фичи 0282, 0474).
    ///
    /// ⚠️ Прежде здесь стояла координата вместилища, и разница была
    /// наблюдаема: две формулы уровня модели печатались с одной и той же
    /// строкой `1:1` (замер 0168). Своей позиции у формулы в дереве не было —
    /// её завели 0282 (охранная) и 0471 (темпоральная).
    ///
    /// ⚠️ Запасной ход остаётся: формула, построенная ВНЕ разбора, координаты
    /// не имеет — там и печатается место вместилища.
    pub loc: Location,
...
fn collect_formula(
    ...
    match formula {
        // Позиция берётся у САМОЙ формулы (фича 0474): с 0471 темпоральная её
        // несёт, а прежде сюда шла координата вместилища — три формулы уровня
        // модели печатали `SE-055` с одной и той же строкой `1:1`, и понять,
        // о которой речь, было нельзя. Запасной ход тот же, что у охранной
        // формы: формула, построенная вне разбора, своей позиции не имеет.
