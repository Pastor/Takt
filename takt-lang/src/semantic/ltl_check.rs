...
//! [`unused_variable_warnings`](crate::unused_variable_warnings); `taktc` их пока
...
fn collect_model(model: &Rc<RefCell<ModelNode>>, out: &mut Vec<Diagnostic>) {
    ...
    for site in model_ltl_formulas(&borrowed) {
        ...
        out.push(
            Diagnostic::warning(
                loc,
                // ⚠️ Номера фич из ТЕКСТА убраны (фича 0332): автор модели о них
                // не знает, а ссылка на задачу в диагностике читается как
                // обещание работы. Инструмент назван — этого довольно.
