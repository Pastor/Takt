...
pub enum VariableNode {
    ...
    Port {
        ...
        /// Тип переменной.
        ty: TypeNode,
        /// Размещение порта — выражение адреса из `at <адрес>` (фича 0187).
        ///
        /// `ExpressionNode::None`, если `at` не написан: адрес тогда приходит по
        /// имени порта (оператор `address`, внешняя карта), а полноту проверяет
        /// слой адресов после разрешения источников, а не объявление.
        ///
        /// ⚠️ Поле **переименовано** из `expr` намеренно: адрес и начальное
        /// значение делили одно поле, и добавить второе рядом со старым именем
        /// значило бы оставить потребителей молча читать не то. Переименование
        /// заставляет компилятор показать все места.
        address: ExpressionNode,
        /// Начальное значение порта — выражение из `:=` (фича 0187).
        ///
        /// `ExpressionNode::None`, если не задано. У **входного** порта задавать
        /// его незаконно (`SE-092`): значение входа приходит извне.
        init: ExpressionNode,
