...
pub(crate) fn construct_stages_within(
    ...
    // Перепривязка вызовов в ИМЕНОВАННЫХ условиях (фича 0503) — сразу после
    // стадии 5 и ДО стадий 4 и 6: обе копируют значение условия себе, и
    // перепривязка после них не дошла бы до копий. Поиск функции отдаёт снимок,
    // а на стадии 3 в карте лежала ещё неразрешённая функция.
