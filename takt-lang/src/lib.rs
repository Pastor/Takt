...
pub mod address_map;
pub mod compile_cli; // подкоманда `compile`, вынесена из бинарника (правило размера, 0043)
...
pub mod style; // стиль оформления: канон именования (фича 0226)
...
/// Компиляция по имени цели: одна точка выбора для CLI и модуля (фича 0531).
pub mod compile;
