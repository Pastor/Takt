//! Ошибки, которые парсер отдаёт как `ParseError::User`: тип [`LexicalError`],
//! их позиции и коды.
//!
...
//!
//! ⚠️ Имя типа говорит о лексике, а содержимое шире: сюда же попадают **правила
//! языка, проверяемые действием грамматики** (фича 0189, `SY-007`). Причина
//! техническая: `ParseError::User` параметризован **одним** типом ошибки, и
//! завести второй, не переписав сигнатуры всех точек разбора, нельзя. Код
//! диагностики при этом честный — `SY-`, а не `LE-`.
...
pub enum LexicalError {
    ...
    /// Оператор-выражение без эффекта: `x + 1;`, `#0x100.4;` (фича 0189).
    ///
    /// Оператором остаются **присваивание** и **вызов функции**: у них есть
    /// действие. Прочее выражение в позиции оператора — вычисление, результат
    /// которого некуда деть.
    ///
    /// Проверка стоит в **грамматике**, а не в семантике: этого требовал
    /// заказчик (решение 6A ADR 0189), и тот же приём уже применён к позиции
    /// присваивания (фикс 0187-01). Цена прежнего молчания подтверждена пробой:
    /// `always {{ x + 1; }}` язык принимал, цель `c` печатала `model->x + 1;`,
    /// и отказ приходил от **чужого** инструмента на порождённом файле
    /// (`cc -Wall -Werror`: `-Wunused-value`).
    #[error(
