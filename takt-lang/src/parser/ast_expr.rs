...
pub enum Expression {
    ...
    ///
    /// Хранит **выбор автора**, а не канонический вид: `None` в позиции бита
    /// означает, что бит не записан (`#0x100`), `Some(0)` — записан явно
    /// (`#0x100:0`). Формы означают одно и то же, но форматтер обязан печатать
    /// исходную (синонимы не канонизируются — то же правило, что у
    /// `while`/`loop`).
    ///
    /// Адрес хранится в `i128`, как и всякий числовой литерал (фича 0157):
    /// сужение до `i64` — задача семантики, и она делает его **явным отказом**,
    /// а не `as i64`.
    ///
    /// ⚠️ Ширина доступа в узле **не хранится**: её задаёт приведение
    /// (`as u32`) либо битовый доступ (`.4`), стоящие над обращением. Свёртку
    /// тройки `{адрес, бит, ширина}` делает семантика в единой воронке.
