...
# Предел тела — тот же, что у сервиса (TAKT_WEB_BODY_LIMIT). Действует на весь
# блок; у соседа он свой и, если задан, останется больший из указанных явно.
...
# ⚠️ Завершающая косая в proxy_pass СРЕЗАЕТ префикс: сервис получает адрес от
# своего корня. Убери её — и он увидит ${PREFIX}${PREFIX}/...
#
