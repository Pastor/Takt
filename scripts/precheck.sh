...
#
# `-Wextra` добавлен фичей 0220 — но с ЯВНЫМ исключением одного класса.
# Замер 2026-08-17: под `-Wall -Wextra` корпус даёт 49 предупреждений, гейт
# `c-hal` ниже — 45, и ВСЕ они `-Wunused-parameter` на трёх структурных
# параметрах: `main` (указатель на корень в сигнатурах под-моделей, 41+36),
# `model` (указатель на модель в порождённых пользовательских функциях, 8+7),
# `m` (в помощниках HAL, 2). Убрав этот класс, получаем ноль в обоих гейтах —
# то есть весь прочий `-Wextra` (`-Wsign-compare`, `-Wtype-limits`,
# `-Wmissing-field-initializers`, …) вывод уже проходит, и брать его нужно.
#
# ⚠️ Исключение НАЗВАНО, а не забыто: единообразие сигнатур порождённых функций
# — контракт вызова, и убирать параметр по факту неиспользования значит делать
# прототип зависимым от тела (эмиссия идёт из четырёх модулей генератора). Эта
# работа заведена кандидатом; у цели `rust` она уже сделана предикатом
# `needs_hal`. Снять исключение — убрать `-Wno-unused-parameter` в ДВУХ местах.
#
...
cmake -DCMAKE_BUILD_TYPE=Debug -G Ninja \
  -DCMAKE_C_FLAGS="-Wall -Wextra -Wno-unused-parameter -Werror" \
...
if require_tool cc "гейт c-hal, фикс 0020-01; обычно есть на всех платформах"; then
  ...
  for lam_file in examples/*.takt; do
    ...
    for c in "$chal_dir"/*.c; do
      ...
      # цели `c` одна, иначе класс, невидимый здесь, всплывал бы там. Замер и
      # причина исключения `-Wno-unused-parameter` — у гейта корпуса выше
      # (здесь тот же класс даёт 45 предупреждений, после исключения — ноль).
