...
if require_tool python3 "проверка ссылок, правило 14; apt install python3"; then
  ...
  # Реестры стадий против файлов на диске (фича 0164). Реестр — вход в проект:
  ...
  echo "Реестры стадий против диска (фича 0164)..."
...
# Снимки порождённого кода в book/ (фича 0274): файл в `generated/<цель>/` есть
# ВЫВОД инструмента на соседнем примере, и приложение «Порождённый код примера»
# читает его напрямую. Замер 2026-08-19 нашёл ТРИ отставших снимка: `rust` и
# `sv` несли имя константы без квалификации владельцем (фича 0193), `st` — без
# блока первого скана (фича 0191), то есть документ показывал автомат, сдвинутый
# на такт. Прежде расхождение находили только глазами.
...
# Команды README (фича 0275): то, что документ предлагает набрать, обязано
# работать. Замер 2026-08-17 нашёл `cargo install --path grammar` — команду,
# мёртвую с переименования крейта (фича 0100); первый же прогон гейта нашёл
# вторую беду: `taktc compile -t st-at --address-map …` роняло компилятор
# ПАНИКОЙ (координата карты печаталась по тексту модели). Гейт ссылок этого не
# видит по устройству — он проверяет пути в markdown-ссылках, а не вызовы.
