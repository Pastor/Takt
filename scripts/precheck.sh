...
if require_tool python3 "проверка ссылок, правило 14; apt install python3"; then
  ...
  echo "Вердикт реестров стадий 5–6 (фича 0218)..."
...
# которой неверны все четыре элемента. ⚠️ Имена в ПОРОЖДАЕМОМ коде (`lam_q_*`,
# `LAM_Q_*`) гейт не проверяет намеренно — граница названа в его заголовке.
# Быстрая — до тестов.
"$(dirname "$0")/check-legacy-names.sh"

# Сторож гейта выше (фича 0161): мутацией доказывает, что каждый запрещённый
# шаблон ловится, падение идёт СПИСКОМ, исключение `docs/` действует, а граница
# написана. Гейт, который никогда не падал, неотличим от неработающего.
...
# `probe.sh` ловит каждую известную ловушку пробы и не поднимает ложную тревогу
# на годной. Стоит ЗДЕСЬ, после сборки: скрипту нужны собранные taktc и
# takt-sim. ⚠️ Повод прямой — `run_simulations.sh` сломался в день фичи 0251
# (искал бинарник в старом каталоге) и не был замечен, потому что его никто не
# запускает: инструмент без сторожа неотличим от сломанного.
