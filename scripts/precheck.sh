...
if require_tool python3 "проверка ссылок, правило 14; apt install python3"; then
  ...
  # Символы вне шрифта документа (фича 0146). Сборка book/ в предкоммит не
  # входит (нужны mdbook/pandoc/xelatex + сам шрифт), а `xelatex` на непокрытом
  # глифе не падает — он печатает предупреждение и МОЛЧА выбрасывает символ из
  # PDF (фикс 0132-01). Поэтому покрытие проверяется текстом, по снимку cmap.
