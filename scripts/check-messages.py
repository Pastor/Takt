...
* `M8` - три пробела подряд в однострочном тексте: след строки, склеенной в
  исходнике переносом с отступом. Многострочный текст (справка с колонками, где
  есть `\\n`) под правило не подпадает — там выравнивание законно.
    ...
    for lang, entries in catalogues.items():
        for key, value in entries.items():
            if "\\n" not in value and "   " in value:
                errors.append(
                    f"M8 {lang}.txt: ключ '{key}' — три пробела подряд "
                    "(след строки, склеенной в исходнике)"
                )
        if lang == BASE_LANG:
            continue
