...
# Второй признак заглушки (фича 0332): ссылка на ЗАДАЧУ либо ФИЧУ в тексте
# диагностики. Замер 2026-08-20 нашёл **четыре** такие ветви в `st_expr.rs`
# («задача 0041-05», «часть 2 задачи 0041-04» ×2, «задача 0041-03»), и ни одну
# из них гейт не видел: они написаны не словом «пока», а обещанием работы.
# Фича 0041 при этом закрыта — обещание пережило её на год.
#
# ⚠️ Ищется только в строке с КАВЫЧКОЙ: упоминание фичи в комментарии законно и
...
def scan_sources(files):
    ...
    for path, text in files.items():
        ...
        for index, line in enumerate(lines):
            ...
            # ⚠️ Обещание задачи считается заглушкой, только если рядом строится
            # ДИАГНОСТИКА (есть `with_code`). Иначе в улов попадают справка CLI
            # («цель st … (фича 0041)») и комментарии, которые цель печатает в
...
def self_test():
    ...
    # Второй признак заглушки — обещание задачи в ТЕКСТЕ диагностики (0332).
    ...
    # ⚠️ Упоминание фичи ВНЕ диагностики законно: так объясняют решение в
    # комментарии и печатают пояснение в порождённый код. Гейт, ловящий и это,
