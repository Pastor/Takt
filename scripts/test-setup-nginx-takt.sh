...
#   N2  **завершающая косая в `proxy_pass` есть** — она срезает префикс, и без
#       неё сервис увидел бы `/takt/takt/...`;
...
grep -qF "proxy_pass http://takt_backend/;" "$WORK/conf" || {
  echo "  ПРОВАЛ: N2 нет завершающей косой в proxy_pass — сервис увидит /takt/takt/..."
  exit 1
}
echo "  OK: N2 завершающая косая на месте — префикс срезается"
