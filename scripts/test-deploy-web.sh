...
#   E5  шаблон nginx несёт то же правило кеша, что и сервер, и предел тела;
#   E6  нет `node` либо модуля — мягкий пропуск, под `PRECHECK_STRICT=1` ошибка.
#
...
# ── E5 и E4 не требуют сборки: проверяются текстом ───────────────────────────
NGINX="$ROOT/web/deploy/nginx.conf.template"
for needle in "immutable" "no-cache" "client_max_body_size" "server_tokens off"; do
  grep -q "$needle" "$NGINX" || {
    echo "  ПРОВАЛ: E5 в шаблоне nginx нет '$needle'"
    exit 1
  }
done
grep -q '__SERVER_NAME__' "$NGINX" && grep -q '__UPSTREAM__' "$NGINX" || {
  echo "  ПРОВАЛ: E5 шаблон nginx без подстановок — он не шаблон"
  exit 1
}
echo "  OK: E5 шаблон nginx несёт правило кеша, предел тела и подстановки"
...
if TAKT_NODE=такого-нет "$0" 2>&1 | grep -q "пропуск"; then
  echo "  OK: E6 без node — мягкий пропуск"
else
  echo "  ПРОВАЛ: E6 без node пропуск не назван"
  exit 1
fi
if TAKT_NODE=такого-нет PRECHECK_STRICT=1 "$0" >/dev/null 2>&1; then
  echo "  ПРОВАЛ: E6 под PRECHECK_STRICT=1 отсутствие node прошло"
  exit 1
fi
echo "  OK: E6 под PRECHECK_STRICT=1 отсутствие node — ошибка"
