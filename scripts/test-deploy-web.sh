...
# ⚠️ Сервер вкладывает под префикс ВЕСЬ роутер: при `TAKT_WEB_BASE_PATH=/takt`
...
# ⚠️ Дерево копируется целиком и раньше сборки, поэтому строка в журнале рушит
...
# ⚠️ Кеш нужен ОБЕИМ сборкам — модуля и сервера: без него любая из них
...
# ⚠️ Оборванный канал без «живых» проб висит бесконечно, и выкладка выглядит
