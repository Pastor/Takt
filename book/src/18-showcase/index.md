...
`GoingDown` — движение к цели. `Stopping` — торможение: привод выключается, тормоз
зажимается. `Boarding` — посадка с открытыми дверьми и выдержкой. `Leaving` —
закрытие дверей и возврат в `Waiting`.

Связь с оборудованием — через **порты с адресами** (2). Вызов приходит на входной
порт `call`, а **положение кабины — на входной порт `at_floor`**: это датчик
шахтной системы позиционирования. Команды приводу и дверям кладутся в один регистр
`0x50000010` по отдельным битам (мотор вверх — бит 0, вниз — 1, тормоз — 2, двери
— 3), индикатор этажа — порт `display`. Состояние привода и дверей дублируется во
внутренних переменных `moving`/`doors` (выходной порт читать нельзя), и на них
стоит **инвариант** `SafeMove` — «привод выключен **или** двери закрыты», то есть
кабина никогда не движется с открытыми дверьми; проверяется на **каждом** такте.

Здесь важно, **что такое такт**. Такт — это шаг **логики** автомата, а не единица
времени или расстояния: частота тактирования произвольна (хоть 1 МГц) и к физике
отношения не имеет. Поэтому положение кабины **не вычисляется счётом тактов** —
оно приходит с датчика: индикатор просто следует за `at_floor` (3), а движение
прекращается, когда **датчик** доложил о достижении цели — `ref Stopping: at_floor
>= call;` (5). Двигает кабину привод (`motor_up`/`motor_down`), а фиксирует —
датчик; автомат лишь командует и читает показания.

Переходы из `Waiting` (4) разбирают вызов: `call` равен текущему этажу — сразу
`Boarding`; выше — `GoingUp`; ниже (и не ноль) — `GoingDown`. После торможения
переход в `Boarding` задан безусловным `next` — тормоз уже отработал, ждать нечего.
Единственное, что здесь **законно** считать тактами, — это **отрезок времени**:
выдержка дверей на посадку. Счётчик `dwell` (6) растёт каждый такт, и по достижении
порога `DWELL_TICKS` (1) кабина закрывает двери. Порог задан **в тактах и
подстраивается под частоту** устройства (комментарий указывает и длительность, и
заложенную частоту) — а не выдаёт число за секунды.
...
JSON-файлом — на каждом такте значения входных портов; здесь симулятор играет роль
**окружения**: он и подаёт вызов `call`, и двигает датчик `at_floor`, изображая
физическое перемещение кабины по шахте:
...
Вызван 3-й этаж — кабина трогается вверх (шаг 1). Пока датчик `at_floor` показывает
1, потом 2 (шаги 2–3), кабина едет; как только датчик доложил `3` (шаг 4) —
торможение. Обрати внимание: `display` следует **за датчиком**, а остановка
наступает **по показанию датчика**, а не по числу тактов — сколько тактов длилось
движение, неважно. Дальше двери открываются и держат выдержку `DWELL_TICKS = 3`
такта (шаги 5–7 — это отрезок **времени**), затем закрываются (`Leaving`, шаг 8) и
кабина возвращается в покой (шаг 9). Ни на одном шаге `moving` и `doors` не равны
единице одновременно — инвариант держится.

И это не просто удача сценария: инвариант `SafeMove` **сторожит** прогон.
Симулятор проверяет его в начале **каждого** такта, и стоит модели нарушить
правило — прогон останавливается, а не выдаёт молча недостоверную трассу. Если бы
автор ошибся — скажем, в `enter` состояния `GoingUp` по недосмотру открыл двери
(`doors := 1`), пустив кабину в движение с открытыми дверьми, — тот же сценарий
дал бы не трассу, а срыв на первом же нарушении:
...
Поэтому симуляция здесь — не только показ поведения, но и **проверка правил
безопасности по ходу прогона**: зелёная трасса выше — это ещё и свидетельство,
что `SafeMove` не нарушен ни на одном её шаге.
...
вершины суть состояния, а рёбра — переходы. Вершины обозначены компактно `S₁`,
...
такте. Симулятор проверяет её по ходу прогона и при нарушении останавливает
прогон с диагностикой `SIM-025` — ровно так, как в отрицательном прогоне из
раздела «Симуляция» выше; в порождённом C та же формула становится
`assert` в начале `_tick` (см. фрагмент кода). Так **одно** объявление в модели
даёт сразу три уровня защиты: проверку при симуляции (guard ловит ошибку до
прошивки), защиту в целевом коде (`assert` на устройстве) и вход в верификацию —
правило безопасности «не двигаться с открытыми дверьми» записано **один раз**, а
работает везде.
