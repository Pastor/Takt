# Фича 0480: Цель st: имена одного POU обязаны различаться без учёта регистра
...
- **Tier:** 2
- **Класс:** генерация
- **Связанные issue (анализ):** кандидат блока 2 `FEATURES.md` (заведён при закрытии [0407](0407-const-power-fold.md), 2026-09-01)
...
одно имя, объявленное дважды. Цель `st` такие пары не сторожит: `taktc`
...
⚠️ **Языковые разделы не трогаются, и это проверено чтением:** ни `ST-014`, ни
...
- **Признак строится из того, что цель ПЕЧАТАЕТ**, а не из того, что объявлено
  (урок фикса 0378-01: ложный отказ хуже пропуска).
...
4. **Три частных правила** (переменная ↔ константа, порт ↔ переменная, порт ↔
   ...
   первом же новом виде объявления — класс 0084/0193/0195.
...
3. Корпус `examples/` собирается по-прежнему (гейт цели `st` зелен).
...
Довести сторожа имён цели `st` до полноты по **одному POU**: сегодня он знает
...
⚠️ Первая редакция пробы дала **посторонний** `ST-014`: контрольную константу
...
пять контролей, прогон `iec2c` на выводе контролей, гейт цели `st` на корпусе,
предкоммит.
...
⚠️ Риск ложного отказа снимается тем же приёмом, что в фиксе 0378-01: набор
занятых имён строится из того, что цель **печатает**, и каждая запись проверена
прогоном `iec2c`.

### Риски и зависимости

- **Риск: корпус перестанет собираться.** Снимается прогоном гейта цели `st`
...
### Задача 0480-01 — носитель отказа `ST-025`
...
### Задача 0480-02 — накопитель занятых имён в точке эмиссии
...
⚠️ Отдельного списка служебных имён **не заводится**: он разошёлся бы с печатью
при первом же новом служебном объявлении (класс 0084/0193/0195).

⚠️ Проверка стоит **после** фильтра использования и **после** ветви
...
принимает. Корпус собирается — гейт цели `st` зелен.

### Задача 0480-03 — документ
...
⚠️ Языковые разделы не трогались: ни `ST-014`, ни `ST-023` в них не
...
| K7 | корпус `examples/` | гейт цели `st` зелен |
...
и `iec2c` принял вывод каждого контроля. K7 — зелен (гейт цели `st` на корпусе
...
⚠️ Третья мутация вначале **не ловилась**, и это дефект тест-плана, а не кода:
...
Фиксов не заведено. Соседний класс вынесен кандидатом: цель `rust` на
...
Правило **одно**, и признак строится из того, что цель **уже напечатала**
(задача 0480-02): отдельного списка служебных имён нет — он разошёлся бы с
