...
- **Статус:** ТЕСТИРОВАНИЕ — реализация (`0025-01`…`0025-05`) завершена, отчёт
  ...
  выполнены пункты **собственного тест-плана**: фикстуры
  `simulation/tests/data/eval/`, интеграционный слой `simulation/tests/`, перенос
  примеров в `README.md` (правила 15, 16), автосверка с `lamc -t c` (A8).
  До их закрытия (`0025-06`…`0025-08`) фича в `ГОТОВО` не переводится: проверка
  невоспроизводима, а именно отсутствие такого слоя и позволило дефектам прожить
  незамеченными
