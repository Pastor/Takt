...
- **Date:** 2026-07-30
- **Authors:** Архитектор
- **Related issues:** [Фича 0157](0157-literal-u64-representation.md);
  предшественник — [ADR 0128](0128-lexer-literal-overflow.md#архитектура-adr) (паника → `LE-009`),
  смежные — [ADR 0078](0078-bit-array-semantics.md#архитектура-adr) (`[bit;N]`),
  [ADR 0127](0127-int-overflow-semantics.md#архитектура-adr) (переполнение целых),
  [фича 0144](0144-int-literal-exponent.md) (экспонента у целого),
  [ADR 0171](0171-c-gate-werror.md#архитектура-adr) (гейт `c` под `-Werror`).
...
- Экспонента у целого литерала ([0144](0144-int-literal-exponent.md))
...
- Язык меняется: расширено множество допустимых литералов, добавлена
  диагностика диапазона. ⚠️ **Номер версии при этом не растёт**: правило 22 —
  версия накапливается по циклам, а `0.8.0` (поднята фичей 0185) ещё не
...
- **Влияние на порядок разработки:** ничего не блокирует и не разблокирует
  напрямую. Смежная [0144](0144-int-literal-exponent.md) (экспонента
...
(`-Wconstant-conversion` под `-Werror`, [ADR 0171](0171-c-gate-werror.md#архитектура-adr)).
...
(«усечение при записи», [ADR 0127](0127-int-overflow-semantics.md#архитектура-adr)) было
...
идут под `-Wall -Werror` ([ADR 0171](0171-c-gate-werror.md#архитектура-adr)). То есть
...
(правило 11 + детерминизм [0048](0048-deterministic-codegen.md#архитектура-adr)).
