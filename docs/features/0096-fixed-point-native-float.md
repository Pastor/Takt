...
- **Связанные issue (анализ):** новая фича (запрос заказчика 2026-07-19); **прямо
  пересматривает** отвергнутый [ADR 0061](0061-fixed-point-type.md#архитектура-adr) Option B
...
⚠️ **Это прямо пересматривает [ADR 0061](0061-fixed-point-type.md#архитектура-adr):** тот
...
[ADR 0096](0096-fixed-point-native-float.md#архитектура-adr)): флаг переключает **обе**
...
- **Related issues:** [Фича 0096](0096-fixed-point-native-float.md);
  **пересматривает** [ADR 0061](0061-fixed-point-type.md#архитектура-adr) (Option B, драйвер 1)

### Context

Фича [0061](0061-fixed-point-type.md) дала языку тип `q(m, n)` и
...
целочисленный Q по флагу (embedded). Дизайн — [ADR 0096](0096-fixed-point-native-float.md#архитектура-adr)
(Option A). Фича **сознательно пересматривает** [ADR 0061](0061-fixed-point-type.md#архитектура-adr)
(Option B/драйвер 1) с сохранением инварианта побитовой сверки **внутри режима**.

### Зависимости фичи (правило 17/19)

- **Зависит от:** **[0061](0061-fixed-point-type.md)** (жёстко) — вся
...
| Симулятор | двухрежимный эталон `float` | T7, T8, T9 | ✅ (0096-02 `sv` Q; 0096-03 native≠Q сторож) |
| Явный `q(m,n)` (0061) | **не задет** | T10 | ✅ (сверки 0061 зелены; гейт исходного типа) |
