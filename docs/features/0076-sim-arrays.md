...
- **Related issues:** [Фича 0076](0076-sim-arrays.md); смежна [0078](0078-bit-array-semantics.md) (семантика `[bit;N]`); продолжает [0034](0034-sim-struct-types.md) (place/update структур), [0025](0025-simulator-expression-eval.md) (семантика вычислений).
...
чтении. Решение — по [ADR 0076](0076-sim-arrays.md#архитектура-adr) (Option A): единый
...
- **Зависит от:** формально `нет` (все нужные части уже в ядре). Продолжает
  [0034](0034-sim-struct-types.md) (`eval/place`, `update`,
  `StructRegistry`) и [0025](0025-simulator-expression-eval.md)
  (`eval/`, `coerce_to_type_with`) — переиспользует их механизмы, но не ждёт от
  них новой работы. Смежна [0078](0078-bit-array-semantics.md)
...
- **Влияние на порядок разработки:** завершение снимает блокировку T18
  тест-плана [0029](0029-c-type-mapping.md) (сверка по `Array`).
...
Реализация по [ADR 0076](0076-sim-arrays.md#архитектура-adr), Option A — единый механизм
...
Реализовано по [ADR 0076](0076-sim-arrays.md#архитектура-adr) (Option A) — симулятор
