...
- **Зависит от:** **нет** (проверено аналитиком 2026-07-17 — см. [анализ](0058-rust-tail-return-if-else.md#анализ), раздел «Зависимости фичи»). Опора — **закрытая** [0048](0048-deterministic-codegen.md): без детерминизма генерации критерий A2 неисполним.
...
- **Related issues:** [Фича 0058](0058-rust-tail-return-if-else.md); наследует политику линтов [ADR 0050](0050-rust-backend.md#архитектура-adr) (вариант (а): `-D warnings`, не глушить)
...
⚠️ Дефект был **скрыт пропуском** `comprehensive` в гейте цели `rust` и обнажился,
когда [0030](0030-comprehensive-example-fix.md#архитектура-adr) сняла пропуск: пропуск, объявленный
...
3. **Не сломать то, что работает.** Корпус (`examples/`) сегодня гейт проходит;
   генерация детерминирована (фича [0048](0048-deterministic-codegen.md#архитектура-adr)), поэтому
...
- **A-4. Вторая находка пробы — вне объёма 0058.** Проба формы 2 показала, что
  ...
  только на публичных). Класс — ровно как у [0026](0026-c-root-typedef.md#архитектура-adr): дыра
...
- **Зависит от:** **нет**.
  ...
  - **Опора.** Требуемый детерминизм генерации даёт **закрытая**
    [0048](0048-deterministic-codegen.md) — критерий A2 (побайтовая
...
- **Влияние на порядок разработки:** фича **никого не разблокирует** и **никем не
  заблокирована**. Соседство с [0059](0059-rust-shared-struct.md)
...
закрытой [0026](0026-c-root-typedef.md): «дыра не в коде, а в
...
`tail_foldable(stmt)` (предикат из [0058-01](0058-rust-tail-return-if-else.md#разработка)):
