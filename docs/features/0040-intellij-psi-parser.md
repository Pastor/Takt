...
- **Related issues:** [Фича 0040](0040-intellij-psi-parser.md); развивает
  [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr) (плоский `LamParser`) и
  [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr) (лексер, автономность плагина);
  пересекается с фичами [0038](0038-intellij-semantic-tokens.md)
  (LSP4IJ / семантическая подсветка) и [0039](0039-intellij-reformat.md)
...
Дать в плагине `extensions/intellij-lam` то, что лёгкий путь [0023](0023-intellij-navigation-include.md)
...
|---|---|---|
| [0040-01](0040-intellij-psi-parser.md#разработка) | Структурный PSI-парсер: каркас и узлы деклараций | R7, R8, R9 (ядро) |
| [0040-02](0040-intellij-psi-parser.md#разработка) | Ссылки и резолв имён на PSI; `PsiReference` для путей `import` | R5, R8 |
| [0040-03](0040-intellij-psi-parser.md#разработка) | Find usages поверх PSI | R1 |
| [0040-04](0040-intellij-psi-parser.md#разработка) | Rename (штатный рефакторинг IDEA) | R3 |
| [0040-05](0040-intellij-psi-parser.md#разработка) | Арбитраж с LSP4IJ и приёмка R2/R4/R6 от 0038 | R2, R4, R6, R10 |
...
> [0038](0038-intellij-semantic-tokens.md) (правило 17): работы не
...
- Навигация 0023 — **риск регресса**: форма дерева меняется; 47 тестов обязаны
  ...
  `LamGotoDeclarationHandler` на PSI — задача [0040-02](0040-intellij-psi-parser.md#разработка).
...
> **Планируется (разработка не начата).** Фича **ЗАБЛОКИРОВАНА** зависимостью
> [0038](0038-intellij-semantic-tokens.md); задача выполняется после
> [0040-01](0040-intellij-psi-parser.md#разработка).
...
1. **`LamNamedElement`** — `NAME_ID` из [0040-01](0040-intellij-psi-parser.md#разработка)
   реализует `PsiNamedElement` (`getName`/`setName`/`getNameIdentifier`).
   `setName` — основа rename ([0040-04](0040-intellij-psi-parser.md#разработка)).
2. **`LamNameReference`** — `PsiReferenceBase<LamNameElement>` на идентификаторе-
   ...
   реализуется** (R6 принимается от 0038, см. [0040-05](0040-intellij-psi-parser.md#разработка)).
...
> **Планируется (разработка не начата).** Фича **ЗАБЛОКИРОВАНА** зависимостью
> [0038](0038-intellij-semantic-tokens.md); задача выполняется после
> [0040-02](0040-intellij-psi-parser.md#разработка).
>
...
3. **Опора на ссылки [0040-02](0040-intellij-psi-parser.md#разработка)**: usages — это
...
4. **Область — файл.** Кросс-файловые usages **не реализуются**: это работа
   ...
   `semantic/tree.rs`).
5. **Арбитраж с LSP4IJ** — [0040-05](0040-intellij-psi-parser.md#разработка): не показывать
...
> ([0040-02](0040-intellij-psi-parser.md#разработка)) rename — единственное, чего путь LSP4IJ
...
1. **`LamNamedElement.setName`** (введён в [0040-02](0040-intellij-psi-parser.md#разработка)) —
...
- `lam-lsp` — **н/п**; при арбитраже ([0040-05](0040-intellij-psi-parser.md#разработка))
...
> **Планируется (разработка не начата).** Фича **ЗАБЛОКИРОВАНА** зависимостью
> [0038](0038-intellij-semantic-tokens.md). Задача **завершающая**:
...
[0038](0038-intellij-semantic-tokens.md) и проверяются её тест-планом
