...
- **Date:** 2026-08-16
- **Authors:** Архитектор
- **Related issues:** [Фича 0167](0167-c-enum-constants-usage.md); прямой образец — [ADR 0066](0066-st-bool-literals.md#архитектура-adr) (та же болезнь у цели `st`, приём `coerce_to`); смежно с [ADR 0195](0195-target-name-collisions.md#архитектура-adr) (имя перечислителя строит одна функция, столкновения имён в одном пространстве), [ADR 0193](0193-shared-const-qualified.md#архитектура-adr) (имя константы квалифицировано владельцем), [ADR 0060](0060-enum-width-shared-layer.md#архитектура-adr) (знак и ширина перечисления — общий слой)
...
Ровно то положение, из которого фича [0066](0066-st-bool-literals.md#архитектура-adr) вывела
цель `st`.
...
программу, класс [0193](0193-shared-const-qualified.md#архитектура-adr)); недостаёт **второго**
сегмента.
...
1. **Вывод не должен противоречить сам себе.** Цель объявляет имя и печатает
   ...
   [0066](0066-st-bool-literals.md#архитектура-adr) для `st`.
...
5. **Имя строит одна функция** (урок [0195](0195-target-name-collisions.md#архитектура-adr)):
...
- **Зависит от:** `нет`. Приём (`coerce_to` по целевому типу) закрыт фичей
  [0066](0066-st-bool-literals.md) для цели `st`; конструктор имени
  рядом с `port_enum_variant` заведён фичей
  [0195](0195-target-name-collisions.md).
...
[0193](0193-shared-const-qualified.md): ключ строится из `upper`
...
[0167-01](0167-c-enum-constants-usage.md#разработка).
