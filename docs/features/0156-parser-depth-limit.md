...
- **Related issues:** [Фича 0156](0156-parser-depth-limit.md); продолжение
  [ADR 0129](0129-semantic-deep-nesting.md#архитектура-adr) (предел глубины в семантике, `SE-062`)

### Context

Фича [0129](0129-semantic-deep-nesting.md) закрыла переполнение стека
...
3. **Предел `MAX_PARSE_DEPTH = 64`.** ⚠️ **Уточнён разработкой до 96** — см.
...
> [0156-02](0156-parser-depth-limit.md#разработка)): предел — 96, а не 64.**
...
[ADR 0156](0156-parser-depth-limit.md#архитектура-adr) принял **Option C**: глубина АСД
...
Задача продолжает [0129](0129-semantic-deep-nesting.md): та закрыла
...
- **Влияние на порядок разработки:** ни одну фичу не разблокирует и не блокирует.
  ...
  - [0099](0099-module-size-core.md) (заморожена) — правило размера
...
- **Версия языка (правило 22): не поднимается.** Изменение языковое (новая
  ...
  неё, как у [0157](0157-literal-u64-representation.md). Констатация
...
1. **Предел 64 → 96** (задача [0156-02](0156-parser-depth-limit.md#разработка)).
...
2. **Заглушки при утилизации не понадобились**
   (задача [0156-03](0156-parser-depth-limit.md#разработка)): дети
...
[0168](0168-generator-warnings-return.md). Записан в блок кандидатов
`FEATURES.md`.
