...
- `taktc fmt` падает на выражении с ~3000 скобок (рекурсия печати форматтера).
- Производный `Clone` для `ast::Expression` падает на ~2000 вложенных `if`
  (`verify`/`compile`; подтверждено `lldb`).
- Порог **на порядок выше** прежнего (240–280) и достижим только порождённым
  текстом — то есть это не боль пользователя, а незакрытый класс.
