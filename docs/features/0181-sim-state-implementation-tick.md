...
- **Related issues:** [Фича 0181](0181-sim-state-implementation-tick.md); закрывает [фикс 0057-01](../fixes/0057-01-sim-composition-next.md); разблокирует [фичу 0166](0166-sv-example-sequential-composition.md); наследует [ADR 0057](0057-sv-sequential-composition.md#архитектура-adr)
...
- фича [0166](0166-sv-example-sequential-composition.md)
...
- **Влияние на порядок разработки:** завершение **разблокирует**
  [0166](0166-sv-example-sequential-composition.md) — корпусной
...
| Задача | Содержание |
|---|---|
| [0181-01](0181-sim-state-implementation-tick.md#разработка) | деление `takt-sim/src/unit/mod.rs` (988/1000) — вынос такта в `unit/tick.rs`, поведение не меняется |
| [0181-02](0181-sim-state-implementation-tick.md#разработка) | `state_impls` в `UnitKind::Node`; построение реализации с контекстом узла; общий контекст для `Concatenation` (Д1, Д2) |
| [0181-03](0181-sim-state-implementation-tick.md#разработка) | такт узла тикает реализацию; спуск в неё во всех обходах; наблюдение после завершения (R1–R3, R5, R6, Д3) |
| [0181-04](0181-sim-state-implementation-tick.md#разработка) | тройная сверка sim↔C↔SV и значенческие тесты (A1–A7) |
...
4. **Фича [0166](0166-sv-example-sequential-composition.md)
