...
- **Related issues:** [Фича 0041](0041-st-backend.md); потребляет
  `AddressMap` из [ADR 0020](0020-port-address-decl.md#архитектура-adr); образец «второго
  бэкенда» — [фича 0009](0009-plantuml-generator.md); ядро генерации —
  [фича 0001](0001-core-language-c.md); урок отображения типов —
  [фича 0029](0029-c-type-mapping.md).
...
> и вернёт `ST-003` в рантайме. **Это неверно для внутрикрейтового кода:**
...
(`ST-004`…`ST-006` заняты задачей 0041-05 — см. [0041-05](0041-st-backend.md#анализ);
`ST-009`…`ST-011` — задачей 0041-04.)
...
### Открытые вопросы — **ЗАКРЫТЫ** (пробы 0041-06 + проверка 0041-02, 2026-07-15)
...
валидны по всему корпусу (проверено подстановкой фиктивного тела), но гейт
...
| `TypeNode` | ST-тип (T-таблица [0041-02](0041-st-backend.md#анализ)) | Размер | Пример |
|---|---|---|---|
...
2. **`VAR_GLOBAL` vs `VAR_EXTERNAL`.** Размещённые переменные объявляются
   ...
   вопросом О1 из [0041-03](0041-st-backend.md#анализ) (как под-FB видят переменные
...
| **П4** | **Перечислимый `TYPE`**: `TYPE Floor : (Bottom := 80, Top); END_TYPE` | `iec2c probe_enum.st` | **Решает T13** ([0041-02](0041-st-backend.md#анализ)): прямое отображение или откат Option C (`USINT` + константы) |
| **П5** | `ARRAY [0..3] OF USINT` в `VAR` блока FB | `iec2c probe_array.st` | Подтверждает T12 |
| **П6** | `LREAL` | `iec2c probe_lreal.st` | Подтверждает T11 |
| **П7** | `VAR_IN_OUT` в FB; вызов FB из FB | `iec2c probe_inout.st` | **Решает О1** ([0041-03](0041-st-backend.md#анализ)) — как параллельные под-FB видят переменные корня. Самый рискованный вопрос семантики |
...
|---|---|---|
| [0041-02-type-mapping.md](0041-st-backend.md#анализ) | 0041-02 | Отображение `TypeNode` → типы IEC; таблица `T1..T14`; отказ вместо тихой потери; урок 0029 |
...
[0041-02](0041-st-backend.md#анализ).
...
[0041-02](0041-st-backend.md#анализ) — готовая спецификация того, как должно быть.
...
- **R5. Отображение типов.** По нормативной таблице `T1..T14`
  ([0041-02](0041-st-backend.md#анализ)); в частности `Bit`→`BOOL`,
...
- **R6. Отображение состояний/переходов.** `FUNCTION_BLOCK` на модель,
  ...
  через `is_done` — изоморфно C ([0041-03](0041-st-backend.md#анализ)).
...
- **R8. Потребление карты адресов.** В цели `st-at` порты размещаются как
  ...
  [0041-05](0041-st-backend.md#анализ). Используемый порт без адреса → **SE-052**
...
- **R10. Валидность порождённого ST.** Порождённый ST принимается MatIEC `iec2c`,
  ...
  [0041-06](0041-st-backend.md#анализ).
...
| A9 | **Гейт:** `iec2c` принимает ST всех `examples/*.lam`; его C компилируется `cc` | Шаг в `precheck.sh` по образцу существующего C-гейта (cmake/ninja). **При недостижимости — задокументированный отказ** ([0041-06](0041-st-backend.md#анализ)) |
...
|---|---|---|---|
| **0041-01** | Каркас бэкенда: `Language::ST`, `generator/st/`, `StMap`, цели `st`/`st-at` | R1, R2, R3, R11 | [dev](0041-st-backend.md#разработка) |
| **0041-06** | **Проба-гейт:** валидация ST через MatIEC `iec2c` (идёт **второй**) | R10 | [dev](0041-st-backend.md#разработка) · [анализ](0041-st-backend.md#анализ) |
...
- **Следующая задача — [0041-06](0041-st-backend.md#разработка)** (проба-гейт
  MatIEC), а не 0041-02: её исход меняет проектные решения
  (`--st-configuration`, перечислимые `TYPE`, `VAR_IN_OUT`).

**Следующая задача — [0041-06](0041-st-backend.md#разработка) (проба-гейт MatIEC),
...
##### Открытые вопросы — **ЗАКРЫТЫ пробой [0041-06](0041-st-backend.md#разработка)** (2026-07-15)
...
> ✅ **ЗАКРЫТО пробой П7** ([0041-06](0041-st-backend.md#разработка), 2026-07-15):
...
и это ровно класс дефекта фичи 0025.
...
| **О3** | `continue` внутри `for` с шагом → **`ST-011`** | В Lam шаг выполняется и после `continue`, в `WHILE`-развёртке — нет. Тождественной развёртки **не существует**, поэтому громкий отказ вместо тихого расхождения (класс 0025) |
...
**падает с segmentation fault** (а не диагностикой). Для гейта это важно: падение
...
| Файл | Содержание |
|---|---|
| `generator/st/st_expr.rs` (новый) | `print_expression`, `print_condition` — два печатника (инвариант ADR 0019); 14 юнит-тестов |
...
Решение 0041-02 (целые → `USINT`…`LINT`) остаётся в силе.
...
6. **`extern fn`** — **РИ5 ЗАКРЫТ пробой П9** ([0041-06](0041-st-backend.md#разработка),
...
Цель `st-at` по корпусу (гейт `iec2c`):
...
| `stacker` | ✅ валиден |
| `elevator_mini` | ⛔ **`SE-052`** — и это ПРАВИЛЬНО: его порты не имеют адресов, а `st-at` без них размещать нечего. Нужна внешняя карта (`examples/elevator_mini.plc.map`, задача 0041-07) |
...
| **А3** | Размер локации — из `TypeNode`, **не** из C-типа | Цель `c` печатает для `bit` → `int` (дефект Д2 фичи 0029). Унаследовать это в адресацию нельзя: `%IX` и `%ID` указывают на **разные ячейки** |
...
- **`VAR_GLOBAL` vs `VAR_EXTERNAL` — ЗАКРЫТО пробой П8**
  ([0041-06](0041-st-backend.md#разработка), 2026-07-15). Форма
  ...
  **Следствие, меняющее план: цель `st-at` ОБЯЗАНА эмитить полную обёртку**
  ...
  Пересечение с О1 задачи [0041-03](0041-st-backend.md#разработка) **снято**: там
...
| **П4** | Перечислимый `TYPE Floor : (Bottom := 80, Top); END_TYPE` | `iec2c probe_enum.st` | **Решает T13** ([0041-02](0041-st-backend.md#разработка)): прямое отображение или откат Option C (`USINT` + константы) |
| **П5** | `ARRAY [0..3] OF USINT` в `VAR` блока FB | `iec2c probe_array.st` | Подтверждает T12 |
| **П6** | `LREAL` | `iec2c probe_lreal.st` | Подтверждает T11 |
| **П7** | `VAR_IN_OUT`; вызов FB из FB | `iec2c probe_inout.st` | **Решает О1** ([0041-03](0041-st-backend.md#разработка)) — как под-FB видят переменные корня. Самый рискованный вопрос семантики фичи |
| **П8** | `VAR_GLOBAL … AT %IX256.0` + `VAR_EXTERNAL` в FB | `iec2c probe_at.st` | Подтверждает форму [0041-05](0041-st-backend.md#разработка); решает вопрос `VAR_GLOBAL`/`VAR_EXTERNAL` |
...
1. **Гейт нельзя включить в `precheck.sh` сейчас** — он валил бы предкоммит до
   ...
   > гейта — ещё нет.** Разделены две вещи, которые пункт смешивал. В предкоммит
   ...
   >
   > Коды возврата `iec2c` пригодны для гейта — измерено: валидный ST → 0,
...
2. **0041-02 обязана** эмитить хотя бы `VAR … END_VAR` — иначе вывод остаётся
   ...
   > **недостаточен**, и как критерий приёмки 0041-02 — **недостижим**.
   ...
   > разбирается. Тело — `CASE state OF` — предмет **0041-03**, поэтому гейт
...
#### Группа 6. Валидность порождённого ST (гейт, [анализ 0041-06](0041-st-backend.md#анализ))
...
[отчёт о тестировании](0041-st-backend.md#отчёт-о-тестировании). Фиксов не заводилось: все
