...
<Определяется на стадии анализа. Предварительно: наблюдаемое поведение целей
меняется (запись бита начинает работать там, где сегодня порождается невалидный
код), поэтому раздел о битовом доступе в [`book/`](../../book/) придётся
сверить — обещает ли он запись и в каком виде.>
