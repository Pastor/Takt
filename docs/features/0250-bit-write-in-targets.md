...
- **Related issues:** [Фича 0250](0250-bit-write-in-targets.md);
  место записи — [ADR 0249](0249-assign-to-call-place.md#архитектура-adr);
  находка гейта заглушек — [ADR 0217](0217-stub-branch-gate.md#архитектура-adr);
  эталон не вправе отставать — [ADR 0248](0248-sim-builtin-functions.md#архитектура-adr);
  `[bit;N]` как упакованный вектор — [ADR 0078](0078-bit-array-semantics.md#архитектура-adr);
  «в IEC сдвигов над числами нет» — [ADR 0061](0061-fixed-point-type.md#архитектура-adr);
  «гейт доказывает компилируемость, не верность» — [ADR 0045](0045-sv-backend.md#архитектура-adr),
  [ADR 0050](0050-rust-backend.md#архитектура-adr)

### Context

Фича [0249](0249-assign-to-call-place.md#архитектура-adr) внесла запись одного бита в список
...
- **Зависит от:** **нет** (все опоры закрыты).
  - **место записи** — фича [0249](0249-assign-to-call-place.md)
  ...
  - **`[bit;N]` как упакованный скаляр** — фича
    [0078](0078-bit-array-semantics.md) (`ГОТОВО`): слой
...
- **R4. Сдвиг на нуль не эмитится.** Маска нулевого разряда есть просто `1`;
  нулевой разряд в корпусе обычен. ⚠️ Линт тут ни при чём — замер: `clippy -D
...
| A10 | Запись `SIM-017` из реестра заглушек удалена, гейт зелёный (объявлено две) | `scripts/check-stub-branches.py` |
...
- **Риск: `clippy -D warnings` отвергнет порождённый Rust.** Снижается A3 —
  прогоном настоящего `clippy-driver`, а не чтением. ⚠️ Прогон и опроверг
...
[0250-01](0250-bit-write-in-targets.md#разработка),
[0250-02](0250-bit-write-in-targets.md#разработка),
[0250-03](0250-bit-write-in-targets.md#разработка).
