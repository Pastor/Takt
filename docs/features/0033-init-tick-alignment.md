...
- **Связанные issue (анализ):** новая фича — дефект выявлен **автосверкой**
  [`0025-07`](0025-simulator-expression-eval.md#разработка) при первом же прогоне
...
- **Related issues:** [Фича 0033](0033-init-tick-alignment.md); связь с
  [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr) (сдвиг выявлен автосверкой
  `0025-07`, вынесен из объёма 0025) и с
  [ADR 0026](0026-c-root-typedef.md#архитектура-adr) (typedef корня — мешает сверять модели
  глубины 1)
...
переходы (см. [`0025-04`](0025-simulator-expression-eval.md#разработка)). Синтетических
...
- Модели глубины 1 нельзя собрать компилятором C до закрытия
  [0026](0026-c-root-typedef.md#архитектура-adr) (нет `typedef` корня) — потактовое покрытие
...
Расхождение выявлено автосверкой [`0025-07`](0025-simulator-expression-eval.md#разработка)
...
[ADR 0033](0033-init-tick-alignment.md#архитектура-adr) принята **Option B**: такт на
...
это дефект [0026](0026-c-root-typedef.md) (нет `typedef` корня для
...
- **Зависит от:** **нет**. Проверены пересечения с 0025, 0026, 0020, 0012, 0041;
  ...
  |---|---|---|---|
  | [0025](0025-simulator-expression-eval.md) Вычислитель симулятора | **ГОТОВО** | Источник находки; даёт инфраструктуру автосверки (`conformance_c_tests.rs`) и семантику «`enter` не стоит такта» (`0025-04`), к которой приводится генератор C | **Нет** — закрыта, зависимость удовлетворена |
  | [0026](0026-c-root-typedef.md) `typedef` корня | СОЗДАНА | Без него порождённый C для моделей **глубины 1** не компилируется (Ф5) → потактовое покрытие базового случая недостижимо | **Нет** — см. обоснование ниже |
  | [0020](0020-port-address-decl.md) Адрес порта | ГОТОВО | 0033 снимает инвариант «цель `c` байт-в-байт неизменна»; нужно проверить, что `c-hal` и таблица адресов не задеты | Нет — проверка в объёме 0033 |
  | [0012](0012-simulation-core.md) Ядро симуляции | ГОТОВО | Симулятор по Option B **не меняется** | Нет |
  | [0041](0041-st-backend.md) Бэкенд ST | СОЗДАНА | 0033 задаёт контракт «вход не стоит такта», который 0041 обязан соблюсти | Нет — 0033 идёт раньше и разблокирует правильную постановку 0041 |
...
- **Влияние на порядок разработки:** 0033 следует брать **раньше**
  [0041](0041-st-backend.md) (бэкенд ST): 0041 обязан
...
(фича [0029](0029-c-type-mapping.md)), и сверяться с ним значило бы
...
[ADR 0033](0033-init-tick-alignment.md#архитектура-adr); здесь — следствия принятого
решения.
...
| Задача | Содержание |
|---|---|
| [`0033-01`](0033-init-tick-alignment.md#разработка) | Генератор C: вход в стартовое состояние не расходует такт (диспетчеризация `INIT` до `switch`); пересмотр codegen-снапшотов; `c-hal` (R1–R3, R6, R7, R9) |
| [`0033-02`](0033-init-tick-alignment.md#разработка) | Потактовая автосверка: расширение `conformance_c_tests.rs`, фикстуры глубин 1–3 и композитов, негативная проверка (R4, R5, R10) |
| [`0033-03`](0033-init-tick-alignment.md#разработка) | Семантика такта в README, версия языка `0.3.0`, `CHANGES.md` (R8, правила 15, 16, 22) |
...
[0026](0026-c-root-typedef.md)). После ручной правки заголовка в
...
([`0025-04`](0025-simulator-expression-eval.md#разработка)). Требование R7, проверка T13,
...
создана задачей [`0025-07`](0025-simulator-expression-eval.md#разработка). Механизм рабочий и
переиспользуемый:
...
Причина сужения устраняется задачей [`0033-01`](0033-init-tick-alignment.md#разработка).
...
(глубина 1) больше **не** отложена: [0026](0026-c-root-typedef.md)
...
6. **Новые фикстуры** (`simulation/tests/data/eval/`): `depth3.lam` (T2),
   ...
   `depth1.lam` (T4 — **после [0026](0026-c-root-typedef.md)**).
...
8. **Комментарий модуля** (`:29–40`) — удалить раздел «Сверяются финальные
   ...
   `Rational`/`Bit`/`Array` (фича [0029](0029-c-type-mapping.md)) —
...
| Генератор C | **н/п** | Правится в [`0033-01`](0033-init-tick-alignment.md#разработка) |
...
- **Версия языка — `0.2.0`**: `README.md:1007` («**Версия языка: 0.2.0** (крейт
  ...
  [0020](0020-port-address-decl.md) (адрес порта); до неё `0.1.0` —
  фича [0021](0021-swap-assign-compare.md) (операторы `:=` / `=`).
- **Семантика такта 1 нигде не специфицирована.** Ни `README.md`, ни `docs/` не
  ...
  (`enter_initial_state`, [`0025-04`](0025-simulator-expression-eval.md#разработка)).
- Именно из-за этого пробела дефект прожил незамеченным до автосверки
  [`0025-07`](0025-simulator-expression-eval.md#разработка): расхождение двух исполнителей не с
...
[`0025-07`](0025-simulator-expression-eval.md#разработка)) **вынужденно** сравнивает
...
| T4 | Тело стартового состояния в C, глубина 1 | Плоская фикстура `depth1.lam` | Тело исполнено на **такте 1** (сегодня — такт 2). **Отложено до [0026](0026-c-root-typedef.md)**: без `typedef` корня C не компилируется (`error: must use 'struct' tag`) | R1 / A3 |
...
- **Сверка по типам** ограничена `Integer`/`Enum`/`Bool`: для `Rational`, `Bit`,
  `Array` эталон C дефектен (фича [0029](0029-c-type-mapping.md)).
...
Генератор C перестал тратить такт на синтетический `INIT` (ADR 0033, Option B):
...
**Реализация** (задачи
[0033-01](0033-init-tick-alignment.md#разработка)…[0033-03](0033-init-tick-alignment.md#разработка)):
...
- **Документация:** README — раздел «Семантика первого такта» с верифицированным
  ...
  бэкендов; `CHANGES.md` — ломающее изменение поведения.

**Живая проверка:** `cargo test --all-features` — 24 сьюта ok; cmake+ninja
собирают порождённый C; `run_simulations.sh` 5/5; детерминизм (гейт 0048) цел.
