...
### Задача 0392-01: общий носитель правила
...
теперь зовёт его же — правило одно на две цели (класс 0084/0193/0195).

⚠️ **Порог принадлежит целевому языку, а не правилу.** В Rust сдвиг на ширину
...
### Задача 0392-02: печать у цели `c`
...
⚠️ Печатник у цели `c` **один**: сдвига в `ConditionNode` не существует вовсе
(проверено грепом) — в отличие от классов 0359/0363, где правка одного
печатника чинила половину входов.

⚠️ **Переменная величина остаётся прежней** — названная граница. У цели `rust`
...
### Задача 0392-03: сторожа

- `takt-lang/tests/targets/c_shift_width_tests.rs` — текст вывода плюс прогон
  **настоящего `cc`** флагами гейта цели; контролей три: законная величина,
...
| T2 | Порождённый C под флагами гейта | собирается |
...
| Мутация | Результат |
|---|---|
| вернуть прежнюю печать | ✗ краснеют оба сторожа (сверка и текст) |
| знаковый случай печатать нулём | ✗ краснеет сверка значений и один текстовый |

⚠️ Вторая мутация даёт **валидный** C с неверным значением — её ловит только
сверка значений.
...
**Готово.** Вывод цели `c` собирается флагами её гейта, значения совпадают с
эталоном и целью `rust`; вывод корпуса не изменился.

## Итог (что сделано)

Цель `c` перестала печатать сдвиг, который её собственный гейт отвергает.
