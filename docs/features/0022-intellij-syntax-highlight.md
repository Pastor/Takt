...
- **Related issues:** [Фича 0022](0022-intellij-syntax-highlight.md);
  переиспользует токены языка из фичи [0021](0021-swap-assign-compare.md)
  (операторы `:=`/`=`/`<=`) и опционально сервер [0011](0011-lsp-server.md) `lam-lsp`

### Context

У языка Lam уже есть редакторная оснастка: LSP-сервер `lam-lsp`
([0011](0011-lsp-server.md); `textDocument/semanticTokens`,
...
- **Операторы** после фичи [0021](0021-swap-assign-compare.md):
...
[0021](0021-swap-assign-compare.md) (`:=` присваивание, `=` сравнение,
`<=` реляционный, `==` выведен).
