...
- **Related issues:** [Фича 0034](0034-sim-struct-types.md); связь с
  [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr) (ядро вычисления, инвариант
  «вся семантика — только в `eval/`»); фича [0006](0006-structs.md)
...
рецидив корневой причины фичи [0025](0025-simulator-expression-eval.md#анализ).
...
поэтому 0034 зависит от [0029](0029-c-type-mapping.md#анализ) («Генератор C: отображение
...
| Фича | Связь | Вердикт |
|---|---|---|
| [0006](0006-structs.md) «Структуры и `typedef struct`» | `ГОТОВО`; даёт `StructDefinitionNode`, `TypeNode::Struct`, C-генерацию — весь фундамент 0034 | Зависимость **удовлетворена** (закрыта) |
| [0025](0025-simulator-expression-eval.md) «Вычислитель выражений» | `ГОТОВО`; даёт `eval/`, `Value`, `coerce_to_type`, `deny(wildcard)`. 0034 закрывает её пробел T22 | Зависимость **удовлетворена** (закрыта) |
| [0032](0032-state-io-variables.md) «`--save-state` не сохраняет переменные» | `state_io` читает `Unit::variables`, а запись идёт в кэш `ModelNodeContext` — дефект **ортогонален** структурам (ломается на `u8` так же) | **Не зависимость.** Задача 0034-05 обязана не углубить дефект, но чинит его 0032 |
| [0029](0029-c-type-mapping.md) «Отображение типов в C» | Расширит сверку на поля `[T;N]`/`bit`/`float` | **Не зависимость** (см. разбор выше) |
| [0026](0026-c-root-typedef.md) «`typedef` корневой модели» | Обходится обёрткой в `model` | **Не зависимость** (см. разбор выше) |
...
|---|---|---|
| [0034-01](0034-sim-struct-types.md#разработка) | Ядро: `Value::Struct`, реестр структур, `coerce_to_type` | R1, R2 |
| [0034-02](0034-sim-struct-types.md#разработка) | Чтение поля и разрешение неоднозначности `BitAccess` | R3 |
| [0034-03](0034-sim-struct-types.md#разработка) | Запись в поле: lvalue-путь вместо SIM-017 | R4 |
| [0034-04](0034-sim-struct-types.md#разработка) | Инициализаторы структур и устранение второго вычислителя | R5, R6 |
| [0034-05](0034-sim-struct-types.md#разработка) | Наблюдаемость значений и сверка с эталоном C | R8, R9 |
...
`builder.rs::eval_expr`, см. задачу [0034-04](0034-sim-struct-types.md#разработка)).
...
- **Фича:** [0034](0034-sim-struct-types.md)
- **ADR:** [0034](0034-sim-struct-types.md#архитектура-adr) · **Анализ:** [0034](0034-sim-struct-types.md#анализ) · **Тест-план:** [0034](0034-sim-struct-types.md#тест-план)
- **Дата:** 2026-07-19
