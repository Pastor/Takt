...
- **Автомат:** состояния → `enum` + `match` (не целочисленные константы);
  контракт [ADR 0033](0033-init-tick-alignment.md#архитектура-adr) (вход в стартовое
...
1. **Зеркальная калька с C гейт не проходит.** C-бэкенд эмитит состояния `_END`
   ...
   [ADR](0050-rust-backend.md#архитектура-adr) (задача 0050-02).
...
[0050-02](0050-rust-backend.md#разработка).
...
- **Related issues:** [Фича 0050](0050-rust-backend.md); наследует
  контракт такта из [ADR 0033](0033-init-tick-alignment.md#архитектура-adr) (вход в стартовое
  состояние не расходует такт) и принцип «громкий отказ вместо тихого пропуска»
  из [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr)/[ADR 0028](0028-c-generator-stubs.md#архитектура-adr);
  повторяет структуру решения соседних бэкендов [0041](0041-st-backend.md#архитектура-adr) (ST) и
  [0045](0045-sv-backend.md#архитектура-adr) (SV).
...
|---|---|
| Для модели без под-моделей typedef корня не эмитится → порождённый C **не компилируется** (8 ошибок `cc`) | фича [0026](0026-c-root-typedef.md) |
| `Array(size, elem)` → `uint{size}_t`, где `size` — **число элементов**: `[u8; 4]` → `uint4_t` (типа не существует) | `generator/c/mod.rs:150`, фича [0029](0029-c-type-mapping.md) |
...
| Снимок достижимых состояний/моделей (`minimap::Map`, `BTreeMap` → детерминизм) | `semantic/minimap.rs`, фича [0048](0048-deterministic-codegen.md) |
| Образец обхода переходов | `generator/c/c_model.rs:430` |
| Контракт такта (INIT не расходует такт) | [ADR 0033](0033-init-tick-alignment.md#архитектура-adr) |
...
| Такт | `Init` диспетчеризуется **до** `match` | контракт [ADR 0033](0033-init-tick-alignment.md#архитектура-adr) — сдвиг тактов 0 |
...
7. Контракт [0033](0033-init-tick-alignment.md#архитектура-adr): вход в стартовое состояние не
...
открытые дефекты отображения ([0026](0026-c-root-typedef.md) —
порождённый C не компилируется для модели без под-моделей;
[0029](0029-c-type-mapping.md) — `[u8;4]` → `uint4_t`, `bit` → `int`,
...
- **Зависит от:** `нет`. Проверены кандидаты в зависимости:
  ...
  | Фича | Проверка | Вывод |
  |---|---|---|
  | [0026](0026-c-root-typedef.md), [0029](0029-c-type-mapping.md) (дефекты `c`) | правки лежат в `generator/c/` | **не зависит**: у цели `rust` свой модуль; дефекты — мотивация, а не зависимость |
  | [0033](0033-init-tick-alignment.md) (INIT-такт) | `ГОТОВО`; задаёт **контракт** для будущих бэкендов | **не зависит** (контракт исполняем, не блокирует) |
  | [0048](0048-deterministic-codegen.md) (детерминизм) | `ГОТОВО`; `minimap::Map` и словари — `BTreeMap` | **не зависит**; детерминизм цели получается даром |
  | [0020](0020-port-address-decl.md) (адреса портов) | карта не потребляется (порты через HAL) | **не зависит**; MMIO-режим — кандидат |
  | [0041](0041-st-backend.md) (ST), [0045](0045-sv-backend.md) (SV) | оба `РАЗРАБОТКА`; общий слой бэкендов **не заводится** (решение из `FEATURES.md`) | **не зависит**; работы независимы |
  | [0027](0027-module-size-split.md) (размер модулей) | новый модуль пишется сразу дробно (≤ ~1000 строк) | **не зависит** |
...
- **R7. Автомат.** Состояния → `enum` + `match`; переходы — по образцу
  `c_model.rs:430`. Контракт [0033](0033-init-tick-alignment.md#архитектура-adr): вход в
...
|---|---|---|
| [0050-01](0050-rust-backend.md#разработка) | Каркас: `Language::Rust`, `generator/rust/`, `RustMap`, `-t rust`, `compile_to_rust` | R1 |
| [0050-02](0050-rust-backend.md#разработка) | Гейт `rustc` + `clippy`; **решение по политике линтов** | R8, R9 |
| [0050-03](0050-rust-backend.md#разработка) | Отображение типов + диагностики; `repr` по диапазону | R3, R4 |
| [0050-04](0050-rust-backend.md#разработка) | Имена, raw-идентификаторы, `RS-004` на `Self` | R5 |
| [0050-05](0050-rust-backend.md#разработка) | `struct` модели, HAL-трейт, `in`/`out`/`inout` | R6 |
| [0050-06](0050-rust-backend.md#разработка) | Автомат: `enum`+`match`, контракт 0033, композиция | R7 |
| [0050-07](0050-rust-backend.md#разработка) | Выражения, условия, `fn`, `extern fn`, `debug` | R2, R12 |
| [0050-08](0050-rust-backend.md#разработка) | Примеры, контрпримеры, документация | R10, R11 |
...
5. Каркас эмитит **минимальный компилируемый** модуль (шапка `#![no_std]` +
   ...
   (наследие [ADR 0028](0028-c-generator-stubs.md#архитектура-adr): проглоченная ошибка +
...
зафиксированных дефекта** (фича [0029](0029-c-type-mapping.md)):
...
- Тест: модель **без** портов не эмитит ни трейта, ни параметра типа `H`.
  ...
  [0050-02](0050-rust-backend.md#разработка)): гейт диктует форму эмиссии — калька с C не годится.
...
- **Контракт такта** — [ADR 0033](0033-init-tick-alignment.md#архитектура-adr), версия
...
4. **Ограничение R9 действует и здесь.** Проба: варианты `End`/`Tick`, которые
   ...
   состояний обязана следовать решению [0050-02](0050-rust-backend.md#разработка) — эта задача
...
> **Реализовано.** (2026-07-16). План по ADR 0050 (правила 15, 16).
...
| T3 | Гейт `clippy` | ✅ | `-D warnings`, код 0. `comprehensive` пропускается **громко** — дефект самого примера (`temperature <= 0` при `u8`), принадлежит фиче [0030](0030-comprehensive-example-fix.md) |
...
`Idle = 670` — см. отчёт [0045](0045-sv-backend.md#отчёт-о-тестировании)). Известное ограничение
...
- Цель транслирует **весь корпус** (5 из 5 — больше любой другой, кроме
  `c`/`plantuml`). Гейт `rustc` + `clippy` под `-D warnings`; `unsafe`
...
- **`extern fn` → методы трейта** (`elevator.rs` несёт 8), `debug` → метод трейта
  (решение (а) задачи 0050-07). То есть цель заметно мягче `sv`, где оба
...
- **Разработка «Реализовано» ≠ фича проверена.** Все восемь задач были помечены
  выполненными, гейт зелёный — а сверки поведения не существовало. Пробел нашла
  **приёмка**, и нашла потому, что фича 0045 сформулировала правило: гейт
