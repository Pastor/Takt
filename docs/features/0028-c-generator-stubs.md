# Фича 0028: Заглушки генератора C: диагностика вместо тихого пропуска

> ⚠️ **КОД `CC-014` ЗАНЯТ ДРУГОЙ ФИЧЕЙ — ADR требует правки** (2026-07-15).
> ADR 0028 (**Accepted**) и задача `0028-01` называют код `CC-014`, но его заняла
> реализованная фича [0029](0029-c-type-mapping.md) («бит-вектор непредставимой
> ширины»). Обе фичи выбирали код **грепом свободного** в разное время и друг
> друга не увидели; 0029 идёт первой в очереди и уже в коде. Заняты
> `CC-014`…`CC-017` — **0028 при реализации берёт `CC-018`**. Перед началом
> работ: поправить ADR 0028, задачу `0028-01` и тест-план. Системная причина —
> отсутствие реестра кодов диагностик (кандидат в `FEATURES.md`).

- **Номер:** 0028
- **Статус:** **РАЗРАБОТКА** (стадии 2–3 пройдены: ADR — Accepted, анализ —
  готов, зависимости проставлены, декомпозиция `0028-01…03` заведена)
- **Зависит от:** **нет** — проставлено аналитиком на стадии 3 (правило 17).
  Проверены пересечения с 0027 (делит `c_expr.rs`), 0029 (правит
  `generator/c/`), 0026, 0025, 0020 — жёстких зависимостей нет,
  `ЗАБЛОКИРОВАНА` не ставится. Обоснование — в
  [анализе](../analyze/0028-c-generator-stubs.md), раздел «Зависимости фичи».
  **Рекомендация по порядку (правило 19):** брать **раньше 0027 и 0029** — обе
  правят `generator/c/`, и при обратном порядке малые правки 0028 придётся
  переносить на разделённые модули.
- **Связанные issue (анализ):** новая фича (кандидат из витрины `FEATURES.md`,
  выявлен при обзоре 2026-07-14)
- **Крейт:** `grammar` (затрагивает `generator/c/c_model.rs`, `generator/c/c_expr.rs`)
- **Приоритет:** **высокий (Tier 1 — корректность кодогенерации)**. Не высший:
  дефект **латентен** — ни один файл `examples/` его не задевает (проверено
  прогоном корпуса), в отличие от [0025](0025-simulator-expression-eval.md), где
  заявленная функциональность крейта не работала в основном. Но класс дефекта —
  тот же, а последствия тяжелее: компилятор молча выдаёт **мёртвый автомат** с
  кодом возврата `0`, и порождённый C собирается без замечаний. Для языка синтеза
  автоматов промышленных систем управления (правило 12) это худший режим отказа —
  он обнаруживается на объекте, а не в CI. Объём при этом мал (две локальные
  правки в одном файле), то есть отношение «цена/риск» — в пользу немедленного
  исполнения.

## Ссылки на артефакты (жизненный цикл, правило 17)

| Стадия | Артефакт |
|---|---|
| Архитектура (ADR) | [`docs/adr/0028-c-generator-stubs.md`](../adr/0028-c-generator-stubs.md) — **Accepted** (Option B: ошибка `CC-014` для достижимой заглушки + удаление недостижимой) |
| Анализ | [`docs/analyze/0028-c-generator-stubs.md`](../analyze/0028-c-generator-stubs.md) — **готов** (вердикт о достижимости с пробами, зависимости, R1–R10, A1–A12, декомпозиция `0028-01…03`) |
| Разработка | [`0028-01`](../development/0028-01-c-generator-stubs.md) диагностика `CC-014` · [`0028-02`](../development/0028-02-dead-branch-cleanup.md) чистка мёртвой ветки · [`0028-03`](../development/0028-03-fixtures-tests.md) фикстуры и тесты — **разработка не начата** |
| Тест-план | [`docs/tests/0028-c-generator-stubs.md`](../tests/0028-c-generator-stubs.md) — **готов** (T1–T18, пример и контрпример, правило 16) |
| Отчёт о тестировании | [`docs/reports/`](../reports/README.md) — при переходе к стадии 6 |
| Исправления | [`docs/fixes/`](../fixes/README.md) (при необходимости `0028-YY-*`) |

## Краткое описание

Генератор C печатает заглушки **в порождаемый C-код** вместо того, чтобы
сообщать о проблеме. Анализ установил, что заявленные в кандидате две заглушки
имеют **противоположную природу**:

- **`//TODO: условный переход … не поддерживается`** (`c_model.rs:316`) —
  **достижима** (проба воспроизводит): непереводимое условие приводит к тому,
  что переход **не генерируется**, `Err` **проглатывается**, а `lamc` печатает
  «Скомпилировано» и завершается с кодом `0`. На выходе — **мёртвый автомат**,
  который собирается C-компилятором без замечаний. Это тот же класс «тихого
  пропуска», что и фича [0025](0025-simulator-expression-eval.md).
- **`//FIXME: Пока не реализовано`** (`c_model.rs:769`) — **недостижима**
  (статическое доказательство): `state_at` фильтрует элементы по `is_state()`,
  а `Element` имеет ровно три варианта, два из которых уже покрыты цепочкой
  `if let`. Это **мёртвый код**, маскирующийся под незавершённую работу.

Фича заменяет тихий пропуск на жёсткую ошибку компиляции `CC-014` (с причиной и
позицией `ref` в исходнике), удаляет мёртвую ветку и закрепляет её
недостижимость исчерпывающим `match` — чтобы новый вариант `Element`/`StateExtend`
валил сборку, а не выпадал в молчаливый no-op.

> Фича зарегистрирована из бэклога кандидатов `FEATURES.md`; далее проходит
> жизненный цикл по правилу 17.

<!-- При ЗАКРЫТИИ фичи (стадия 8, статус ГОТОВО) сюда добавляется раздел
     «## Итог (что сделано)» —
     ссылки на отчёт/фиксы (правило 21). Незакрытые фичи «Итога» не имеют. -->
