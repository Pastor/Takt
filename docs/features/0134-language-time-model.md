...
- ⚠️ **Долг задачи 0134-04:** снять временную ветку в гейте примеров документа
  (`precheck.sh`): пример раздела «Механизмы времени» компилируется целью
  `plantuml`, потому что цель `c` время ещё не поддерживает (см.
  [0141](0141-book-time-section.md)).
