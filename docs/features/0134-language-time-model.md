...
нет. Принято в [ADR 0134](0134-language-time-model.md#архитектура-adr):
...
**Требуется, в большом объёме.** Анализ ([0134-08](0134-language-time-model.md#анализ))
...
- **Related issues:** [Фича 0134](0134-language-time-model.md); опирается на
  [ADR 0127](0127-int-overflow-semantics.md#архитектура-adr) (обёртка беззнакового определена),
  [ADR 0096](0096-fixed-point-native-float.md#архитектура-adr) (прецедент осознанного исключения из
  принципа «CLI не меняет логику»), [ADR 0061](0061-fixed-point-type.md#архитектура-adr) (прецедент
...
7. **Обёртка беззнакового определена** ([ADR 0127](0127-int-overflow-semantics.md#архитектура-adr)),
...
[ADR 0127](0127-int-overflow-semantics.md#архитектура-adr). Так переполнение счётчика не ломает
...
[фичи 0096](0096-fixed-point-native-float.md)), и должен быть **явно
...
⚠️ **Умножение и деление длительности не реализованы** (`duration × целое`,
...
Корректность при переполнении счётчика держится на [ADR 0127](0127-int-overflow-semantics.md#архитектура-adr):
...
[ADR 0127](0127-int-overflow-semantics.md#архитектура-adr) — то есть здесь язык, эталон и
...
| `eval/` симулятора | см. [0134-03](0134-language-time-model.md#анализ) | тот же `deny` |
...
⚠️ **Отдельный раздел заведён** (уточнение заказчика 2026-07-28): «Механизмы
времени» — `book/src/12-time/`, фича [0141](0141-book-time-section.md),
...
- **Зависит от:** [0127](0127-int-overflow-semantics.md) — **закрыта
...
|---|---|---|
| [0134-01](0134-language-time-model.md#анализ) | Лексика, грамматика, АСД: литерал, `clock`, `after`/`every` | R2, R5, R12 |
| [0134-02](0134-language-time-model.md#анализ) | Тип `duration`, слой `semantic::duration`, диагностики | R3, R4, R7, R8 |
| [0134-03](0134-language-time-model.md#анализ) | Симулятор: виртуальные часы, `eval`, сценарий, трасса | R6, R9, R12 |
| [0134-04](0134-language-time-model.md#анализ) | Цели `c` / `c-hal`: колбэк, счётчики, дефолтный HAL | R1, R6, R8, R9 |
| [0134-05](0134-language-time-model.md#анализ) | Цель `rust`: метод трейта `Hal`, обёртка, линты | R1, R6, R9 |
| [0134-06](0134-language-time-model.md#анализ) | Цели `st` / `st-at`: штатный `TON` | R1, R6, R9 |
| [0134-07](0134-language-time-model.md#анализ) | Цели `sv` / `sv-mmio`: вход `time_ms`, профиль тактов | R1, R6, R7, R9 |
| [0134-08](0134-language-time-model.md#анализ) | CLI и печать, сопровождающие слои, `book/`, свод | R10, R11, A11, A12 |
...
После [0134-01](0134-language-time-model.md#разработка) литерал времени **разбирался**, но
...
После [0134-02](0134-language-time-model.md#разработка) тип `duration` существовал, но
...
- **Сравнение разностью** `(uintN)((uintN)now - t0) >= D_MS` (R8): беззнаковая
  обёртка нормирована [ADR 0127](0127-int-overflow-semantics.md#архитектура-adr), поэтому
...
[0134-08](0134-language-time-model.md#анализ) (действие A-2, прецедент 0043).
