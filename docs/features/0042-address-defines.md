...
- **Related issues:** [Фича 0042](0042-address-defines.md); развивает
  [ADR 0020](0020-port-address-decl.md#архитектура-adr) (слой `AddressMap`, приоритет источников);
  пересекается с [ADR 0041](0041-st-backend.md#архитектура-adr) и [ADR 0043](0043-address-map-export.md#архитектура-adr)
...
правится. Фича вынесена из [0020](0020-port-address-decl.md) (ADR 0020,
Action items: «`-D`/инъекция define’ов … выносятся в **отдельные фичи**»).

По [ADR 0042](0042-address-defines.md#архитектура-adr) принято: define'ы **ортогональны**
...
- **Зависит от:** **нет**.
  ...
  **Обоснование.** Единственная содержательная зависимость — ядро подсистемы
  адресов фичи [0020](0020-port-address-decl.md) (слой `AddressMap`,
...
- **Влияние на порядок разработки (правило 19).** 0042 формально не разблокирует
  ...
  |---|---|---|---|
  | [0041](0041-st-backend.md) «Бэкенд генерации в Structured Text» | `СОЗДАНА` | Второй потребитель `AddressMap`: `address → AT %IX…` (ADR 0020, «Потребители»). Читает `AddressResolution.map`, т.е. **результат** вычисления адреса | Нет по смыслу; **есть по коду** — обе фичи трогают `resolve_addresses` (0042 меняет сигнатуру, добавляя `env`) |
  | [0043](0043-address-map-export.md) «Экспорт карты адресов во внешний формат» | `СОЗДАНА` | Третий потребитель: `lamc address-map --emit …` выгружает `AddressMap` наружу | Нет по смыслу; **есть по коду** — тот же модуль `address_map.rs` |
...
**Предшественник:** [0042-01](0042-address-defines.md#разработка) (вычислитель выражений
...
**Разрешение имён в выражении адреса отсутствует** — см. [0042-01](0042-address-defines.md#разработка),
...
**Предшественники:** [0042-01](0042-address-defines.md#разработка) (вычислитель),
[0042-02](0042-address-defines.md#разработка) (`AddressEnv`). Эта задача — проводка наружу и
...
[0042-01](0042-address-defines.md#разработка)).
