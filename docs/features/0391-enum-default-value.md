...
- **Related issues:** [Фича 0391](0391-enum-default-value.md);
  замер вырос из [0353](0353-c-default-init.md#архитектура-adr) (умолчание у цели `c`),
  соседняя механика — [0379](0379-sv-enum-reset.md#архитектура-adr) (сброс перечислимого
  регистра у `sv`)
...
- **Зависит от:** нет. Опорные фичи закрыты: [0353](0353-c-default-init.md)
  (умолчание у `c`), [0379](0379-sv-enum-reset.md) (сброс `sv`),
  [0338](0338-enum-in-function.md) (перечисление у `st`).
- **Блокирует:** ничего.
...
### Задача 0391-01: носитель правила
...
⚠️ «Первый» — **по тексту**, а не по значению: порядок объявления в языке
значим (0034), и при `enum E { B = 2, A = 1 }` ответы различаются. Автор видит
текст, а не таблицу значений.

### Задача 0391-02: четыре потребителя
...
### Задача 0391-03: сторожа и документ
...
- Тесты 0379 (`conformance_sv_enum_reset_tests`) **переписаны**: они
...
Не найдено. Два теста фичи 0379 закрепляли прежнее правило и переписаны.
...
⚠️ Правило записано в `book/` (раздел «Типы»): умолчание — наблюдаемое
