...
- **Зависит от:** **нет** жёстких зависимостей (проверено аналитиком 2026-07-17 — см. [анализ](0061-fixed-point-type.md#анализ)). **Очерёдность:** [0060](0060-enum-width-shared-layer.md) идёт **первой** (Tier 2 против Tier 3) — обе правят `*_type.rs` всех целей, и после 0060 у 0061 появляется образец «факт в общем слое → отображение в цели».
...
- **Связанные issue (анализ):** новая фича (перевод кандидата из `FEATURES.md`); закрывает отложенный **Option R-B** [ADR 0045](0045-sv-backend.md#архитектура-adr)
...
- **Related issues:** [Фича 0061](0061-fixed-point-type.md); исполняет отложенное решение [ADR 0045](0045-sv-backend.md#архитектура-adr) (вопрос 4, **Option R-B вынесен в кандидаты**)
...
`--float-width=64` (фича [0029](0029-c-type-mapping.md#архитектура-adr)); симулятор тоже f64.
...
3. **Округление и переполнение — часть формата, а не деталь реализации.** Если их
   ...
   доказанный при [0060](0060-enum-width-shared-layer.md#архитектура-adr): пустое перечисление
...
- **Зависит от:** **нет** (жёстких блокировок).
  - **Код.** Фича трогает язык (`lexer`/`grammar.lalrpop`/`ast`/`type_node`),
    ...
    [0060](0060-enum-width-shared-layer.md) — по `*_type.rs` **всех**
  ...
  - **Смежность.** `/` над `q` синтезируется в аппаратный делитель — то же, о чём
    кандидат `SV-009` ([0064](0064-sv-divider-warning.md)). Не
...
- **R-1. Тихое расхождение арифметики** (главный риск; ровно то, из-за чего 0045
  ...
  из [0060](0060-enum-width-shared-layer.md) — факт в общий слой,
  отображение в цель.
...
Язык не выражает формат fixed-point — ровно причина, по которой [ADR 0045](0045-sv-backend.md#архитектура-adr)
...
- ⚠️ `/` синтезируется в **аппаратный делитель** — крупный и медленный блок;
  предупреждение об этом — объём [0064](0064-sv-divider-warning.md)
...
1. **Реальная дробная модель** (регулятор — типовой потребитель fixed-point на
   ...
   **Зачем именно пример, а не фикстура:** фикстуры пишет разработчик — они
   ...
   дефекты [0026](0026-c-root-typedef.md) и
...
| T16 | **Пример-регулятор** ✅ | Реальная дробная модель | `examples/regulator.lam` проходит `examples_scenario_tests` (Adjust→Settled→Done) и **все** гейты корпуса (`c`/`st`/`rust`/`sv`) | — / A9 |
...
- **Предупреждение о делителе (`SV-009`)** — `/` над `q` синтезируется в аппаратный
  делитель, но предупреждение — объём [0064](0064-sv-divider-warning.md).
