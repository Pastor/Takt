...
`0531-09f-2`, `0531-09f-3`, `0531-09g`, `0531-09h`, `0531-09i` и `0531-10a`
(разделы
...
| 16 | `0531-09p` | **цель и ключи сборки в метаданных проекта и в архиве** (поручение заказчика 2026-09-05) — **в проработке**, см. раздел «Часть 09p» | 09b, 09g | круговой рейс архива несёт цель и ключи; чужой проект открывается его сборкой, а не сборкой читателя |
| 15 | `0531-09n` | **файлы Markdown и файлы прогонов в проекте** (поручение заказчика 2026-09-05) — **в проработке**, см. раздел «Часть 09n» | 09b, 09g | по разделу «Часть 09n» |
...
⚠️ **Раздел — проработка.** Ниже названо, что уже есть, что придётся решить, и
чем это отличается от черновика, который такие величины уже хранит.
...
| Задача | Предмет |
|---|---|
| `09p-1` | колонки цели и ключей у проекта, API, правило «проект задаёт, черновик перекрывает» |
...
⚠️ **Раздел — ПРОРАБОТКА, не план работ.** Ниже названо, что придётся решить,
и где у проекта уже есть правила, которые новое обязано не нарушить. Задачи
`09n-1`, `09n-2`… заводятся после ответов заказчика.
...
##### Кандидаты в задачи (после ответов)

| Задача | Предмет |
|---|---|
| `09n-1` | род файла в базе и в API, пределы, архив с поднятой версией формата |
| `09n-2` | страница: список файлов с родами, вторая область, переключение |
| `09n-3` | подсветка Markdown (свой разбор, как у JSON сценария) |
| `09n-4` | просмотрщик рендера — с решённым вопросом безопасности |
| `09n-5` | файлы прогонов: хранение, выбор активного, связь со вкладкой «Прогон» |
