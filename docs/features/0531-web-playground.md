...
##### Задача `0531-09m` — ключи сборки выбором, а не строкой (ОТКРЫТА)
...
1. **Откуда список.** Модуль пока не отдаёт описи ключей; нужна ручка вроде
   `takt_flags` — либо признать список частью веба и завести сторож сверки.
2. **Ручной ввод.** Оставлять ли поле строкой рядом с выбором (для ключей,
   которых UI не знает) — и что сильнее при расхождении.
3. **Что попадает в черновик и в ссылку.** Сейчас в них едет строка; при
   выборе состав ключей — это состояние, и его форма меняется.
