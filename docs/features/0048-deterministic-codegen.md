...
- **Зависит от:** `нет` (проставлено аналитиком, стадия 3, 2026-07-16).
  Пересечения по файлам — не зависимости: [0027](0027-module-size-split.md)
  трогает `c_expr.rs` (0048 его не касается) и `validate.rs`/`lsp.rs` (только в
  опциональном объёме «гигиена»), [0026](0026-c-root-typedef.md) закрыта.
...
- **Related issues:** [Фича 0048](0048-deterministic-codegen.md)
...
[0041](0041-st-backend.md#архитектура-adr)) сортирует у себя в **13** местах и потому
...
- Пересечение по файлам с [0026](0026-c-root-typedef.md#архитектура-adr) (закрыта) и
  [0027](0027-module-size-split.md#архитектура-adr) (`c_expr.rs`) — конфликты слияния вероятны.
...
([ADR 0048](0048-deterministic-codegen.md#архитектура-adr), Option A — решение заказчика
...
- **Пересечения по файлам — не зависимости** (правило 19, критерий 1 неприменим):
  [0027](0027-module-size-split.md) перекладывает `c_expr.rs`
  ...
  [0026](0026-c-root-typedef.md) закрыта. Конфликт слияния возможен
...
- **Влияние на порядок разработки:** завершение 0048 **никого не
  ...
  неисполнимым для [0045](0045-sv-backend.md). Положение 0048 в
...
| Задача | Содержание |
|---|---|
| [0048-01](0048-deterministic-codegen.md#разработка) | Упорядочить общий слой: `ModelNode` (11 полей), `minimap::Map.elements`, `Ord` для `Name`, снять `with_capacity` |
| [0048-02](0048-deterministic-codegen.md#разработка) | Детерминировать `topological_sort_models` цели `c`; исправить докстринг-заблуждение |
| [0048-03](0048-deterministic-codegen.md#разработка) | Гейт воспроизводимости в `precheck.sh` + тесты-сторожа (`plantuml`, диагностики) |
| [0048-04](0048-deterministic-codegen.md#разработка) | Перегенерировать `examples/generated/`, обновить `CLAUDE.md`/`README.md`/`CHANGES.md` |
...
построению**, независимо от [0048-01](0048-deterministic-codegen.md#разработка):
...
`st-at`). Порядок эмиссии перенесён с привычки бэкенда на **тип контейнера**
общего слоя.

**Реализация** (крейт `grammar`, задачи
[0048-01](0048-deterministic-codegen.md#разработка)…[0048-04](0048-deterministic-codegen.md#разработка)):

- `ModelNode` (11 полей-словарей) и `minimap::Map.elements` — `HashMap` →
  `BTreeMap` (алфавитный порядок; ADR 0048, Option A — решение заказчика). Ключ
...
- **Жёсткий гейт воспроизводимости** в `precheck.sh` (`diff -r` двух прогонов) +
  тесты-сторожа (`plantuml/mod.rs`, `codegen_tests.rs`, `semantic_tests.rs`).
...
собирают порождённый C; негативная проба подтвердила, что гейт ловит регрессию.
Побочный выигрыш — детерминирован порядок диагностик и автодополнения LSP.
