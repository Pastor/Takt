...
- **Связанные issue (анализ):** новая фича (из блока «Кандидаты» `FEATURES.md`:
  ...
  [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr), Option C)
...
> [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr)); далее проходит жизненный
> цикл по правилу 17.

## Архитектура (ADR)

> ## ⚠️ Пересмотр (2026-07-19): драйвер 1 (платформа) изменён
>
...
> лишь минимальная линейка/JDK. Открытый `until-build` (фикс 0023-01) сохранён.
>
...
- **Related issues:** [Фича 0038](0038-intellij-semantic-tokens.md);
  реализует **отложенный Option C** из [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr);
  наследует ограничения плагина из [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr);
  потребляет `lam-lsp` из фичи [0011](0011-lsp-server.md)

### Context

Плагин `extensions/intellij-lam` (фичи [0022](0022-intellij-syntax-highlight.md),
[0023](0023-intellij-navigation-include.md), обе `ГОТОВО`, версия
...
1. **Редакция IDE.** Плагин собирается под `platformType = IC`
   ...
   ещё в [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr) (Option C, Cons) и
   [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr) (Option C, Cons).
...
4. **Общая инфраструктура с 0039.** Фича [0039](0039-intellij-reformat.md)
...
[0040](0040-intellij-psi-parser.md)), классифицирующий имена
...
[ADR 0038](0038-intellij-semantic-tokens.md#архитектура-adr): LSP4IJ + готовый
...
- **Зависит от:** **нет**. Обе предпосылки закрыты (`ГОТОВО`):
  - [0011](0011-lsp-server.md) — `lam-lsp` с реализованными semantic
    tokens (см. таблицу выше). Не «в планах», а рабочий код.
  - [0022](0022-intellij-syntax-highlight.md) — плагин, поверх
    которого строится надстройка (`LamHighlighterColors`, `LamFileType`,
    Gradle-сборка). Также используется плоский PSI из
    [0023](0023-intellij-navigation-include.md) (`ГОТОВО`), но
...
- **Вердикт по связи с [0039](0039-intellij-reformat.md) (Reformat
...
- **Вердикт по связи с [0040](0040-intellij-psi-parser.md)
...
делает LSP4IJ-путь выгодным и для фичи [0039](0039-intellij-reformat.md).
...
⚠️ **Пересмотр драйвера 1 ADR (см. врезку в [ADR 0038](0038-intellij-semantic-tokens.md#архитектура-adr)).**
...
| Крейт `grammar` | **н/п** — сервер уже реализует semantic tokens; тесты — задача [0038-03](0038-intellij-semantic-tokens.md#разработка) |
| Крейт `simulation` | **н/п** — не затрагивается |
...
[0038-01](0038-intellij-semantic-tokens.md#разработка).
...
| Крейт `grammar` | **н/п** — легенда уже есть; тесты — [0038-03](0038-intellij-semantic-tokens.md#разработка) |
...
| `grammar`: `semantic_tokens` (продуктивный код) | T1–T7 | ⚠️ **правки не только в тестах**: зонд вскрыл дефект классификации под-моделей → фикс [0038-01](../fixes/0038-01-subtree-token-classification.md) | ✅ (0038-03 + фикс 0038-01) |
...
- `textDocument/formatting` в IntelliJ — предмет фичи
  [0039](0039-intellij-reformat.md).
...
[0039](0039-intellij-reformat.md) в LSP4IJ-варианте (`textDocument/formatting`
сервер уже отдаёт) и снимает блок с [0067](0067-intellij-rename-psi-import.md).
...
- **Сервер был готов не полностью** — [фикс 0038-01](../fixes/0038-01-subtree-token-classification.md)
...
сервера), 0038-02 (маппинг 10 типов легенды → цвета + сторож синхронизации),
...
- **Остатки** (документированы, не блокируют): A11 (цвета в редакторе — визуально
  ...
  кандидат-фикс (тихая деградация уже работает).
- **Разблокирует:** [0039](0039-intellij-reformat.md) в LSP4IJ-варианте
  (`textDocument/formatting` сервер отдаёт) и [0067](0067-intellij-rename-psi-import.md).
- **Уроки:** *«сервер готов» — проверяй зондом, а не по документам* (класс дефекта
  0025: зелёные тесты «не паникует», результат не сверялся); *внешняя зависимость
