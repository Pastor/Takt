...
⚠️ **Пересъём ОПРОВЕРГ запись кандидата дважды.** Она утверждала, что `st` и
...
⚠️ **Tier 1** остаётся: эталон и цели расходятся **значением**, и у эталона
...
⚠️ Контроль опроверг и **третье** утверждение записи — «в теле та же запись
...
(`takt-sim/src/eval/ops.rs::power`), цели переводят (фичи 0328, 0329). Не знает
...
1. в таблицу добавляется знак `"**"`: счёт `wrapping_pow` в `i128` — та же
   обёртка, которой живёт вся таблица (норма ADR 0127);
2. в обход сырого АСД добавляется `E::Power(loc, l, r) => binary("**", …)` —
   единственная точка арифметики модуля.

⚠️ **Второго знания о степени не заводится.** Семантика задана фичей 0328
...
⚠️ **Показатель за пределами `u32` — отказ, а не обрезание.** Отрицательная
...
- **Свернуть степень в теле** (общая свёртка выражений). Правило 0192
  сворачивает **инициализаторы**, и расширение его на тела — другая фича с
  другой ценой (проверки идут после понижения, урок 0383).
...
⚠️ **Язык адресов не меняется**: матчер выражения адреса (`address_map/eval.rs`)
...
вместо молчаливого нуля. Версия языка **0.13.0** поднята фичей 0404 в текущем
...
`Power` существуют с фичи 0328, автодополнение и подсветка их уже знают. Новых
...
| T3 | Контроль: `const K := 2 ** 3;` в **выведенном** типе | значение `8`, тип по правилу 0285 |
...
| T8 | Гейты предкоммита (корпус, детерминизм, инструменты целей) | зелены |
...
### Задача 0407-01: степень в таблице операций
...
### Задача 0407-02: ветвь обхода и переводы ошибки
...
- `takt-lang/tests/semantic/const_power_fold_tests.rs` — вывод целей `c`, `st`,
  ...
  сторож **класса** — тождественность вывода выводу для готового литерала
  (приём 0143: за границей семантики степени не существует).
...
знания о семантике степени не заведено — она задана фичей 0328 и живёт там же,
...
⚠️ **Соседний класс, вынесенный кандидатом.** Контрольный вход показал, что
...
корпус его не покрывает (степени в `examples/` нет), поэтому гейт цели молчит.
