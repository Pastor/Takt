...
**Гибрид по флагу генерации** (Option E [ADR 0185](0185-model-parameters.md#архитектура-adr),
...
- **Related issues:** [Фича 0185](0185-model-parameters.md); снимает ограничение О8 [анализа 0182](0182-pid-library-and-application.md#анализ); опирается на [ADR 0026](0026-c-root-typedef.md#архитектура-adr) (структуры цели `c`), [ADR 0045](0045-sv-backend.md#архитектура-adr) (уплощение композиции), [ADR 0184](0184-imported-shared-variables.md#архитектура-adr) (усыновление импортированного поддерева)
...
- **Влияние на порядок разработки:** ничего не блокирует; **улучшает**
  результат закрытой [0182](0182-pid-library-and-application.md) —
...
- **Смежные, но не зависимости:**
  - [0170](0170-fixed-point-saturation.md) — насыщение для `q`:
...
- **О9. Константный вычислитель — второй интерпретатор.** Семантика вычислений
  ...
  ([0042](0042-address-defines.md), `address_map/eval.rs` — имена +
  целочисленная арифметика) и выдержки `after`
  ([0143](0143-after-const-duration.md),
...
узких вычислителя: адресные выражения ([0042](0042-address-defines.md)
— имена и целочисленная арифметика) и константные выдержки `after`
([0143](0143-after-const-duration.md) — длительности). Вызов функции
...
Пример фичи [0182](0182-pid-library-and-application.md) —
