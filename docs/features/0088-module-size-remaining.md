...
- **Related issues:** [Фича 0088](0088-module-size-remaining.md);
  продолжение [ADR 0027](0027-module-size-split.md#архитектура-adr) (правило размера + храповик
  `check-module-size.sh`); образцы разделения — `semantic/validate/`, `lsp/`,
  `generator/c/c_expr/`, `semantic/index/`, `address_map/` (ADR 0069).

### Context

Фича [0027](0027-module-size-split.md#архитектура-adr) взяла в объём **три** худших модуля и
**заморозила** остальные храповиком `scripts/check-module-size.sh` (реестр
...
нарушителей лимита размера модуля (решение заказчика, [ADR 0088](0088-module-size-remaining.md#архитектура-adr),
...
| T7 | Заведена фича-преемник по 6 оставшимся файлам | закрытие 0088 | карточка [0099](0099-module-size-core.md), статус `СОЗДАНА` | A5 |
...
| T7 | Заведена фича-преемник | ✅ | [0099](0099-module-size-core.md), `СОЗДАНА` (6 файлов) |
...
вынесен в **фичу-преемник [0099](0099-module-size-core.md)** (критерий
