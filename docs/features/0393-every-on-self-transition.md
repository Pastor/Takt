...
⚠️ **Класс ШИРЕ, чем описывала карточка: расходился не только `every`, но и
...
### Задача 0393-01: признак входа — смена состояния
...
⚠️ `enter`/`exit` при самопереходе исполняются **по-прежнему**: это разные
вопросы, и разница названа в документе (`book/src/12-time/`).

### Задача 0393-02: документ
...
⚠️ Время в тесте ведёт **прогон** (`set_time_ns`), как в сверке 0134-09:
...
⚠️ `enter`/`exit` при самопереходе исполняются по-прежнему — «вход» для них и
