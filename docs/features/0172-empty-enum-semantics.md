...
**Сделано** (задача [0172-02](0172-empty-enum-semantics.md#разработка)):
...
- **Date:** 2026-08-16
- **Authors:** Архитектор
- **Related issues:** [Фича 0172](0172-empty-enum-semantics.md); прямое следствие [ADR 0060](0060-enum-width-shared-layer.md#архитектура-adr) (правило 3 — «пустое перечисление: трактовку выбирает цель, вопрос семантики языка вынесен кандидатом»); смежно с [ADR 0078](0078-bit-array-semantics.md#архитектура-adr) (тот же класс: язык не определил случай, ответ дала реализация), [ADR 0189](0189-anonymous-ports.md#архитектура-adr) и [ADR 0236](0236-c-unresolved-condition-refusal.md#архитектура-adr) (диагностику о программе автора даёт язык, а не чужой инструмент)
...
1. **Правило языка обязано быть решением, а не побочным эффектом.** Класс
   [0078](0078-bit-array-semantics.md#архитектура-adr): язык не определил случай — ответ дала
...
2. **Диагностика адресуется автору программы.** Правило пачки
   ([0130](0130-diagnostics-batch.md)): сообщение обязано нести
...
5. **Недостижимость ветвей должна держаться названной причиной.** Сегодня
   ...
   точности устройство, которое фича [0236](0236-c-unresolved-condition-refusal.md#архитектура-adr)
...
- Диагностика попадает в общий вход `collect_compile_diagnostics` — значит
  ...
  [0151](0151-diagnostics-batch-within-check.md): два пустых
...
- Отказ переезжает из синтаксиса в семантику: файл с пустым перечислением
  ...
  [ADR 0024](0024-lam-formatter.md#архитектура-adr)) его **отформатирует** вместо отказа.
...
- Абстракция по данным ([0068](0068-verify-data-properties.md))
...
- **Зависит от:** `нет`. Слой `enum_facts` (фича
  [0060](0060-enum-width-shared-layer.md)) закрыт, накопление
  диагностик внутри проверки (фича
  [0151](0151-diagnostics-batch-within-check.md)) закрыто, печать
  пустой формы форматтером (фича [0024](0024-lam-formatter.md))
...
[0211](0211-c-missing-start-state-diagnostic.md) и
[0212](0212-c-diagnostic-without-code.md) («диагностика ничего не
...
[0184](0184-imported-shared-variables.md)), поэтому пустое
...
[0228](0228-compile-warning-position.md)). Дублирования нет: каждое
...
- **R2. Текст называет правило и выход.** В сообщении есть имя перечисления,
  ...
  [0231](0231-diagnostic-text-no-debug.md)).
...
подъём сделала фича [0207](0207-bitwise-not-unsigned-literal.md)) —
...
задачи [0172-01](0172-empty-enum-semantics.md#разработка) и
[0172-02](0172-empty-enum-semantics.md#разработка).
