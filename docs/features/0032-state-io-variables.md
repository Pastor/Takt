...
`inout`-порт. Решение — [ADR 0032](0032-state-io-variables.md#архитектура-adr), Option C.
...
- **Related issues:** [Фича 0032](0032-state-io-variables.md);
  дефект на территории [фичи 0014](0014-state-io.md) (`ГОТОВО`);
  связь с [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr) — то же явление
...
Решение прямо продолжает линию [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr):
...
Довести решение [ADR 0032](0032-state-io-variables.md#архитектура-adr) (Option C: единое
...
|---|---|---|
| [0014](0014-state-io.md) «Сохранение и загрузка состояния» (`ГОТОВО`) | Дефект живёт на территории 0014 (`state_io.rs`, флаги `--save-state`/`--load-state`). Но фича **закрыта**, а закрытая фича зависимостью быть не может (правило 17; тот же разбор, что для 0012 в [анализе 0025](0025-simulator-expression-eval.md#анализ)). 0032 чинит её дефект. | **не зависимость** (закрыта) |
| [0025](0025-simulator-expression-eval.md) «Починка вычислителя» (`ГОТОВО`) | 0032 опирается на `ModelNodeContext` как на единственный путь записи — механизм, зафиксированный [`0025-02b-1`](0025-simulator-expression-eval.md#разработка). Фича закрыта. | **не зависимость** (закрыта) |
| [0012](0012-simulation-core.md) «Крейт simulation» (`ГОТОВО`) | Закрыта. | **не зависимость** (закрыта) |
| [0036](0036-sim-visibility.md) «Согласование видимости публичного API `simulation`» (`СОЗДАНА`) | Пересечение проверялось **по коду**, а не по созвучию названий. 0032 меняет трейт `Context` — он `pub(crate)` (`context.rs:4`), в публичный API не входит. Публичная поверхность (`Unit::variable`, `Unit::tick`, `state_io::*`) сигнатур не меняет. | **не зависимость**; работы не пересекаются |
| [0034](0034-sim-struct-types.md) «Структурные типы в симуляторе» (`СОЗДАНА`) | `Value` не имеет структурного варианта, поэтому структурные переменные не сохранятся и после 0032. Но это ограничение **`Value`**, а не хранилища: 0032 сохраняет ровно то, что симулятор умеет вычислять. Ждать 0034 незачем — сохранение `Number`/`Real`/`Boolean`/`Array` самоценно. | **не зависимость**; ограничение фиксируется в R6 |
| [0033](0033-init-tick-alignment.md) «Согласование тактов с C» (`СОЗДАНА`) | Касается смещения трасс симулятора и порождённого C. Снимок состояния берётся из симулятора и с C не сверяется. | **не зависимость** |
...
- **Влияние на порядок разработки:** 0032 **следует поднять** в
  ...
  [0044](0044-sim-assert-invariant.md) «юнит-конструкции языка для
...
- **R6. Границы (зафиксированное ограничение, не требование).** Сохраняются
  ...
  [0034](0034-sim-struct-types.md), а не этой. Константы в снимок не
...
Реализация Option C [ADR 0032](0032-state-io-variables.md#архитектура-adr): контекст
...
[0032-02](0032-state-io-variables.md#разработка).
...
(разбор причины — [0032-01](0032-state-io-variables.md#разработка), раздел «Что было»).
...
[0032-01](0032-state-io-variables.md#разработка). Закрывает **R2, R3, R6, R7**.
...
[0032-01](0032-state-io-variables.md#разработка): пока карт две, `dump` пришлось бы
...
[0032-01](0032-state-io-variables.md#разработка)), а их падение на старом коде
...
Проверяется решение [ADR 0032](0032-state-io-variables.md#архитектура-adr) (Option C:
...
- **Класс дефекта закрыт** (драйвер № 1 ADR): у значения один адрес, четвёртый
  потребитель (`assert` из [0044](0044-sim-assert-invariant.md)) не
...
[0032-01](0032-state-io-variables.md#разработка)…[0032-03](0032-state-io-variables.md#разработка)):
