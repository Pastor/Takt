...
- **Статус:** **ГОТОВО** (2026-07-19). Развилка разрешена заказчиком в пользу
  **LSP4IJ-варианта (Option B)** после принятия LSP4IJ в фиче
  ...
  Подзадачи Option C [`0039-01`](0039-intellij-reformat.md#разработка),
  [`0039-02`](0039-intellij-reformat.md#разработка),
  [`0039-03`](0039-intellij-reformat.md#разработка) **сняты как неактуальные**.
...
- **Связанные issue (анализ):** новая фича (из блока «Кандидаты в фичи»
  ...
  [0024-04](0024-lam-formatter.md#разработка), раздел «Находка»)
...
PSI-структуры — поэтому плоский `LamParser` (решение [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr))
...
- **Status:** Accepted — с **обновлением решения от 2026-07-19**: развилка
  ...
  принятие LSP4IJ в фиче [0038](0038-intellij-semantic-tokens.md).
...
- **Related issues:** [Фича 0039](0039-intellij-reformat.md); развивает
  [ADR 0024](0024-lam-formatter.md#архитектура-adr) (ядро форматтера) и наследует принципы
  [ADR 0022](0022-intellij-syntax-highlight.md#архитектура-adr) / [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr)
  (самодостаточность плагина); смежные фичи — [0038](0038-intellij-semantic-tokens.md),
  [0040](0040-intellij-psi-parser.md)

### Context

Фича [0024](0024-lam-formatter.md) дала **каноническое ядро печати**
...
[0024-04](0024-lam-formatter.md#разработка) зафиксировала находку:
...
Фича [0040](0040-intellij-psi-parser.md) строит настоящее PSI-дерево;
...
- Одной инфраструктурой закрывается и фича [0038](0038-intellij-semantic-tokens.md):
...
решает принять LSP4IJ ради фичи 0038» (см. «Явная развилка для заказчика» выше).
Это условие **наступило**: фича [0038](0038-intellij-semantic-tokens.md)
...
задачи 0024-04; клиент LSP4IJ и маппинг языка — с фичи 0038. Фича 0039
**схлопывается до приёмки** (как и предсказывал аналитик): автотест A2 +
документация. Подзадачи разработки [0039-01](0039-intellij-reformat.md#разработка)
/ [0039-02](0039-intellij-reformat.md#разработка) /
[0039-03](0039-intellij-reformat.md#разработка) описывали Option C и **сняты как
...
Версия плагина — уже `0.5.0` (поднята фичей 0038; форматирование — часть того же
...
[0024](0024-lam-formatter.md). Ядро печати (`grammar/src/format/`,
`format_source`) готово; готовы и два его потребителя — `lamc fmt` и LSP
`textDocument/formatting`. Не хватало **третьего** — плагина IDE.

[ADR 0039](0039-intellij-reformat.md#архитектура-adr) принял **Option C**: внешний
...
Витрина `FEATURES.md` и задача [0024-04](0024-lam-formatter.md#разработка)
...
#### Фича [0040](0040-intellij-psi-parser.md) (полноценный PSI-парсер) — зависимости **нет**
...
#### Фича [0038](0038-intellij-semantic-tokens.md) (semantic tokens через `lam-lsp`) — зависимости **нет**, но есть **связь по решению**
...
#### Фичи [0022](0022-intellij-syntax-highlight.md) / [0023](0023-intellij-navigation-include.md) — **закрыты** (`ГОТОВО`)

Обе в статусе `ГОТОВО`; их артефакты — фундамент фичи (подпроект, сборка, тесты,
`LamFileType`). Блокировать нечему.

#### Фича [0024](0024-lam-formatter.md) (ядро форматтера) — **закрыта** (`ГОТОВО`)
...
> LSP4IJ в фиче [0038](0038-intellij-semantic-tokens.md). Если LSP4IJ
...
| LSP `textDocument/formatting` | `grammar/src/bin/lam_lsp.rs:50` (capability), `:144–162` (обработчик); ядро — `grammar/src/lsp.rs:146` | готово (задача [0024-04](0024-lam-formatter.md#разработка)) |
...
| **`LamParser` — плоский** | `parser/LamParser.kt:15–24` | **подтверждено**: цикл `advanceLexer()` под одним `rootMarker`; структура **не строится** — осознанное решение [ADR 0023](0023-intellij-navigation-include.md#архитектура-adr), Option A |
...
Задача [0024-04](0024-lam-formatter.md#разработка) и витрина `FEATURES.md` исходили из
...
1. **`LamExternalFormatter : AsyncDocumentFormattingService`**
   ...
   - пустой stdout при коде `0` → тоже отказ (защита от «отформатировать во
     что-то»; R4);
   - путь к бинарнику — из резолвера задачи [0039-02](0039-intellij-reformat.md#разработка)
...
| Настройки пути к `lamc` | вынесено | задача [0039-02](0039-intellij-reformat.md#разработка) |
| Тесты «байт-в-байт» | вынесено | задача [0039-03](0039-intellij-reformat.md#разработка) |
...
[ADR 0039](0039-intellij-reformat.md#архитектура-adr) принял Option C, сознательно
...
4. **Диагностика** (R6): при неудаче реформата — `Notification` с указанием, что
   ...
   - `lamc` отработал, но вернул ошибку — **это не проблема настройки**, а отказ
     ...
     [0039-01](0039-intellij-reformat.md#разработка), R4).
...
задача [0024-04](0024-lam-formatter.md#разработка)) сверяет вывод `formatting_edits` с
...
Проверка самого канона печати — область фичи [0024](0024-lam-formatter.md)
...
| [0024](0024-lam-formatter.md) Ядро форматтера | канон печати не дублируется и не переопределён: IDE отдаёт **ровно** вывод `lamc fmt` | T1–T4 | ⬜ |
| [0024](0024-lam-formatter.md) Контракт `lamc fmt --stdin` | потребляется как есть; правок ядра не требуется | T5, T18 | ⬜ |
| [0022](0022-intellij-syntax-highlight.md) Подсветка | не сломана; работает и **без** `lamc` | T11, T23 | ⬜ |
| [0023](0023-intellij-navigation-include.md) Навигация, плоский `LamParser` | инвариант «структура не строится» **подтверждён**, а не отменён | T15–T17, T23 | ⬜ |
...
- **Фича:** [0039](0039-intellij-reformat.md)
- **ADR:** [0039](0039-intellij-reformat.md#архитектура-adr) (Option C → **обновление
  решения 2026-07-19: Option B, LSP4IJ**)
- **Тест-план:** [0039](0039-intellij-reformat.md#тест-план)
- **Дата:** 2026-07-19
...
**Option B (LSP4IJ)** — после закрытия фичи [0038](0038-intellij-semantic-tokens.md)
...
объём подзадач [0039-01](0039-intellij-reformat.md#разработка)/[02](0039-intellij-reformat.md#разработка)/[03](0039-intellij-reformat.md#разработка).
