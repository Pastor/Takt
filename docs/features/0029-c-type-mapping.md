...
- **Статус:** **ГОТОВО** (закрыта 2026-07-15). Оба вопроса, блокировавшие
  ...
  фича, правило 22); фича [0028](0028-c-generator-stubs.md) берёт **`CC-018`**.
...
- **Related issues:** [Фича 0029](0029-c-type-mapping.md); опирается на
  [ADR 0025](0025-simulator-expression-eval.md#архитектура-adr) (раздел «Пригодность эталона C» —
  там дефект впервые зафиксирован и сузил критерий A8); пересекается с
  [ADR 0020](0020-port-address-decl.md#архитектура-adr) (цель `c-hal` потребляет C-тип порта для
  ширины доступа); заблокирован [фичей 0026](0026-c-root-typedef.md)
...
Дефект выявлен при анализе [0025](0025-simulator-expression-eval.md#анализ) в разделе
...
Решение — [ADR 0029](0029-c-type-mapping.md#архитектура-adr): `Array` — гибрид
...
|---|---|---|
| [`0029-01`](0029-c-type-mapping.md#разработка) | `Array`: гибрид (бит-вектор / настоящий массив), диагностики `CC-014`/`CC-015` | 0026 |
| [`0029-02`](0029-c-type-mapping.md#разработка) | `Bit` → `uint8_t`; согласованная правка `width_from_ctype` (`c-hal`) | 0026 |
| [`0029-03`](0029-c-type-mapping.md#разработка) | `Rational` → `double`; опция `--float-width` в `GenerateOptions`/CLI | 0026 |
| [`0029-04`](0029-c-type-mapping.md#разработка) | Расширение сверки с симулятором (A8 фичи 0025); примеры в документации языка (правило 16) | 01, 02, 03 |
...
> `--float-width=32|64` — `d9c4fe0`). Блокировка фичей 0026 снята её закрытием.
...
1. **Критерий A8 фичи 0025 сужен.** `0025-simulator-expression-eval.md#анализ`:
...
| T21 | полный прогон зелёный | ✅ 1753 теста, `precheck.sh` = 0; clippy — **новых предупреждений нет** (фон ~120 — объём фичи 0046) |
...
**Предусловие всего плана:** фича [0026](0026-c-root-typedef.md)
...
> предупреждений (объём фичи [0046](0046-build-warnings-cleanup.md)),
...
2. **Фича 0028 берёт `CC-018`.** Конфликт разрешён в пользу 0029 (реализована,
   ...
   проставлены в карточку [0028](0028-c-generator-stubs.md) и задачу
...
**Проверка:** 1753 теста зелёные, `precheck.sh` = 0; clippy — **новых предупреждений не добавлено** (фон ~120 предсуществующих — объём фичи 0046).
...
2. **Фича [0028](0028-c-generator-stubs.md) берёт `CC-018`.** Конфликт кодов
