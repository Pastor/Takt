...
### Задача 0390-01: разворот порта в семантике
...
не знают (приём 0143/0192/0397/0400), и пять отказов (`CC-015`, `ST-004`,
`RS-016`, `SV-002`) стали недостижимы.

⚠️ **Правая часть бывает ДВУХ видов**, и оба штатны: агрегат (`po := {1, 2};`)
...
⚠️ **Адрес листа — базовый плюс смещение поля** в байтах, формой `Number`, а
...
⚠️ **Разворот зовут только те цели, что порт не умеют** (`c`, `c-hal`, `rust`,
...
### Задача 0390-02: единая точка понижений
...
- `takt-lang/tests/targets/port_composite_tests.rs` — тест 0350, закреплявший
  отказ `CC-015`, переписан на проверку перевода (класс 0191).
...
[0413](0413-rust-field-condition.md): сравнение поля структуры в условии цель
...
⚠️ **Решение заказчика — Option C**, тогда как аналитик предлагал Option A
...
⚠️ Порт-**массив** остаётся за границей: у `sv` его отвергает шапка модуля
