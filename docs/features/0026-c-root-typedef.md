...
- **Связанные issue (анализ):** новая фича (из блока «Кандидаты в фичи»
  ...
  [0020-05](0020-port-address-decl.md#разработка), раздел «Найденное попутно»).
...
- **Related issues:** [Фича 0026](0026-c-root-typedef.md); наследует
  локальную правку задачи [0020-05](0020-port-address-decl.md#разработка)
  (цель `c-hal`, [ADR 0020](0020-port-address-decl.md#архитектура-adr))
...
Архитектурное решение — [ADR 0026](0026-c-root-typedef.md#архитектура-adr), Option A
...
- **Зависит от:** **нет.**
  ...
  - **По коду.** Правка целиком внутри `generate_header`
    ...
    ветки не касается.
  - **Фича [0020](0020-port-address-decl.md) (`c-hal`) — закрыта.**
    Дефект найден попутно её задачей [0020-05](0020-port-address-decl.md#разработка),
  ...
  - **Фича [0025](0025-simulator-expression-eval.md) — закрыта.**
    Потребитель дефекта (обёртка в фикстуре сверки), не блокер.
  - **Смежные кандидаты по генератору C — пересечения нет:**
    [0028](0028-c-generator-stubs.md) (заглушки) правит
    `generator/c/c_model.rs:316,769`; [0029](0029-c-type-mapping.md)
...
> [ADR 0026](0026-c-root-typedef.md#архитектура-adr), Option A.
...
[0029](0029-c-type-mapping.md) — её критерий «порождённый C
...
[отчёт о тестировании](0026-c-root-typedef.md#отчёт-о-тестировании). Фиксов не заводилось.
