...
0061) — в [ADR 0097](0097-pid-regulator-example.md#архитектура-adr).
...
- **Related issues:** [Фича 0097](0097-pid-regulator-example.md);
  строится на [ADR 0061](0061-fixed-point-type.md#архитектура-adr) (тип `q(m, n)`); расширяет
  пропорциональный `examples/regulator.lam` (задача 0061-05)

### Context

Фича [0061](0061-fixed-point-type.md) дала языку дробный тип
...
anti-windup) — [ADR 0097](0097-pid-regulator-example.md#архитектура-adr), Option A
(позиционный ПИД с anti-windup, объект 1-го порядка). Компилятор **не** меняется —
фича живёт в `examples/` + сценарный контракт.

### Зависимости фичи (правило 17/19)

- **Зависит от:** **[0061](0061-fixed-point-type.md)** (жёстко) — тип
...
anti-windup и объектом первого порядка (Option A [ADR 0097](0097-pid-regulator-example.md#архитектура-adr)).
...
Приведено в [ADR 0097](0097-pid-regulator-example.md#архитектура-adr): непрерывный ПИД,
...
[ADR 0097](0097-pid-regulator-example.md#архитектура-adr); отчёт —
...
громоздок) — кандидат A-5 [ADR 0061](0061-fixed-point-type.md#архитектура-adr).

> **Пост-закрытие ([фикс 0097-01](../fixes/0097-01-pid-native-float.md), 2026-07-19).**
> По решению заказчика пример переведён с явного `q(8, 8)` на прозрачный `float`
> (механизм фичи [0096](0096-fixed-point-native-float.md)): тип в исходнике —
