...
|---|---|---|---|
| Язык (синтаксис/семантика) | **меняется** — новый тип; версия **не растёт** (цикл 0.3.0 не выпущен) | T1–T7, T17, T18 | ⬜ |
| Симулятор | **эталон** Q-арифметики | T9, T10, T19 | ⬜ |
| Цель `c` / `c-hal` | `q` работает; ловушка C11 закрыта | T10, T11, T12 | ⬜ |
| Цель `rust` | `q` работает (`>>` знакового в Rust **определён**) | T10 | ⬜ |
| Цель `st` / `st-at` | `q` работает; сдвигов над числами нет → преобразования в месте операции | T10 | ⬜ |
| Цель `sv` | **ради неё фича**: синтезируется | T8, T10 | ⬜ |
...
| `float` | **не задет** | T13 | ⬜ |
| Корпус | **не задет** (0 `float`) | T14, T17 | ⬜ |
