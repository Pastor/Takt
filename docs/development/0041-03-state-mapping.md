...
> **Часть 1 реализована 2026-07-15: простые модели** (`st_model.rs`, 12
> юнит-тестов). **Часть 2 не начата:** композиция `M1 | M2` / `M1 + M2`,
> плетение `VAR_IN_OUT`, подключение к `mod.rs` (шапка POU: `state`, `is_done`,
> поднятые объявления).
