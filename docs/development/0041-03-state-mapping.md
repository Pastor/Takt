...
> **Часть 1 (простые модели) и часть 2 (композиция `M1 | M2`, `VAR_IN_OUT`,
> подключение к `mod.rs`) реализованы 2026-07-15** — `st_model.rs`, 15
> юнит-тестов. **Часть 3 не начата:** последовательная композиция `M1 + M2` и
> **новый блокер — функции, читающие состояние модели** (см. ниже).
>
...
|---|---|---|
| `comprehensive` | ✅ | ❌ функции читают состояние модели |
| `elevator_mini` | ✅ | ❌ то же |
| `stacker` | ✅ | ❌ то же |
| `elevator` | ⛔ `ST-011` (конкатенация) | — |
| `extend_complex` | ⛔ `ST-011` (конкатенация) | — |
...
### НОВЫЙ БЛОКЕР: функции Lam читают состояние модели, а `FUNCTION` в ST — чистая
...
**Направление решения (часть 3):** тот же приём, что для под-моделей — считать по
`compute_usage`, какие переменные корня функция трогает, и добавить их в её
`VAR_IN_OUT`, а вызовы дополнить аргументами из **единого источника истины**
(`StMap::shared_variables`). Константы (`MAX_TEMP`) можно объявлять
`VAR CONSTANT` внутри самой функции. Требует пробы: принимает ли MatIEC
`VAR_IN_OUT` у `FUNCTION`.
