...
- **`VAR_GLOBAL` vs `VAR_EXTERNAL`.** Размещённые переменные объявляются
  `VAR_GLOBAL`; FB, их использующий, по стандарту должен объявить `VAR_EXTERNAL`.
  Требует ли этого MatIEC — **проба П8** (0041-06). **Пересекается с вопросом О1**
  задачи [0041-03](0041-03-state-mapping.md): если там победит `VAR_GLOBAL`
  (О1-а) — решения **сливаются**.
