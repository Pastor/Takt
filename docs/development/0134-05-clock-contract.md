...
>
> **Статус: реализована** (вместе с CLI-частью 0134-08). Разбор подкоманды
> `compile` вынесен в библиотеку (`takt_lang::compile_cli`), добавлен флаг
> `--tick-hz`, `resolve_profile` превращён в сверку (`SE-069`/`SE-070`), в
> порождённый C эмитится отпечаток `_Static_assert` (идея 3), каждая длительность
> печатается (`SE-071`). `precheck.sh` зелёный.
