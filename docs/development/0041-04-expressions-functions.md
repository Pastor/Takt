...
6. **`extern fn`** — **РИ5, открытый вопрос.** В чистом ST объявить функцию без
   тела **нельзя**. Варианты: (а) эмитить `FUNCTION` с пустым телом +
   предупреждение `ST-009`; (б) не эмитить, требовать внешний файл + `ST-009`;
   (в) флаг `--st-extern-header`. **Решается пробой П9 задачи 0041-06.**
7. **Диагностики:**
