...
| `SE-093` | Начальное значение выходного порта выставляют **пока не все** цели: `c`/`c-hal`/`rust` умеют (задача 0187-03), `sv`/`sv-mmio`/`st`/`st-at` — нет (предупреждение; снимается задачей 0187-04, фича 0187) | `takt-lang/src/semantic/validate/port_init.rs` |
