...
| `CC-023` | Узел не прошёл семантическое понижение и в C не переводится: неразрешённое условие, выражение или оператор (вид назван в тексте, позиция — у узла). Защита в глубину: из корректной программы состояние недостижимо — его отсекает семантика (`SE-025`/`SE-003`), фича 0236 | `takt-lang/src/generator/c/c_unresolved.rs` |
