...
| `CC-021` | Обращение к ячейке по адресу целью `c` не транслируется: её порты идут через колбэки HAL, адресов она не знает (ADR 0020). Доступ по адресу дают `c-hal`, `st-at`, `sv-mmio` (фича 0189, решение 4A) | `takt-lang/src/generator/c/c_anon.rs` |
...
| `RS-024` | Параметр целевой модели не найден при печати аргумента инстанцирования (фича 0185) | `takt-lang/src/generator/rust/rust_model.rs` |
...
| `SE-082` | **RETIRED** (0185-04): временный сторож «аргументы инстанцирования не применяются» (0185-02) снят — значения применяются всеми целями. Номер не переиспользовать | — |
...
| `SE-084` | Функция не вычисляется при компиляции: `extern`, встроенная, нет `return`, несовпадение числа аргументов | `takt-lang/src/semantic/const_eval/call.rs` |
...
| `SE-086` | Специализация модели: имя занято либо параметр/родитель не найден (фича 0185) | `takt-lang/src/semantic/specialize.rs` |
| `SE-087` | Специализация модели с собственными вложенными моделями не поддерживается (фича 0185) | `takt-lang/src/semantic/specialize.rs` |
| `SE-088` | Параметр там, где нужна величина, известная при генерации (`after PARAM`, адрес порта): константой он становится только при `--parameters=specialize` — сообщение называет режим (фича 0185) | `takt-lang/src/semantic/parameter_const.rs` |
| `SE-089` | Числовой литерал не помещается в тип приёмника (`var a: u8 := 300;`): лексер принимает объединение диапазонов всех целых типов, а влезает ли значение в конкретный — знает семантика (фича 0157) | `takt-lang/src/semantic/validate/literal_range.rs` |
| `SE-090` | Однобитный порт с адресом без позиции бита: принят бит 0 (предупреждение). Умолчание нормируется слоем адресов — до `c-hal`/`st-at`/`sv-mmio` и обеих выгрузок (фича 0176) | `takt-lang/src/address_map/resolve.rs` |
| `SE-091` | **RETIRED** (фича 0187): предупреждение о подозрительном адресе 0/1 потеряло предмет — размещение задаётся `at`, а `:=` означает начальное значение, то есть перепутать их больше нечем. Номер не переиспользовать под другой смысл | — |
| `SE-092` | Начальное значение у **входного** порта: значение входа приходит извне, задавать его нечем (фича 0187) | `takt-lang/src/semantic/validate/port_init.rs` |
| `SE-093` | **RETIRED** (задача 0187-04): временное предупреждение «начальное значение выставляют не все цели» снято — `sv`, `sv-mmio`, `st` и `st-at` научились выставлять его наравне с `c`/`c-hal`/`rust`, и молчаливой потери, о которой оно говорило, больше нет. Номер не переиспользовать под другой смысл | — |
| `SE-094` | Начальное значение порта не вычисляется при компиляции: значение выставляется до первого такта, поэтому переменная модели источником быть не может (фича 0187, задача 03) | `takt-lang/src/semantic/declaration.rs` |
| `SE-095` | Присваивание в позиции значения там, где его пропускает грамматика: **именованный аргумент вызова функции** (`twice(a := 3)`). Форма законна лишь при инстанцировании модели, а на уровне АСД это один узел `Function` — различает только семантика. Прочие формы отсекает `SY-006` (фикс 0187-01) | `takt-lang/src/semantic/validate/assignment_position.rs` |
| `SE-096` | Запись по адресу в ячейку с необъявленным направлением (**предупреждение**): проверить законность записи компилятор не может. Текст называет способ снять — объявить именованный порт с тем же адресом (фича 0189, решение 3B) | `takt-lang/src/semantic/anon_collect.rs` |
| `SE-097` | У обращения `#АДРЕС` не задана разрядность доступа: по адресу может лежать байт, слово или бит, и выбирать за автора нельзя (класс 0029 — ширина доступа к MMIO угадыванию не подлежит) | `takt-lang/src/semantic/anon_port.rs` |
| `SE-098` | Позиция бита или разрядность поля выходит за слово доступа (`[0, 63]`, `бит + ширина ≤ 64`), либо позиция бита задана дважды, либо тип не задаёт разрядности. Пределы те же, что у именованного порта (`SE-060`, фича 0098) | `takt-lang/src/semantic/anon_port.rs` |
| `SE-099` | Обращение по адресу в инициализаторе объявления: содержимое памяти до первого такта неизвестно, и эталон дал бы ноль, а `c-hal` — чтение регистра (фича 0189) | `takt-lang/src/semantic/validate/anon_init.rs` |
...
| `SIM-034` | Аргументы инстанцирования заданы композиции без собственных состояний — значения хранить негде (фича 0185) | `takt-sim/src/unit/builder.rs` |
| `SIM-035` | Значение аргумента инстанцирования не вычислено построителем юнита (фича 0185) | `takt-sim/src/unit/builder.rs` |
...
| `ST-005` | **RETIRED** (фича 0176): умолчание «бит 0» нормируется слоем адресов и объявляется `SE-090` — печатнику локации говорить об одном решении второй раз незачем. Номер не переиспользовать под другой смысл | — |
...
| `ST-017` | Аргумент инстанцирования не печатается инициализатором экземпляра FB (фича 0185) | `takt-lang/src/generator/st/st_model.rs` |
| `ST-018` | Обращение к ячейке по адресу целью `st` не транслируется: она порождает библиотеку блоков, где порт есть вход/выход `FUNCTION_BLOCK`, а локации нет вовсе. Соберите целью `st-at` (фича 0189) | `takt-lang/src/generator/st/mod.rs` |
| `ST-019` | Поле **со смещением** (`#0xАДРЕС:БИТ as ТИП`) не выразимо локацией IEC 61131-3: она даёт либо бит (`%MX512.3`), либо целое слово (`%MB512`). Молча выронить смещение нельзя — вышло бы обращение к другому месту памяти (фича 0189) | `takt-lang/src/generator/st/mod.rs` |
...
| `SV-016` | Модель инстанцирована с разными наборами аргументов: уплощение цели `sv` даёт экземплярам одни регистры, разные настройки невыразимы (фича 0185) | `takt-lang/src/generator/sv/sv_fsm.rs` |
| `SV-017` | Обращение к ячейке по адресу целью `sv` не транслируется: у RTL адресного пространства нет — сигнал приходит на вывод кристалла. Соберите парной целью `sv-mmio`, где ячейка становится регистром файла (фича 0189) | `takt-lang/src/generator/sv/mod.rs` |
...
| `SY-006` | Присваивание в позиции значения: `:=` разбирается только как оператор тела, шаг `for` и именованный аргумент вызова. Прежде форму принимала грамматика, а отвергала семантика (`SE-095`) — граница языка была описана в двух местах (фикс 0187-01) | `takt-lang/src/parse_entry.rs` |
| `SY-005` | Превышен предел вложенности разбора (`MAX_PARSE_DEPTH`): дерево программы глубже, чем переживают рекурсивные потребители АСД — печать форматтера, `Clone`, `Drop` (фича 0156) | `takt-lang/src/lib.rs` |
| `SY-007` | Выражение в позиции оператора не имеет эффекта: оператором остаются присваивание и вызов функции. Прежде язык принимал любое выражение, а отказ давал **чужой** инструмент на порождённом файле (`cc -Wall -Werror`: `-Wunused-value`) — фича 0189, решение 6A | `takt-lang/src/parser/lex_error.rs` |
| `SY-008` | Адресный литерал `0xАДРЕС:бит` стоит там, где вычисляется значение: адрес есть свойство размещения (`at`, оператор `address`, внешняя карта), а ячейка адресуется формой `#АДРЕС as ТИП`. Прежде форма принималась и давала молчаливое расхождение эталона и цели `c` (фича 0189, решение 7A) | `takt-lang/src/parse_entry.rs` |
