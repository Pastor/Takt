...
| `SE-095` | Присваивание стоит там, где вычисляется значение: в языке присваивание — **оператор** (позиции оператора — тело и шаг `for`). Прежде форму не отвергал никто: `c` печатал `void + int`, `rust` — `() + {integer}`, `st` — невалидный ST, отказ приходил от чужого инструмента (фича 0187, ось 4) | `takt-lang/src/semantic/validate/assignment_position.rs` |
