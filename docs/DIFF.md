...
- **Модель времени.** **Физическая, встроенная в язык.**
  *Механизм:* длительность — **литерал с единицей** (`ms`, `s`, `min`, `h`),
  допустимый прямо в операторах ожидания. Рантайм ведёт внутренние часы и
...
| Céu | **литерал длительности** | `await 3s`, `every 500ms` | мс/с/мин/ч | рантайм | единицы |
...
1. **Процесс, который естественно раскладывается на последовательность операций
   ...
   последовательная композиция является лишь у двух позиций из 26. ⚠️ С оговоркой раздела 11.7:
...
- **T4. Это второе осознанное исключение из принципа «CLI не меняет логику».**
  ...
  фичи 0096). Флаг частоты меняет численный результат так же — и должен быть
...
⚠️ **Конфликт, который придётся решить явно.** На ПЛК штатный `TON` отсчитывает
...
пишется руками**; работает на всех целях без рантайма; ловит переполнение и
...
⚠️ Всё перечисленное — **предложение аналитика**, а не решение. Выбор варианта,
...
⚠️ **Главное: самая уникальная возможность хуже всего проверена.** Симулятор
**не исполняет** последовательную композицию с переходом `next` (открытый фикс
0057-01): берёт `next` сразу, шаги не тикает. Поэтому единственный пример корпуса
...
не подаётся и на гейт синтеза. Вывод: **прежде чем продвигать композицию как
...
1. **Сначала — довести существующее до проверяемости:** закрыть фикс 0057-01
   (симулятор исполняет `+` с `next`), добавить SV-транслируемый пример с `+` в
   корпус и в гейт синтеза. Без этого шага любое продвижение композиции как
...
> хватает трёх вещей: доказанной проверками зрелости (фикс 0057-01), параметров и
> интерфейсов моделей.
