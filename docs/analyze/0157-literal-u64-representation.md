...
- **Влияние на порядок разработки:** ничего не блокирует и не разблокирует
  ...
  `i64`). Фича меняет версию языка `0.8.0 → 0.9.0` (правило 22).
...
| A9 | версия языка `0.9.0` согласована в трёх источниках | `scripts/check-language-version.sh` |
...
|---|---|---|
| `0157-01` | Носитель `i128`: лексер, `token.rs`, `grammar.lalrpop`, `ast.rs`; приём `[i64::MIN, u64::MAX]`; `LE-009` переписан; каскад `SY-002` снят | тесты лексера/парсера; A7 |
| `0157-02` | Носитель в семантике: `ExpressionNode`/`ConditionNode`, три константных вычислителя, `enum_facts`, `bit_vector`, `lower_float`, `type_inference` | `cargo test`; R2 |
| `0157-03` | `SE-089` — литерал вне диапазона типа приёмника (+ реестр диагностик) | A3 |
| `0157-04` | Симулятор: `Value::Number(i128)`, вычисление и обёртка, три адаптера узла, `state_io` через `Number::from_i128` | A2, A8 |
| `0157-05` | Цель `c`: суффикс `ULL` по значению | A5 |
| `0157-06` | Цель `sv`: размерная форма по ширине приёмника (расширение `Scope`) | A4 |
| `0157-07` | Цели `st`/`rust`/`plantuml`: проверка печати, фикстуры, снапшоты; сверка `examples/generated/` | A1, A6 |
| `0157-08` | Документирование (правило 24): `book/src/02-lexical`, `book/src/03-types`, реестр диагностик, версия языка `0.9.0`, `CLAUDE.md` | A9, A10 |

Порядок обязателен: `01 → 02` (без носителя в семантике дерево не собирается),
далее `03`–`07` независимы между собой, `08` — последней.
