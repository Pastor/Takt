...
жёсткая ошибка `CC-014` вместо комментария; для №2 — удаление ветки с заменой
...
сборки** — цена перехода на `CC-014` нулевая.
...
- **Влияние на порядок разработки:** фича **не блокирует** другие и сама не
  ...
  3. 0029 работает с теми же дефектными типами (`Rational`); ошибка `CC-014`
...
- **R2. Диагностика сохраняет причину.** Ошибка получает новый код **`CC-014`**
...
- **R4. Артефакты не создаются при ошибке.** При `CC-014` файлы `.c`/`.h` не
  пишутся (или не остаются на диске), `lamc` печатает «Ошибка компиляции
  [CC-014]: …» и завершается с кодом `1`.
...
- **R9. Публичный API не ломается.** `compile_to_c` сохраняет сигнатуру
  `Result<(), Diagnostic>` (`lib.rs:209`) — `CC-014` идёт по существующему
...
- **R10. Цель `c-hal` не расходится с `c`.** Обе цели используют
  `generate_state_transitions`; поведение `CC-014` одинаково для `-t c` и
  `-t c-hal`.
...
|---|---|---|
| A1 | Проба `float`-порта даёт ошибку `CC-014`, код возврата ≠ 0 | тест `c_stub_tests.rs::float_bit_access_is_error`; ручной прогон `lamc compile -t c` на фикстуре |
| A2 | Текст `CC-014` содержит целевое состояние и причину `CC-001` | assert по подстрокам в тесте (строки захватить зондом, не угадывать — `CLAUDE.md`) |
| A3 | Ошибка указывает на позицию `ref` в исходнике (не `Codegen`) | assert `matches!(diag.loc, Location::Source(..))` в тесте |
| A4 | При `CC-014` файлы `.c`/`.h` не созданы | тест: `assert!(!path.exists())` после неуспешной компиляции |
...
| `.lam` с непереводимым условием | компилируется, код `0`, мёртвый C | **ошибка `CC-014`**, код `1`, C не создан | **слом** — обоснование ниже |
...
|---|---|---|---|
| Р1 | Существует «в поле» `.lam`, полагающийся на текущее поведение | низкая: в репозитории таких нет; поведение бессмысленно (мёртвый автомат) | текст `CC-014` называет точную причину и строку → правка очевидна |
...
|---|---|---|---|
| **0028-01** | [`../development/0028-01-c-generator-stubs.md`](../development/0028-01-c-generator-stubs.md) | `CC-014`: проброс `Err` вместо печати `//TODO`; позиция `ref`; проверка неизменности вывода на корпусе | R1, R2, R3, R4, R8, R9, R10 |
...
| **0028-03** | [`../development/0028-03-fixtures-tests.md`](../development/0028-03-fixtures-tests.md) | фикстуры (пример/контрпример), тесты `c_stub_tests.rs`, защита корпуса от возврата заглушек, документация `CC-014` | A1–A12, правило 16 |
