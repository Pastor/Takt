...
⚠️ **Сдвиг на нуль не эмитится** (`1 << 0` — `clippy::identity_op`, то есть
отказ гейта под `-D warnings`). Нулевой бит в корпусе обычен.
...
За отказом остаётся **одна** форма — запись среза. Держателя в
`scripts/stub-branches.txt` он больше не имеет: отказ становится
**`ПОСТОЯННЫМ`**, потому что срез не переводит ни один потребитель и эталон
отказывает **заодно с целями**, а не вместо них. Текст правится под одну
оставшуюся форму.
...
8. `SIM-017` достижим только записью среза; запись реестра заглушек —
   `ПОСТОЯННЫЙ`; гейт `scripts/check-stub-branches.py` зелёный.
