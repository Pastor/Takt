...
| `b.2 := 1` (запись бита) | **да** | все четыре переводят: `c` печатает `model->b = (model->b & ~(1u << 2)) \| ((1 & 1u) << 2);` | `SIM-017` |
| `arr[0:2] := 1` (запись среза) | да, но не транслируется | `CC-022`, `RS-011`, `ST-011`, `SV-002` — все отказывают | `SIM-017` |

Средняя строка — **самостоятельный дефект того же класса**: запись бита
исполняют все цели, а эталон её не исполняет. Тот же класс, что 0248 (эталон
отстаёт от целей), найденный тем же гейтом заглушек.
...
3. **Эталон не имеет права отставать от целей** (уроки 0245, 0248). Запись
   бита `x.N := v` переводят все четыре цели; отказ эталона на ней — прямое
   расхождение трассы и прошивки.
...
| бит | `b.2 := 1;` | четыре цели; эталон — **этой фичей** |
...
После фичи ветвь остаётся достижимой **только** записью в срез, поэтому:

1. `resolve_place` эталона получает разбор `BitAccess(_, Member::Number)` —
   запись бита исполняется, как у четырёх целей;
2. текст `SIM-017` переписывается на **правду**: речь о срезе, а не о «не
   переменной, не поле и не элементе»;
3. держатель в `scripts/stub-branches.txt` меняется с `0249` на
   `ПОСТОЯННЫЙ` — отказ не временный: срез не транслирует ни один потребитель,
   и эталон отказывает **заодно с целями**, а не вместо них. Полная поддержка
   среза — отдельная работа, выносится кандидатом.
...
- Эталон догоняет цели по записи бита; расхождение трассы и прошивки на `x.N`
  снимается.
- Реестр заглушек перестаёт обещать работу, которой не будет: `SIM-017`
  становится названным постоянным отказом на одной конструкции вместо
  безымянного «пока не поддерживается» на трёх.
...
2. Законные места (`n`, `p.x`, `p.y.v`, `arr[1]`, `b.2`, `led`, `#0x200.5`,
   ...
   называя осиротевшее.
3. `b.2 := 1;` исполняется эталоном; **потактовая сверка** эталона с целью `c`
   на накапливающем теле даёт совпадающие трассы.
4. Накопление: несколько незаконных левых частей в одном теле дают
   **несколько** диагностик, а не первую (образец `literal_range`, 0157).
5. Реестр заглушек: запись `SIM-017` держателя не имеет (`ПОСТОЯННЫЙ`), текст
   ветви говорит о срезе, гейт `scripts/check-stub-branches.py` зелёный.
