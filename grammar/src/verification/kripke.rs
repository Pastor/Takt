...
//! - **Данные вне охвата.** `var`/порты/`cond` в разметку не входят; атом, не
//!   являющийся именем состояния, не молча ложен, а даёт `Verdict::Unsupported`
...
impl Kripke {
    /// Оценка атома в вершине: атом `S` истинен в `k` ⟺ `states[k] == S`.
    ///
    /// Атом, не совпадающий ни с одним именем состояния, ложен **во всех**
    /// вершинах. Молча полагаться на это нельзя (иначе `F temp` тихо «не
    /// держится» вместо честного отказа) — вызывающий обязан сперва отсеять
    /// такие атомы через [`unknown_atoms`].
