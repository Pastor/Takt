...
always {
    ...
    fn syntax_simple() {
        ...
        // 0028-01 ВСКРЫЛА: условие `S(Ping) = End` в C НЕ ПЕРЕВОДИТСЯ ВОВСЕ
        // (`CC-003`: «Ссылки на модели и состояния не поддерживаются в условиях
        // переходов»). Прежде здесь стоял `.unwrap()`, и тест был зелёным —
        // но не потому, что переход генерировался, а потому, что генератор
        // ПРОГЛАТЫВАЛ ошибку, печатая `//TODO` вместо перехода. То есть тест
        // подтверждал успех там, где автомат молча терял ребро.
        //
        // Утверждение анализа 0028 «дефект латентен: ни один .lam корпуса эту
        // ветку не задевает» верно для `examples/`, но НЕ для этого эталона —
        // и вскрылось только здесь.
        //
        // Ожидание приведено к факту, а не к желаемому: пока трансляция
        // `S(Модель) = Состояние` в C не реализована (кандидат в `FEATURES.md`),
        // цель `c` обязана ОТКАЗАТЬ громко. Когда трансляцию сделают — тест
        // упадёт и заставит вернуть сюда проверку успеха.
        let diag = generate(
            Language::C,
            &model.borrow(),
            ".output",
        ...
        .expect_err("условие S(Ping) = End в C не переводится — ожидается CC-018");
        assert_eq!(diag.code.as_deref(), Some("CC-018"));
        assert!(
            diag.notes.iter().any(|n| n.message.contains("CC-003")),
            "исходная причина обязана дойти заметкой: {:?}",
            diag.notes
        );
    }
