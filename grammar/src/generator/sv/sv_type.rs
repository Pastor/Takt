...
// Потребители отображения появляются в задачах 0045-04 (порты), 0045-05
// (состояния) и 0045-06 (выражения); до тех пор модуль покрыт только своими
// юнит-тестами, а `cargo` считает его мёртвым. Пометка **временная** и снимается
// задачей 0045-04 — там же, где `sv_type` вызовется впервые.
#![allow(dead_code)]
