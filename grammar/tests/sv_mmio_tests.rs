...
fn compile(tag: &str, source: &str) -> String {
    let dir = out_dir(tag);
    let env = grammar::parse_defines(&[]).expect("env");
...
fn compile_err(tag: &str, source: &str) -> grammar::diagnostics::Diagnostic {
    let dir = out_dir(tag);
    let env = grammar::parse_defines(&[]).expect("env");
    grammar::compile_to_sv_mmio(
        tag,
...
fn port_without_address_stays_module_port() {
    // `plain_in` без инициализатора → адреса нет; `reg_out := 0x200:0` — адрес.
    // (`:= <число>` у порта — это inline-адрес, а не значение, поэтому «порт без
...
fn external_address_map_is_accepted() {
    ...
    let external = grammar::parse_address_map("sig = 0x2A:0;", 0).expect("карта");
    let warnings = grammar::compile_to_sv_mmio(
        "extmap",
