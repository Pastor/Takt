...
fn env_of(args: &[&str]) -> grammar::AddressEnv {
    let owned: Vec<String> = args.iter().map(|s| s.to_string()).collect();
    grammar::parse_defines(&owned).expect("аргументы define должны разбираться")
}

fn codes_of(r: &grammar::AddressResolution) -> Vec<&str> {
    r.diagnostics
        .iter()
...
fn parse_defines_rejects_bad_format() {
    for bad in ["BTN_ADDR", "=0x1", "1BAD=0x1", "a-b=0x1"] {
        let diags = grammar::parse_defines(&[bad.to_string()])
            .err()
