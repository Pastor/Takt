...
mod goto_exact_file {
    ...
    /// T9/A5: угадывание удалено.
    #[test]
    fn t9_guessing_is_gone() {
        let lsp_rs = std::fs::read_to_string("src/lsp.rs").expect("не прочитать src/lsp.rs");
        assert!(
            !lsp_rs.contains("fn to_snake_case"),
            "`to_snake_case` — угадывание имени файла по имени модели — обязана \
             быть удалена: путь берётся из реестра файлов (0053)"
        );
    }
