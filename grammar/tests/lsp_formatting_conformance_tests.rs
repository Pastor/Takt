...
//! `textDocument/formatting` → [`grammar::lsp::formatting_edits`] → тот же
//! [`grammar::format::format_source`], что и `lamc fmt`. Значит результат
...
fn apply_full_document(source: &str) -> String {
    match grammar::lsp::formatting_edits(source).expect("форматирование удалось")
    {
...
fn a2_reformat_matches_lamc_fmt_over_corpus() {
    ...
    for entry in std::fs::read_dir(&examples).expect("examples/ читается") {
        ...
        assert!(
            grammar::lsp::formatting_edits(&source)
        ...
        let roughed = format!("\n\n  {}\n\n", source.replace(";\n", ";\n\n\n"));
        let from_core = grammar::format::format_source(&roughed)
