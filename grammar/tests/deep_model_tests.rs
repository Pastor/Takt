...
fn plantuml_handles_deep_state_chain() {
    ...
    grammar::compile_to_plantuml("deep.lam", &src, out.to_str().expect("путь"), &[])
...
fn verification_handles_deep_state_chain() {
    ...
    let src = chain_source(DEEP);
    let (ast, _) = grammar::parse(&src, 0).expect("разбор");
    let model = grammar::semantic::tree::construct_model(&ast, None, &[]).expect("семантика");
    let results = grammar::verify_all(Rc::clone(&model));
