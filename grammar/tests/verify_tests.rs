//! Интеграционные тесты верификации LTL (фича 0049): вердикт `lamc verify`
...
fn model_of(fixture: &str) -> Rc<RefCell<grammar::semantic::ModelNode>> {
...
fn verdict_of(fixture: &str) -> Verdict {
    let results = grammar::verify_all(model_of(fixture));
    assert_eq!(
        results.len(),
        1,
...
fn property_string_parses_multi_char_atoms() {
    let phi = grammar::parse_ltl_property("G (Fault -> F Idle)").unwrap();
...
fn property_from_string_is_verified() {
    let phi = grammar::parse_ltl_property("F Done").unwrap();
    assert_eq!(
        grammar::verify_model(model_of("reachable.lam"), &phi),
        Verdict::Holds
    );

    let phi = grammar::parse_ltl_property("F Done").unwrap();
    assert!(matches!(
        grammar::verify_model(model_of("unreachable.lam"), &phi),
        Verdict::Violated(_)
    ));
}
...
fn malformed_property_is_an_error_not_a_panic() {
    assert!(grammar::parse_ltl_property("G (").is_err());
    assert!(grammar::parse_ltl_property("").is_err());
    assert!(grammar::parse_ltl_property("&& F").is_err());
}
...
fn property_list_is_rejected() {
    assert!(
        grammar::parse_ltl_property("F Done, G Idle").is_err(),
...
fn property_with_semicolon_is_rejected_not_truncated() {
    for property in [
        ...
        assert!(
            grammar::parse_ltl_property(property).is_err(),
...
fn verdict_str(src: &str, property: &str) -> Verdict {
    ...
    let phi = grammar::parse_ltl_property(property).expect("разбор свойства");
    grammar::verify_model(model, &phi)
}
...
fn oracle_globally(kripke: &grammar::verification::kripke::Kripke, atom: &str) -> bool {
...
fn oracle_finally(kripke: &grammar::verification::kripke::Kripke, atom: &str) -> bool {
...
fn reachable_from_start(
    kripke: &grammar::verification::kripke::Kripke,
...
fn verdicts_agree_with_brute_force_oracle() {
    ...
    for src in corpus {
        ...
        let kripke = grammar::verification::kripke::build_kripke(&model.borrow())
        ...
        for atom in ["A", "B", "C"] {
            ...
            for (property, expected) in [
                ...
                let phi = grammar::parse_ltl_property(&property).unwrap();
                let holds = matches!(
                    grammar::verify_model(Rc::clone(&model), &phi),
                    Verdict::Holds
                );
                assert_eq!(
                    holds, expected,
...
fn model_without_formulas_yields_no_results() {
    ...
    let model = construct_model(&ast, None, &[]).unwrap();
    assert!(grammar::verify_all(model).is_empty());
}
...
fn verification_terminates_on_examples_corpus() {
    ...
    for entry in std::fs::read_dir("../examples").expect("каталог examples/") {
        ...
        let phi = grammar::parse_ltl_property(&format!("F {state}")).expect("F <состояние>");

        // Сам факт возврата — и есть проверка завершаемости.
        let _verdict = grammar::verify_model(Rc::clone(&model), &phi);
        checked += 1;
    }
    assert!(
        checked > 0,
