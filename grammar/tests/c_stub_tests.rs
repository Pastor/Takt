...
//! переход не генерировался, `lamc` печатал «Скомпилировано» и завершался с
...
use grammar::{GenerateOptions, compile_to_c};
...
fn compile(fixture: &str, name: &str, out: &Path) -> Result<(), grammar::diagnostics::Diagnostic> {
...
fn cc018_same_for_c_hal() {
    ...
    let diag = grammar::generator::generate(
        grammar::generator::Language::C,
        &model.borrow(),
