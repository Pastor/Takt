...
- **Симулятор чинится (фича 0025, в работе).** Выражения и условия **уже
  вычисляются верно** (задачи 0025-01/02a/03): второго вычислителя больше нет,
  паник нет, арифметика и приведение по типу работают. **Ещё сломано:**
  `compile_statement` (`unit/builder.rs:139`) содержит `_ => vec![]` и молча
  роняет 8 из 13 вариантов `StatementNode` — `Loop`, `For`, `Match`, `Variable`
  (локальные `var`!), `Return`, `Break`, `Continue`, `InlineFormula`; вызовы `fn`
  не исполняются. Отсюда: `comprehensive.lam` всё ещё в `Idle` (роняется
  `var delta`), у `stacker.lam` `eta=0`. Чинит задача **0025-02b** (требует потока
  управления в `Execution`, который сейчас `Fn(&mut dyn Context)` без возврата).
  Также: `enter` стартового состояния не исполняется (Д5, задача 04); ошибка
  вычисления пока **неотличима** от ложного условия (`Predicate` → `bool`,
  задача 05) — до неё ошибки **печатаются** в stderr, а не теряются. Новые тесты
  симуляции пиши на вычисленные значения, а не на факт перехода (тесты были
  зелёными именно потому, что значений не сверяли). Инвентарь Д1–Д8 — в карточке
  фичи. К инварианту `ref`-условий отношения **не имеет** — условия разрешаются
  корректно. Решение
  ...
  `simulation/src/eval/` — вся семантика значений живёт **только** там (`ops.rs` —
  операции, `mod.rs::coerce_to_type` — приведение по типу, `error.rs` —
  `EvalError`/`SIM-00x`). Оба адаптера готовы и семантики не содержат:
  `predicate.rs` (условия) и `expression.rs` (выражения). Новую семантику
  вычислений добавляй в `eval/`, а не в адаптеры. `Value` переехал:
