...
- **Симулятор вычисляет модели неверно (фича 0025, в работе).** Крейт
  `simulation` надёжно исполняет только прямые присваивания (`x := 5`, `x := y`)
  в **телах блоков**: `eval_expression_rt` (`unit/builder.rs:135`) покрывает ~6 из
  ~30 вариантов `ExpressionNode` и **молча** пропускает присваивание
  (`builder.rs:220`, без `else`) — это Д1–Д3, чинит задача 0025-02. **Условия
  переходов уже исправны** (задача 0025-03): `predicate.rs` переписан на ядро,
  паники сняты. Следствия, ещё живые: `comprehensive.lam` навсегда в `Idle`, у
  `stacker.lam` `eta=0`; `enter` стартового состояния не исполняется (Д5,
  задача 04). **Не доверяй трассам симулятора до закрытия 0025**; новые тесты
  симуляции пиши на вычисленные значения, а не на факт перехода (тесты были
  зелёными именно потому, что значений не сверяли). Ошибка вычисления пока
  **неотличима** от ложного условия (`Predicate` возвращает `bool`) — задача 05.
  Инвентарь Д1–Д8 — в карточке фичи. К инварианту `ref`-условий отношения **не
  имеет** — условия разрешаются корректно. Решение
  ...
  `simulation/src/eval/` готово (0025-01) — вся семантика значений живёт **только**
  там (`ops.rs` — операции, `mod.rs::coerce_to_type` — приведение по типу,
  `error.rs` — `EvalError`/`SIM-00x`); `predicate.rs` уже переписан как адаптер
  поверх него (0025-03), `builder.rs` — **ещё нет** (0025-02). Новую семантику
  вычислений добавляй в `eval/`, а не в `builder.rs`/`predicate.rs`. `Value`
  переехал: `crate::eval::value::Value`. Модуль под
