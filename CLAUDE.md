...
- **Симулятор вычисляет модели неверно (фича 0025, ADR принят).** Крейт
  `simulation` надёжно исполняет только прямые присваивания (`x := 5`, `x := y`).
  **Не доверяй трассам симулятора до закрытия 0025**; новые тесты симуляции пиши
  на вычисленные значения, а не на факт перехода (тесты зелёные именно потому,
  что значений не сверяют). Инвентарь Д1–Д8 — в карточке фичи; кратко: два
  вычислителя разошлись — `eval_expression_rt` (`unit/builder.rs:135`) покрывает
  ~6 из ~30 вариантов `ExpressionNode` и **молча** пропускает присваивание
  (`builder.rs:220`, без `else`); `flat` (`predicate.rs`) паникует на вызове
  функции (`:111`) и на смешанной арифметике `int`+`real` (`:130`), не вычисляет
  скобки (`:78`) и возвращает `Err` на `EnumVariant`/`State`/`Model`. Ошибка
  вычисления **неотличима** от ложного условия (`create_predicate`:
  `Ok(_) | Err(_) => false`). Следствия: `comprehensive.lam` навсегда в `Idle`,
  у `stacker.lam` `eta=0`, `mode = Manual` ложно. К инварианту `ref`-условий
