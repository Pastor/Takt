...
- **Симулятор чинится (фича 0025, в работе).** Выражения, условия и операторы
  (`var`, `while`, `for`, `match`) **уже работают** (задачи 0025-01/02a/02b-1/03):
  второго вычислителя нет, паник нет, приведение по типу есть.
  **Ещё не сделано:** вызовы `fn` (Д3, Д4) и поток управления
  (`Return`/`Break`/`Continue`) — задача **0025-02b-2**; требует смены сигнатуры
  `Execution` (сейчас `Fn(&mut dyn Context)` без возврата) и `&mut` в вычислителях.
  Отсюда у `stacker.lam` `eta=0`. Также: `enter` стартового состояния не
  исполняется (Д5, задача 04); ошибка вычисления пока **неотличима** от ложного
  условия (`Predicate` → `bool`, задача 05) — до неё ошибки **печатаются** в
  stderr, а не теряются. Новые тесты симуляции пиши на вычисленные значения, а не
  на факт перехода (тесты были зелёными именно потому, что значений не сверяли).
  **`examples/comprehensive.lam` дефектен** (в бэклоге): заявленный в комментарии
  сценарий `→ Cooling → Done` недостижим по его же логике — не принимай его за
  эталон. Инвентарь Д1–Д8 — в карточке фичи. К инварианту `ref`-условий отношения
  **не имеет** — условия разрешаются корректно. Решение
