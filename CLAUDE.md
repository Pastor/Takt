...
- **Генерация C недетерминирована** (выявлено при 0026; кандидат в
  `FEATURES.md`). `states` — `HashMap` (`semantic/mod.rs:101`), поэтому порядок
  вариантов `enum` состояний и веток `switch` **меняется от запуска к запуску**.
  Отсюда: codegen-снапшоты завести нельзя, а числовые значения состояний в ABI
  «плавают» между сборками.
