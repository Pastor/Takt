...
- **Симулятор починен (фича 0025, вердикт отчёта — на доработку).** Выражения, условия,
  ...
  `#[non_exhaustive]`). **Все восемь дефектов Д1–Д8 закрыты, реализация завершена**;
  отчёт (стадия 6) дал вердикт **на доработку**; `0025-06` закрыта — есть фикстуры
  `simulation/tests/data/eval/` и интеграционный слой `simulation/tests/eval_tests.rs`
  (сверяет **значения**; новые тесты симуляции пиши туда). `0025-07` закрыта — есть автосверка с `lamc -t c`
  (`simulation/tests/conformance_c_tests.rs`). Остаётся `0025-08` (примеры в
  README). **Внимание:** порождённый C заводит синтетические `INIT`-такты (тело
  впервые идёт на 3-м такте против 1-го у симулятора) — трассы смещены, сверять
  можно только установившиеся значения (в бэклоге). Ошибка вычисления доходит до CLI:
  `Result<_, Diagnostic>` → `TickResult::Failed` → `RunResult::EvalFailed` →
  ненулевой код возврата; тихого пропуска и печати-в-никуда больше нет. Новые тесты симуляции пиши на вычисленные значения, а не на факт
  перехода (тесты были зелёными именно потому, что значений не сверяли).
  **Ловушки** (все в бэклоге): `examples/comprehensive.lam` дефектен —
  заявленный в комментарии сценарий `→ Cooling → Done` недостижим по его же
  логике, не принимай за эталон; **функции не могут вызывать функции** (семантика
  отвергает), поэтому рекурсия невозможна; **`--save-state` не сохраняет
  переменные модели** — `state_io` читает `Unit::variables`, а присваивания идут в
  кэш `ModelNodeContext` (разные хранилища; дефект фичи 0014, предсуществующий).
