...
- **Симулятор почти починен (фича 0025, в работе).** Выражения, условия,
  ...
  `#[non_exhaustive]`). **Все восемь дефектов Д1–Д8 закрыты.** Осталось: ошибка вычисления
  **неотличима** от ложного условия (`Predicate` → `bool`, задача 05) — до неё
  ошибки **печатаются** в stderr, а не теряются. Новые тесты симуляции пиши на вычисленные значения, а не на факт
