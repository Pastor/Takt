...
- **Генерация недетерминирована — цели `c` и `plantuml`** (фича
  [0048](docs/features/0048-deterministic-codegen.md), заведена). Словари
  `ModelNode` (`semantic/mod.rs:73-115`) и снимок `minimap::Map`
  (`minimap.rs:170`) — `HashMap`, поэтому **все пять** примеров корпуса дают 5
  разных `.h` из 5 прогонов одного бинарника. Плавают порядок под-моделей,
  порядок вариантов `enum` состояний и **числовые значения портов** (проба:
  `CABIN_BUTTON_DC` = 0 или = 2) — то есть ABI. Три корня: `minimap.rs:196`
  (состояния), `minimap.rs:247-253` (под-модели), `c/mod.rs:836`
  (`topological_sort_models` не сортирует вход). Цель **`st` детерминирована** —
  сортирует у себя в 13 местах (образец решения: `st/mod.rs:153` сортирует
  **перед** топологической сортировкой).
  ⚠️ Следствия для работы: **codegen-снапшоты завести нельзя** (требование
  «сверить снапшоты» уже дважды оказалось неисполнимым — 0026 и тест-план 0041);
  **дифф `examples/generated/` может быть чистым шумом перестановки** — не считай
  его регрессом, не проверив. Что **не** плавает: поля структуры и варианты
  пользовательского `enum` (это `Vec`).
