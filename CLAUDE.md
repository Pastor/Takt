...
- **Симулятор вычисляет модели неверно (фича 0025, открыта).** Крейт `simulation`
  исполняет только прямые присваивания (`x := 5`, `x := y`): `eval_expression_rt`
  (`unit/builder.rs:135`) покрывает ~6 из ~30 вариантов `ExpressionNode`, а
  невычислимое выражение **молча** пропускает присваивание (`builder.rs:220`, без
  `else`). Любая арифметика/вызов функции в `enter`/`exit`/`always` не работают:
  `comprehensive.lam` навсегда в `Idle`, у `stacker.lam` `eta=0`. Тесты зелёные —
  они не сверяют значения переменных. **Не доверяй трассам симулятора до закрытия
  0025**; новые тесты симуляции пиши на вычисленные значения. Сопутствующее:
  паника на вызове функции в условии (`predicate.rs:111`), `enter` стартового
  состояния не исполняется (`unit/mod.rs:185`). К инварианту `ref`-условий
  отношения **не имеет** — условия разрешаются корректно.
